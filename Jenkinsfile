    ...
    pipeline {
            ...
            stages {
                ...
                stage('Deploy') {
                    steps {
                        echo 'Deploying..'
                        sh 'ansible-playbook /ansible/deploy.yml -i /ansible/hosts --extra-vars "hosts=test remote_user=jenkins project_root=/cosnics/${BRANCH_NAME} project_local_path=${WORKSPACE}/ cosnics_deploy_database_name=cosnics-${BRANCH_NAME} cosnics_deploy_test_database_name=cosnics-test-${BRANCH_NAME}"'
                    }
                }
                stage('Test') {
                    steps {
