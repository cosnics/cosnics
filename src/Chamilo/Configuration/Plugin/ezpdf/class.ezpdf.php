...
class Cezpdf extends Cpdf
{
    ...
    function ezTable(&$data, $cols = '', $title = '', $options = '')
    {
        ...
        // 'showHeadings' => 0 or 1
        // 'shaded'=> 0,1,2,3 default is 1 (1->alternate lines are shaded, 0->no shading, 2-> both shaded, second uses
        // shadeCol2)
        // 'shadeCol' => (r,g,b) array, defining the colour of the shading, default is (0.8,0.8,0.8)
        // 'shadeCol2' => (r,g,b) array, defining the colour of the shading of the other blocks, default is
        // (0.7,0.7,0.7)
        // 'fontSize' => 10
        ...
        // 'minRowSpace'=> the minimum space between the bottom of each row and the bottom margin, in which a new row
        // will be started
        ...
        // 'protectRows'=>number, the number of rows to hold with the heading on page, ie, if there less than this
        // number of
            ...
            foreach ($data as $row)
            {
                ...
                while (! $abortTable && $ok == 0)
                {
                    ...
                    while (! $abortTable && ($newPage || $newRow))
                    {
                             ...
                             $y < ($this->ez['bottomMargin'] + $options['minRowSpace'])))
                        {
                            ...
                            $this->ezNewPage();
                            // and the margins may have changed, this is due to the possibility of the columns being
                            // turned on
                ...
                if ($options['splitRows'] == 0)
                {
                    if ((($this->ezPageCount != $pageStart) ||
                         ...
                         $columnStart != $this->ez['columns']['colNum'])) && $secondTurn == 0)
                    {
...
function ezText($text, $size = 0, $options = array(), $test = 0)
{
    ...
    // if $test is set then this should just check if the text is going to flow onto a new page or not, returning true
    // or false
        ...
        while (strlen($line) || $start)
        {
            ...
            else
            {
                $left = $this->ez['leftMargin'] +
                     ((is_array($options) && isset($options['left'])) ? $options['left'] : 0);
            }
            if (is_array($options) && isset($options['aright']))
            {
                $right = $options['aright'];
