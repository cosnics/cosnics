...
class Cpdf
{
    ...
    function o_pages($id, $action, $options = '')
    {
        ...
        switch ($action)
        {
            ...
            case 'page' :
                ...
                {
                    // then it should be an array having 'id','rid','pos', where rid=the page to which this one will be
                    // placed relative
        ...
        switch ($action)
        {
            case 'new' :
                ...
                $this->o_pages(
                    $this->currentNode, 
                    'xObject', 
                    array('label' => $options['label'], 'objNum' => $id));
    ...
    function encryptInit($id)
    {
        ...
        $tmp .= chr(hexdec(substr($hex, 4, 2))) . chr(hexdec(substr($hex, 2, 2))) . chr(hexdec(substr($hex, 0, 2))) .
             chr(0) . chr(0);
        $key = $this->md5_16($tmp);
        ...
        if (! isset($this->fonts[$fontName]))
        {
            ...
            if (isset($this->fonts[$fontName]))
            {
                ...
                if (substr($fontName, - 4) == '.afm' && strlen($fbtype))
                {
                    ...
                    $fdopt = array('Flags' => $flags, 'FontName' => $adobeFontName, 'StemV' => 100);                    // don't know what
                                                                                                    // the value for
                                                                                                    // this should be!
    ...
    function setCurrentFont()
    {
        if (strlen($this->currentBaseFont) == 0)
        {
            ...
            $this->selectFont('/var/www/html/hogent/pear/pdf/fonts/Helvetica.afm');
        }
...
{
    if ($r >= 0 &&
         ($force || $r != $this->currentColour['r'] || $g != $this->currentColour['g'] || $b != $this->currentColour['b']))
    {
        $this->objects[$this->currentContents]['c'] .= "\n" . sprintf('%.3f', $r) . ' ' . sprintf('%.3f', $g) . ' ' .
         sprintf('%.3f', $b) . ' rg';
...
{
$this->objects[$this->currentContents]['c'] .= "\n" . sprintf('%.3f', $x1) . ' ' . sprintf('%.3f', $y1) . ' m ' .
 sprintf('%.3f', $x2) . ' ' . sprintf('%.3f', $y2) . ' l S';
}
...
$this->objects[$this->currentContents]['c'] .= "\n" . sprintf('%.3f', $x0) . ' ' . sprintf('%.3f', $y0) . ' m ' .
 sprintf('%.3f', $x1) . ' ' . sprintf('%.3f', $y1);
$this->objects[$this->currentContents]['c'] .= ' ' . sprintf('%.3f', $x2) . ' ' . sprintf('%.3f', $y2) . ' ' .
 sprintf('%.3f', $x3) . ' ' . sprintf('%.3f', $y3) . ' c S';
}
...
$this->objects[$this->currentContents]['c'] .= ' ' . sprintf('%.3f', ($a1 - $c1 * $dtm)) . ' ' .
     sprintf('%.3f', ($b1 - $d1 * $dtm)) . ' ' . sprintf('%.3f', $a1) . ' ' . sprintf('%.3f', $b1) . ' c';
$a0 = $a1;
$b0 = $b1;
$c0 = $c1;
$d0 = $d1;
...
{
$this->objects[$this->currentContents]['c'] .= "\n" . sprintf('%.3f', $x1) . ' ' . sprintf('%.3f', $y1) . ' ' .
 sprintf('%.3f', $width) . ' ' . sprintf('%.3f', $height) . ' re f';
}
...
{
$this->objects[$this->currentContents]['c'] .= "\n" . sprintf('%.3f', $x1) . ' ' . sprintf('%.3f', $y1) . ' ' .
 sprintf('%.3f', $width) . ' ' . sprintf('%.3f', $height) . ' re S';
}
...
{
$this->setStrokeColor(
    $this->currentStrokeColour['r'], 
    $this->currentStrokeColour['g'], 
    $this->currentStrokeColour['b'], 
    1);
}
...
switch ($text[$j])
{
                ...
                if ($k !== false && $text[$j] == ':')
                {
                    ...
                    else
                    {
                        ...
                        if ($final)
                        {
                            ...
                            if (is_array($ret))
                            {
                                // then the return from the callback function could set the position, to start with,
    ...
    case 'c' :
        ...
        if ($k !== false && $text[$j] == ':')
        {
            ...
            {
                // only call the function if this is the final call, ie, the one actually doing printing, not
                // measurement
                if ($final)
                {
                    // need to assess the text position, calculate the text width to this point
                    ...
                    if (is_array($ret))
                    {
                        // then the return from the callback function could set the position, to start with, later will
                        // do font colour, and font
                        foreach ($ret as $rk => $rv)
                        {
                            switch ($rk)
                            {
...
if ($directive)
{
    ...
    if ($i > $start)
    {
        ...
        $this->objects[$this->currentContents]['c'] .= ' /F' . $this->currentFontNum . ' ' . sprintf('%.1f', $size) .
             ' Tf ';
...
while ($p < $len)
{
    ...
    switch ($chunkType)
    {
        ...
        case 'tRNS' :
            ...
            { // indexed color, rbg
                /*
                 * corresponding to entries in the plte chunk Alpha for palette index 0: 1 byte Alpha for palette index
                 * 1: 1 byte ...etc...
                 */
            ...
            { // truecolor
                /*
                 * corresponding to entries in the plte chunk Red: 2 bytes, range 0 .. (2^bitdepth)-1 Green: 2 bytes,
                 * range 0 .. (2^bitdepth)-1 Blue: 2 bytes, range 0 .. (2^bitdepth)-1
                 */
...
$this->objects[$this->currentContents]['c'] .= "\nq";
$this->objects[$this->currentContents]['c'] .= "\n" . sprintf('%.3f', $w) . " 0 0 " . sprintf('%.3f', $h) . " " .
 sprintf('%.3f', $x) . " " . sprintf('%.3f', $y) . " cm";
$this->objects[$this->currentContents]['c'] .= "\n/" . $label . ' Do';
$this->objects[$this->currentContents]['c'] .= "\nQ";
}
