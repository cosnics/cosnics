 ...
 * of PHP code XP style.
 * Adds some useful standard
 * assertions.
 * 
 * @package SimpleTest
 * @subpackage UnitTester
...
class UnitTestCase extends SimpleTestCase
{
    ...
    function assertReference(&$first, &$second, $message = '%s')
    {
        ...
        $message = sprintf(
            $message, 
            '[' . $dumper->describeValue($first) . '] and [' . $dumper->describeValue($second) .
                 '] should reference the same object');
        return $this->assertTrue(SimpleTestCompatibility :: isReference($first, $second), $message);
    }

    /**
     * Will trigger a pass if both parameters refer
     * to different objects.
     * Fail otherwise. The objects
    ...
    /**
     * Prepares for an error.
     * If the error mismatches it
