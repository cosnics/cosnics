...
class SimpleExpectation
{
    ...
    {
        return is_object($expectation) && SimpleTestCompatibility :: isA($expectation, 'SimpleExpectation');
    }
}
...
class EqualExpectation extends SimpleExpectation
{
    ...
    function testMessage($compare)
    {
        ...
        {
            return "Equal expectation fails " . $this->_dumper->describeDifference($this->_value, $compare);
        }
    }
...
class NotEqualExpectation extends EqualExpectation
{
    ...
    function testMessage($compare)
    {
        ...
        if ($this->test($compare))
        {
            return "Not equal expectation passes " . $dumper->describeDifference($this->_getValue(), $compare);
        }
        else
        {
            return "Not equal expectation fails [" . $dumper->describeValue($this->_getValue()) . "] matches";
        }
    }
}
...
class WithinMarginExpectation extends SimpleExpectation
{
    ...
    function _outsideMessage($compare)
    {
        ...
        {
            return "Outside expectation " . $this->_dumper->describeDifference($compare, $this->_upper);
        }
        else
        {
            return "Outside expectation " . $this->_dumper->describeDifference($compare, $this->_lower);
        }
    }
}
...
class ReferenceExpectation extends SimpleExpectation
{
    ...
    function testMessage($compare)
    {
        ...
        {
            return "Reference expectation fails " . $this->_dumper->describeDifference($this->_value, $compare);
        }
    }
...
class IdenticalExpectation extends EqualExpectation
{
    ...
    function testMessage($compare)
    {
        ...
        {
            return "Identical expectation [" . $dumper->describeValue($this->_getValue()) . "] fails with [" .
...
class PatternExpectation extends SimpleExpectation
{
    ...
    function testMessage($compare)
    {
        ...
        {
            $dumper = &$this->_getDumper();
            return "Pattern [" . $this->_getPattern() . "] not detected in [" . $dumper->describeValue($compare) . "]";
        }
    }
    ...
    function _describePatternMatch($pattern, $subject)
    {
        ...
        $dumper = $this->_getDumper();
        return "Pattern [$pattern] detected at character [$position] in [" . $dumper->describeValue($subject) . "] as [" .
             $matches[0] . "] in region [" . $dumper->clipString($subject, 100, $position) . "]";
    }
}
...
class IsAExpectation extends SimpleExpectation
{
    ...
    function _canonicalType($type)
    {
        ...
        $map = array('bool' => 'boolean', 'float' => 'double', 'real' => 'double', 'int' => 'integer');
        if (isset($map[$type]))
        {
            $type = $map[$type];
        }
    ...
    {
        $dumper = &$this->_getDumper();
        return "Value [" . $dumper->describeValue($compare) . "] should be type [" . $this->_type . "]";
    }
}
...
class NotAExpectation extends IsAExpectation
{
    ...
    {
        $dumper = &$this->_getDumper();
        return "Value [" . $dumper->describeValue($compare) . "] should not be type [" . $this->_getType() . "]";
    }
}
...
class MethodExistsExpectation extends SimpleExpectation
{
    ...
    /**
     * Tests the expectation.
     * True if the method exists in the test object.
     * 
    ...
    function testMessage($compare)
    {
        ...
        $method = $this->_method;
        return "Object [" . $dumper->describeValue($compare) . "] should contain method [$method]";
    }
}
?>