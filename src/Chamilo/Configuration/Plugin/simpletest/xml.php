...
class XmlReporter extends SimpleReporter
{
    ...
    {
        return str_repeat($this->_indent, count($this->getTestList()) + $offset);
    }
...
class SimpleTestXmlParser
{
    ...
    function parse($chunk)
    {
        ...
        {
            trigger_error('XML parse error with ' . xml_error_string(xml_get_error_code($this->_expat)));
            return false;
        }
        return true;
    }
    ...
    {
        return in_array($tag, array('NAME', 'PASS', 'FAIL', 'EXCEPTION', 'SKIP', 'MESSAGE', 'FORMATTED', 'SIGNAL'));
    }
    ...
    function _endElement($expat, $tag)
    {
        ...
        elseif ($tag == 'SIGNAL')
        {
            $this->_listener->paintSignal($this->_attributes['TYPE'], unserialize($this->_content));
        }
        elseif ($tag == 'MESSAGE')
        {
