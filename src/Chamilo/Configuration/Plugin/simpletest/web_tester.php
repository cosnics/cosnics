...
class FieldExpectation extends SimpleExpectation
{
     ...
     * @return s boolean True if matching.
     * @access private
     */
    ...
    function testMessage($compare)
    {
        ...
        {
            return "Field expectation [" . $dumper->describeValue($this->_value) . "] fails with [" .
                 $dumper->describeValue($compare) . "] " . $dumper->describeDifference($this->_value, $compare);
        }
    }
}
...
class HttpHeaderExpectation extends SimpleExpectation
{
    ...
    /**
     * Searches the incoming result.
     * Will extract the matching
     * line as text.
     * 
     * @param mixed $compare Raw header block to search.
    ...
    function testMessage($compare)
    {
        ...
        {
            $message = $this->_expected_header . ($this->_expected_value ? ': ' . $this->_expected_value : '');
        }
...
class TextExpectation extends SimpleExpectation
{
    ...
    function testMessage($compare)
    {
        ...
        {
            $dumper = &$this->_getDumper();
            return "Text [" . $this->_getSubstring() . "] not detected in [" . $dumper->describeValue($compare) . "]";
        }
    }
    ...
    function _describeTextMatch($substring, $subject)
    {
        ...
        $dumper = &$this->_getDumper();
        return "Text [$substring] detected at character [$position] in [" . $dumper->describeValue($subject) .
             "] in region [" . $dumper->clipString($subject, 100, $position) . "]";
    }
}
...
class NoTextExpectation extends TextExpectation
{
    ...
    /**
     * Tests the expectation.
     * False if the substring appears
     * in the text.
     * 
     * @param string $compare Comparison value.
     * @return boolean True if correct.
...
class WebTestCase extends SimpleTestCase
{
    ...
    /**
     * Announces the end of the test.
     * Includes private clean up.
     * 
     ...
     * in the current frameset.
     * Only one such error is
     * reported.
     * 
    ...
    function setMaximumRedirects($max)
    {
        if (! $this->_browser)
        {
            trigger_error('Can only set maximum redirects in a test method, setUp() or tearDown()');
        }
     ...
     * testing from behind a firewall.
     * Set URL
     * to false to disable.
     * 
    ...
    {
        return $this->_failOnError($this->_browser->authenticate($username, $password));
    }
     ...
     * The integer index starts from 1.
     * 
    ...
    /**
     * Clicks a visible text item.
     * Will first try buttons,
    ...
    {
        return $this->_failOnError($this->_browser->clickSubmit($label, $additional));
    }
    ...
    {
        return $this->_failOnError($this->_browser->clickSubmitByName($name, $additional));
    }
    ...
    {
        return $this->_failOnError($this->_browser->clickSubmitById($id, $additional));
    }
    ...
    {
        return $this->_failOnError($this->_browser->clickImage($label, $x, $y, $additional));
    }
    ...
    {
        return $this->_failOnError($this->_browser->clickImageByName($name, $x, $y, $additional));
    }
    ...
    {
        return $this->_failOnError($this->_browser->clickImageById($id, $x, $y, $additional));
    }
     ...
     * Confirms that the form element is currently set
     * to the expected value.
     * A missing form element will always
     * fail. If no value is given then only the existence
     * of the field is checked.
     * 
     * @param string $name Name of field in forms.
    ...
    function _assertFieldValue($identifier, $value, $expected, $message)
    {
        if ($expected === true)
        {
            return $this->assertTrue(isset($value), sprintf($message, "Field [$identifier] should exist"));
        }
        if (! SimpleExpectation :: isExpectation($expected))
        {
            $identifier = str_replace('%', '%%', $identifier);
            $expected = new FieldExpectation($expected, "Field [$identifier] should match with [%s]");
        }
    ...
    function assertResponse($responses, $message = '%s')
    {
        ...
        $message = sprintf($message, "Expecting response in [" . implode(", ", $responses) . "] got [$code]");
    ...
    function assertMime($types, $message = '%s')
    {
        ...
        $message = sprintf($message, "Expecting mime type in [" . implode(", ", $types) . "] got [$type]");
    ...
    function assertAuthentication($authentication = false, $message = '%s')
    {
        if (! $authentication)
        {
            $message = sprintf(
                $message, 
                "Expected any authentication type, got [" . $this->_browser->getAuthentication() . "]");
            return $this->assertTrue($this->_browser->getAuthentication(), $message);
        }
        else
        {
            $message = sprintf(
                $message, 
                "Expected authentication [$authentication] got [" . $this->_browser->getAuthentication() . "]");
            return $this->assertTrue(
    ...
    function assertNoAuthentication($message = '%s')
    {
        $message = sprintf(
            $message, 
            "Expected no authentication type, got [" . $this->_browser->getAuthentication() . "]");
    ...
    function assertRealm($realm, $message = '%s')
    {
        ...
        return $this->assert($realm, $this->_browser->getRealm(), "Expected realm -> $message");
    }
    ...
    {
        return $this->assert(new HttpHeaderExpectation($header, $value), $this->_browser->getHeaders(), $message);
    }
    ...
    {
        return $this->assert(new NoHttpHeaderExpectation($header), $this->_browser->getHeaders(), $message);
    }
    ...
    {
        return $this->assert(new TextExpectation($text), $this->_browser->getContentAsText(), $message);
    }
    ...
    {
        return $this->assert(new NoTextExpectation($text), $this->_browser->getContentAsText(), $message);
    }
    ...
    {
        return $this->assert(new PatternExpectation($pattern), $this->_browser->getContent(), $message);
    }
    ...
    {
        return $this->assert(new NoPatternExpectation($pattern), $this->_browser->getContent(), $message);
    }
    ...
    {
        return $this->assertTrue($this->getCookie($name) === false, sprintf($message, "Not expecting cookie [$name]"));
    }
