 ...
 * got broken in PHP 4.3.0.
 * Adds some browser specific
...
class SimpleUrl
{
    ...
    function _chompLogin(&$url)
    {
        ...
        if (preg_match('/^([^\/]*)@(.*)/', $url, $matches))
        {
            ...
            return array(urldecode($parts[0]), isset($parts[1]) ? urldecode($parts[1]) : false);
        }
        $url = $prefix . $url;
        return array(false, false);
    }
