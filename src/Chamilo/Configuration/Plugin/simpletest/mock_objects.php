...
class ParametersExpectation extends SimpleExpectation
{
    ...
    function testMessage($parameters)
    {
        ...
        {
            return "Expectation of " . count($this->_expected) . " arguments of [" .
                 $this->_renderArguments($this->_expected) . "] is correct";
        }
        else
        {
    ...
    function _describeDifference($expected, $parameters)
    {
        ...
        {
            return "Expected " . count($expected) . " arguments of [" . $this->_renderArguments($expected) . "] but got " .
                 count($parameters) . " arguments of [" . $this->_renderArguments($parameters) . "]";
        }
        $messages = array();
...
class CallCountExpectation extends SimpleExpectation
{
    ...
    {
        return 'Expected call count for [' . $this->_method . '] was [' . $this->_count . '] got [' . $compare . ']';
    }
}
...
class MinimumCallCountExpectation extends SimpleExpectation
{
    ...
    function testMessage($compare)
    {
        return 'Minimum call count for [' . $this->_method . '] was [' . $this->_count . '] got [' . $compare . ']';
    }
}
...
class MaximumCallCountExpectation extends SimpleExpectation
{
    ...
    function testMessage($compare)
    {
        return 'Maximum call count for [' . $this->_method . '] was [' . $this->_count . '] got [' . $compare . ']';
    }
}
...
class SimpleCallSchedule
{
    ...
    function expectArguments($method, $args, $message)
    {
        ...
        $this->_expected_args[strtolower($method)] = new ParametersExpectation($args, $message);
    }
...
class SimpleMock
{
    ...
    function _dieOnNoMethod($method, $task)
    {
        ...
        {
            trigger_error("Cannot $task as no ${method}() in class " . get_class($this), E_USER_ERROR);
        }
    }
     ...
     * expected parameters in that call.
     * All
    ...
    function expectAt($timing, $method, $args, $message = '%s')
    {
        ...
        $method = strtolower($method);
        $message .= Mock :: getExpectationLine();
        $this->_expected_args_at[$timing][$method] = new ParametersExpectation($args, $message);
    }
    ...
    function expectCallCount($method, $count, $message = '%s')
    {
        ...
        $message .= Mock :: getExpectationLine();
        $this->_expected_counts[strtolower($method)] = new CallCountExpectation($method, $count, $message);
    }
    ...
    function expectMaximumCallCount($method, $count, $message = '%s')
    {
        ...
        $message .= Mock :: getExpectationLine();
        $this->_max_counts[strtolower($method)] = new MaximumCallCountExpectation($method, $count, $message);
    }
    ...
    function throwOn($method, $exception = false, $args = false)
    {
        ...
        $this->_actions->register($method, $args, new SimpleThrower($exception ? $exception : new Exception()));
    }
     ...
     * arguments.
     * If there is no matching value found
    ...
    function _checkExpectations($method, $args, $timing)
    {
        ...
        {
            $test->assert($this->_expected_args_at[$timing][$method], $args, "Mock method [$method] at [$timing] -> %s");
        }
        elseif (isset($this->_expected_args[$method]))
        {
            $test->assert($this->_expected_args[$method], $args, "Mock method [$method] -> %s");
        }
    }
}
...
class MockGenerator
{
     ...
     * methods.
     * All methods call the _invoke() handler
    ...
    {
        return "    var \$_mocked_methods = array('" . implode("', '", array_map('strtolower', $methods)) . "');\n";
    }
