...
class SimpleDumper
{
    ...
    function describeDifference($first, $second, $identical = false)
    {
        if ($identical)
        {
            ...
            {
                return "with type mismatch as [" . $this->describeValue($first) . "] does not match [" .
    ...
    {
        return "as [" . $this->describeValue($first) . "] does not match [" . $this->describeValue($second) . "]";
    }
    ...
    function _describeStringDifference($first, $second, $identical)
    {
        ...
        $message .= " with [" . $this->clipString($first, 200, $position) . "] and [" .
    ...
    function _describeIntegerDifference($first, $second, $identical)
    {
        ...
        return "because [" . $this->describeValue($first) . "] differs from [" . $this->describeValue($second) . "] by " .
             abs($first - $second);
    }
    ...
    function _describeArrayDifference($first, $second, $identical)
    {
        ...
        {
            return "as key list [" . implode(", ", array_keys($first)) . "] does not match key list [" .
        ...
        foreach (array_keys($first) as $key)
        {
            ...
            return "with member [$key] " . $this->describeDifference($first[$key], $second[$key], $identical);
        }
        return "";
    }
    ...
    function _describeObjectDifference($first, $second, $identical)
    {
        ...
        return $this->_describeArrayDifference(get_object_vars($first), get_object_vars($second), $identical);
    }
