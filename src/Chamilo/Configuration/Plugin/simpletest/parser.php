 ...
 */
foreach (array('LEXER_ENTER', 'LEXER_MATCHED', 'LEXER_UNMATCHED', 'LEXER_EXIT', 'LEXER_SPECIAL') as $i => $constant)
{
    if (! defined($constant))
    {
        define($constant, $i + 1);
...
class ParallelRegex
{
    ...
    function _getCompoundedRegex()
    {
        if ($this->_regex == null)
        {
            ...
            {
                $this->_patterns[$i] = '(' .
                     str_replace(array('/', '(', ')'), array('\/', '\(', '\)'), $this->_patterns[$i]) . ')';
            }
...
class SimpleLexer
{
     ...
     * parsing mode.
     * The pattern does not change the
     * current mode.
     * 
     * @param string $pattern Perl style regex, but ( and )
     *        lose the usual meaning.
     ...
     * mode.
     * Useful for entering parenthesis, strings,
     * tags, etc.
     * 
     * @param string $pattern Perl style regex, but ( and )
     *        lose the usual meaning.
     ...
     * mode.
     * Empty content will be ignored. The lexer
...
class SimpleHtmlSaxParser
{
    ...
    function acceptAttributeToken($token, $event)
    {
        if ($this->_current_attribute)
        {
            ...
            {
                $this->_attributes[$this->_current_attribute] .= SimpleHtmlSaxParser :: decodeHtml($token);
            }
            if ($event == LEXER_SPECIAL)
            {
                $this->_attributes[$this->_current_attribute] .= preg_replace(
                    '/^=\s*/', 
                    '', 
