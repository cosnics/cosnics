...
class pfcProxyCommand_auth extends pfcProxyCommand
{
    ...
    function run(&$xml_reponse, $p)
    {
        ...
        if (in_array($this->name, $admincmd))
        {
            ...
            {
                $xml_reponse->script(
                    "alert('" . addslashes(_pfc("You are not allowed to run '%s' command", $this->name)) . "');");
                return false;
            }
        }
        ...
        if ($this->name == "join" || $this->name == "join2")
        {
            ...
            if (in_array($nickid, $banlist))
            {
                ...
                $xml_reponse->script(
                    "pfc.handleResponse('" . $this->proxyname . "', 'ban', '" . addslashes($msg) . "');");
                return false;
            }
            ...
            if (count($c->frozen_channels) > 0)
            {
                if (! in_array($channame, $c->frozen_channels))
                {
                    ...
                    $xml_reponse->script(
                        "pfc.handleResponse('" . $this->proxyname . "', 'frozen', '" . addslashes($msg) . "');");
                    return false;
                }
            }
        }
