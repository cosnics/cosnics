...
class pfcProxyCommand_checknickchange extends pfcProxyCommand
{
    ...
    function run($xml_reponse, $p)
    {
        ...
        if ($this->name == 'nick')
        {
            ...
            if ($this->name == 'nick' && $oldnick != '' && $newnick != $oldnick && $c->frozen_nick == true &&
                 $owner != $this->proxyname)
            {
                $msg = _pfc("You are not allowed to change your nickname");
                $xml_reponse->script(
                    "pfc.handleResponse('" . $this->proxyname . "', 'nick', '" . addslashes($msg) . "');");
                return false;
            }
            ...
            {
                $xml_reponse->script(
                    "pfc.handleResponse('" . $this->name . "', 'notchanged', '" . addslashes($newnick) . "');");
                return true;
            }
        ...
        if ($this->name != 'connect' &&         // don't check anything on the connect process or it could block the periodic
                                        // refresh
        $c->frozen_nick == true && $oldnick != $c->nick && $c->nick != '' &&         // don't change the nickname to empty or
                                                                             // the asknick popup will loop indefinatly
        $owner != $this->proxyname)
        {
