...
class pfcCommand
{
    ...
    function forceWhoisReload($nickid)
    {
        ...
        foreach ($otherids as $otherid)
        {
             ...
             * NULL, // recipient NULL, // recipientid ); if (!in_array($cmdtmp, $cmdtoplay)) { $cmdtoplay[] = $cmdtmp;
    ...
    function RunPendingCmdToPlay($nickid, $context, $xml_reponse)
    {
        ...
        foreach ($cmdtoplay_ids as $cid)
        {
            ...
            $cmdp['cmdtoplay'] = true; // used to run some specials actions in the command (ex: if the cmdtoplay is a
                                       // 'leave' command, then show an alert to the kicked or banished user)
            if ($c->debug)
     ...
     * '/^\/([a-z0-9]+)\s*([a-z0-9]+)\s*([a-z0-9]+)\s*(.*)/'; $result = array(); // parse the command name (ex:
     * '/invite') if (preg_match($pattern_command, $cmd_str, $res)) { $cmd = $res[1]; $clientid = $res[2]; $recipientid
     * = $res[3]; $params_str = $res[4]; // don't parse multiple parameters for special commands with only one parameter
     * // this make possible to send double quotes (") in these commands if ($one_parameter || $cmd == 'send' || $cmd ==
     * 'notice' || $cmd == 'me') { $result['cmdstr'] = $cmd_str; $result['cmdname'] = $cmd; $result['params'] =
     * array($clientid, $recipientid, $params_str); return $result; } $params = array($clientid, $recipientid); $sep =
     * preg_match('/[^\\\\]"/',$params_str) ? '"' : ' '; if ($sep == ' ') $params_str = ' ' . $params_str; $offset = 0;
     * while (1) { $i1 = strpos($params_str,$sep,$offset); // capture the parameter value if ($i1 !== FALSE) { // remove
     * multi-separators while (1) { if (strpos($params_str,$sep,$i1+1) - $i1 == 1) $i1++; else break; } // search the
     * parameter terminason $offset = $i1+1; $i2 = strpos($params_str,$sep,$offset); if ($i2 !== FALSE) { $offset = $i2
     * + ($sep == '"' ? 1 : 0); $p = substr($params_str, $i1+1, $i2-$i1-1); if (!preg_match('/^\s*$/',$p)) $params[] =
     * $p; } else break; } else break; } // append the tail if ($offset < strlen($params_str)) $params[] =
     * substr($params_str,$offset); $result['cmdstr'] = $cmd_str; $result['cmdname'] = $cmd; $result['params'] =
