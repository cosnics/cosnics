...
{
    $args = func_get_args();
    $serverid = isset($GLOBALS['serverid']) ? $GLOBALS['serverid'] : 0; // serverid is used to avoid conflicts with
                                                                        // external code using same 'i18n' key
    $args[0] = isset($GLOBALS[$serverid]["i18n"][$args[0]]) && $GLOBALS[$serverid]["i18n"][$args[0]] != "" ? ($GLOBALS["output_encoding"] ==
         "UTF-8" ? $GLOBALS[$serverid]["i18n"][$args[0]] : iconv(
            "UTF-8", 
            $GLOBALS["output_encoding"], 
            $GLOBALS[$serverid]["i18n"][$args[0]])) : "_" . $args[0] . "_";
...
class pfcI18N
{
    ...
    function Init($language, $type = "main")
    {
        ...
        $serverid = isset($GLOBALS['serverid']) ? $GLOBALS['serverid'] : 0; // serverid is used to avoid conflicts with
                                                                            // external code using same 'i18n' key
        $GLOBALS[$serverid]['i18n'] = $GLOBALS['i18n']; // do not pass by reference because $GLOBALS['i18n'] is maybe
                                                        // used by unknown external code
    ...
    {
        return /*<GetAcceptedLanguage>*/array(
            'nl_NL', 
            'ko_KR', 
            'nl_BE', 
            'tr_TR', 
