...
function pfc_make_hyperlink($text)
{
    ...
    $ret = preg_replace(
        "#(^|[\n \]])([\w]+?://[\w\#$%&~/.\-;:=,?@+]*)#ise", 
        "'\\1<a href=\"\\2\"" . $target . ">' . pfc_shorten_url('\\2') . '</a>'", 
        $ret);
    ...
    $ret = preg_replace(
        "#(^|[\n \]])((www|ftp)\.[\w\#$%&~/.\-;:=,?@+]*)#ise", 
        "'\\1<a href=\"http://\\2\"" . $target . ">' . pfc_shorten_url('\\2') . '</a>'", 
        $ret);
    ...
    $ret = preg_replace(
        "#(^|[\n \]])([a-z0-9&\-_.]+?)@([\w\-]+\.([\w\-\.]+\.)*[\w]+)#ie", 
        "'\\1<a href=\"mailto:\\2@\\3\">' . pfc_shorten_url('\\2@\\3') . '</a>'", 
        $ret);
...
{
    $text = preg_replace(
        "#<!-- BBcode auto-mailto start --><a href=\"mailto:(.*?)\".*?>.*?</a><!-- BBCode auto-mailto end -->#i", 
        "\\1", 
        $text);
    $text = preg_replace(
        "#<!-- BBCode auto-link start --><a href=\"(.*?)\".*?>.*?</a><!-- BBCode auto-link end -->#i", 
        "\\1", 
        $text);
...
function pfc_shorten_url($url)
{
    ...
    $short_url = ($len > $shurl_w) ? substr($decodedurl, 0, $shurl_begin_w) . "..." . substr(
        $decodedurl, 
        - $shurl_end_w) : $decodedurl;
