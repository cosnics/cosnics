...
/**
 * this file contains a toolbox with misc.
 * usefull functions
 * 
...
function pfc_GetScriptFilename()
{
    ...
    if (function_exists('debug_backtrace'))
    {
        ...
        {
            $f = $db['file'];
            if (! preg_match('/phpfreechat.class.php/', $f) && ! preg_match('/pfcglobalconfig.class.php/', $f) &&
                 ! preg_match('/pfctools.class.php/', $f) && ! preg_match('/pfcinfo.class.php/', $f))
            {
                $sf = $f;
                break;
            }
        ...
        if (isset($_SERVER['PATH_TRANSLATED']) && file_exists($_SERVER['SCRIPT_FILENAME'])) // check for a cgi
                                                                                            // configurations
        {
            $sf = $_SERVER['PATH_TRANSLATED'];
        }
        else 
            if (isset($_SERVER['SCRIPT_FILENAME']) && file_exists($_SERVER['SCRIPT_FILENAME'])) // for non-cgi
                                                                                                // configurations
            {
 ...
 */
function copy_r($source, $dest, $modedir = 06770, $modefile = 0770)
{
...
function file_get_contents_flock($filename, $incpath = false, $resource_context = null)
{
    ...
    {
        user_error(
            'file_get_contents() failed to open stream: No such file or directory [' . $filename . ']', 
            E_USER_WARNING);
        return false;
    }
...
if (! function_exists('file_put_contents'))
{
    ...
    function file_put_contents($filename, $content, $flags = null, $resource_context = null)
    {
        ...
        {
            user_error(
                'file_put_contents() The 2nd parameter should be either a string or an array [' . $filename . ']', 
                E_USER_WARNING);
            return false;
        }
        ...
        {
            user_error(
                'file_put_contents() failed to open stream: Permission denied [' . $filename . ']', 
                E_USER_WARNING);
            return false;
        }
        ...
        /*
         * $use_lock = ($flags & LOCK_EX) ? true : false ; if ($use_lock === true) { if (!flock($fh, LOCK_EX)) { return
         * false; } }
         */
        ...
        {
            $errormsg = sprintf(
                'file_put_contents() Failed to write %d bytes to %s [' . $filename . ']', 
                $length, 
                $filename);
        ...
        {
            $errormsg = sprintf(
                'file_put_contents() Only %d of %d bytes written, possibly out of free disk space. [' . $filename . ']', 
                $bytes, 
                $length);
            user_error($errormsg, E_USER_WARNING);
            return false;
...
if (! function_exists('html_entity_decode'))
{
    ...
    function html_entity_decode($string, $quote_style = ENT_COMPAT, $charset = null)
    {
        ...
        {
            user_error(
                'html_entity_decode() expects parameter 2 to be long, ' . gettype($quote_style) . ' given', 
                E_USER_WARNING);
            return;
        }
