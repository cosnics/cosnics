...
class pfcContainer extends pfcContainerInterface
{
     ...
     * $this->encode($nick), $nickid); $this->setMeta("nickid-to-channelid", $nickid, $this->encode($chan));
     * $this->setMeta("channelid-to-nickid", $this->encode($chan), $nickid); // update the SERVER channel if ($chan !=
     * 'SERVER') $this->updateNick($nickid); return true; }
     */
    ...
    function removeObsoleteNick($timeout)
    {
        ...
        for ($i = 0; $i < count($ret['timestamp']); $i ++)
        {
            ...
            $nickid = $ret['value'][$i];
            if (time() > ($timestamp + $timeout / 1000) && $nickid) // user will be disconnected after 'timeout'
                                                                    // secondes of inactivity
            {
                // get the current user's channels list
                $channels = array();
    ...
    /**
     * Return a unique id.
     * Each time this function is called, the last id is incremented.
     * used internaly
     * @private
     */
    ...
    function setMeta($group, $subgroup, $leaf, $leafvalue = NULL)
    {
        ...
        if ($this->_usememorycache)
        {
            ...
            if (isset($this->_cache[$group]['value']) && ! in_array($subgroup, $this->_cache[$group]['value']))
            {
    ...
    function getMeta($group, $subgroup = null, $leaf = null, $withleafvalue = false)
    {
        ...
        if ($this->_usememorycache)
        {
            ...
            $incache = false;
            if ($subgroup == null && isset($this->_cache[$group]['value']))
            {
                $incache = true;
