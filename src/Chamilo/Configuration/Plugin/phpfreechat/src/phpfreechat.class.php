...
class phpFreeChat
{
    ...
    function &handleRequest($request)
    {
        ...
        if (isset($u->privmsg[$recipientid]))
        {
            ...
            if ($cmdname != "update" && $cmdname != "leave" &&             // do not open the pv tab when other user close the tab
            $cmdname != "quit" && $cmdname != "privmsg2")
            {
    ...
    function &loadStyles($theme = 'default', &$xml_reponse)
    {
        ...
        require_once dirname(__FILE__) . '/../lib/ctype/ctype.php'; // to keep compatibility for php without ctype
                                                                    // support
    ...
    function &loadScripts($theme, &$xml_reponse)
    {
        ...
        $labels_to_load = array("Do you really want to leave this room ?",         // _pfc
