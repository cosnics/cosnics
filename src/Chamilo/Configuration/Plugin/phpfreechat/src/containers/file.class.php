...
class pfcContainer_File extends pfcContainerInterface
{
    ...
    var $_users = array("nickid" => array(), "timestamp" => array());
    ...
    function init(&$c)
    {
        ...
        $errors = array_merge(
            $errors, 
            @test_writable_dir($c->container_cfg_server_dir, "container_cfg_chat_dir/serverid"));
        ...
        if (is_writable(dirname($filename)))
        {
            ...
            // $errors[] = "LOCK_EX feature is not usable on your filesystem. Please do not use the 'file' container
        // (try the 'mysql' container) or swith to another filesystem type.";
        }
    ...
    function rmMeta($group, $subgroup = null, $leaf = null)
    {
        ...
        $dh = opendir($dir);
        readdir($dh);
        readdir($dh); // skip . and .. directories
