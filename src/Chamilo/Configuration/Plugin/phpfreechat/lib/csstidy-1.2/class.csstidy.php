...
class csstidy
{
    ...
    function _unicode(&$string, &$i)
    {
        ...
        if (hexdec($add) > 47 && hexdec($add) < 58 || hexdec($add) > 64 && hexdec($add) < 91 ||
     ...
     * 
     * @param string $content either filename (if $from_file == true), content of a template file, "high_compression",
     *            "highest_compression", "low_compression", or "default"
    ...
    function parse($string)
    {
                    ...
                    if (csstidy :: is_token($string, $i))
                    {
                        if (($string{$i} == ':' || $string{$i} == '=') && $this->property != '')
                        {
                            ...
                            if (csstidy :: property_is_valid($this->property) ||
                                 ! $this->get_cfg('discard_invalid_properties'))
                            {
                ...
                case 'iv' :
                    ...
                    if (csstidy :: is_token($string, $i) || $pn)
                    {
                        ...
                        if (($string{$i} == '}' || $string{$i} == ';' || $pn) && ! empty($this->selector))
                        {
                            ...
                            if ((! $this->invalid_at || $this->get_cfg('preserve_css')) &&
                                 (! $this->get_cfg('discard_invalid_properties') || $valid))
                            {
                            ...
                            if (! $valid)
                            {
                                ...
                                {
                                    $this->log(
                                        'Invalid property in ' . strtoupper($this->get_cfg('css_level')) . ': ' .
                                             $this->property, 
                                            'Warning');
                                }
                            }
        ...
        return ! (empty($this->css) && empty($this->import) && empty($this->charset) && empty($this->tokens) &&
    ...
    function css_add_property($media, $selector, $property, $new_val)
    {
        ...
        {
            if ((csstidy :: is_important($this->css[$media][$selector][$property]) && csstidy :: is_important($new_val)) ||
                 ! csstidy :: is_important($this->css[$media][$selector][$property]))
            {
