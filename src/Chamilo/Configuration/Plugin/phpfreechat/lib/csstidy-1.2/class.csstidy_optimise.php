...
class csstidy_optimise
{
    ...
    function postparse()
    {
        ...
        if ($this->parser->get_cfg('optimise_shorthands') > 0)
        {
            foreach ($this->css as $medium => $value)
            {
                ...
                {
                    $this->css[$medium][$selector] = csstidy_optimise :: merge_4value_shorthands(
                        $this->css[$medium][$selector]);
    ...
    function value()
    {
        ...
        if (isset($shorthands[$this->property]))
        {
            ...
            {
                $this->parser->log(
                    'Optimised shorthand notation (' . $this->property . '): Changed "' . $this->value . '" to "' . $temp .
                         '"', 
                        'Information');
            }
            $this->value = $temp;
    ...
    function shorthands()
    {
        ...
        if ($this->property == 'background' && $this->parser->get_cfg('optimise_shorthands') > 1)
        {
            ...
            $this->parser->merge_css_blocks(
                $this->at, 
                $this->selector, 
                csstidy_optimise :: dissolve_short_bg($this->value));
        }
        if (isset($shorthands[$this->property]))
        {
            $this->parser->merge_css_blocks(
                $this->at, 
                $this->selector, 
                csstidy_optimise :: dissolve_4value_shorthands($this->property, $this->value));
    ...
    function subvalue()
    {
        ...
        if ($temp != $this->sub_value)
        {
            ...
            {
                $this->parser->log(
                    'Fixed invalid number: Changed "' . $this->sub_value . '" to "' . $temp . '"', 
                    'Warning');
            }
            else
            {
                $this->parser->log(
                    'Optimised number: Changed "' . $this->sub_value . '" to "' . $temp . '"', 
                    'Information');
            }
            $this->sub_value = $temp;
        }
        if ($this->parser->get_cfg('compress_colors'))
        {
            ...
            if ($temp !== $this->sub_value)
            {
                ...
                {
                    $this->parser->log(
                        'Fixed invalid color name: Changed "' . $this->sub_value . '" to "' . $temp . '"', 
                        'Warning');
                }
                else
                {
                    $this->parser->log(
                        'Optimised color: Changed "' . $this->sub_value . '" to "' . $temp . '"', 
                        'Information');
                }
                $this->sub_value = $temp;
            }
     ...
     * Converts all rgb() values to #-values and uses the short-form if possible. Also replaces 4 color names by
     * #-values.
     * 
     * @param string $color
     * @return string
    ...
    function merge_4value_shorthands($array)
    {
        ...
        {
            if (isset($array[$value[0]]) && isset($array[$value[1]]) && isset($array[$value[2]]) &&
                 isset($array[$value[3]]) && $value !== 0)
            {
                $return[$key] = '';
    ...
    function dissolve_short_bg($str_value)
    {
        ...
        for ($i = 0; $i < count($str_value); $i ++)
        {
            ...
            {
                if ($have['bg'] === FALSE &&
                     (substr($str_value[$i][$j], 0, 4) == 'url(' || $str_value[$i][$j] === 'none'))
                {
    ...
    function merge_bg($input_css)
    {
        ...
        $number_of_values = @max(
            count(csstidy_optimise :: explode_ws(',', $input_css['background-image'])), 
            count(csstidy_optimise :: explode_ws(',', $input_css['background-color'])), 
            1);
