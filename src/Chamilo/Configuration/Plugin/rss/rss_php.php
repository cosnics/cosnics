<?php
/*
 * RSS_PHP - the PHP DOM based RSS Parser Author: <rssphp.net> Published: 200801 :: blacknet :: via rssphp.net RSS_PHP
 * is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY. Usage: See the documentation at
 * http://rssphp.net/documentation Examples: Can be found online at http://rssphp.net/examples
 */
class rss_php
{
     ...
     * public load methods
     * 
     * @param $includeAttributes BOOLEAN return array;
     *        *
     */
        ...
        foreach ($nodeList as $values)
        {
            if (substr($values->nodeName, 0, 1) != '#')
            {
                ...
                if ($values->attributes)
                {
                    ...
                    {
                        $tempNode[$nodeName]['properties'][$values->attributes->item($i)->nodeName] = $values->attributes->item(
                            $i)->nodeValue;
                    }
                }
    ...
    protected function randomContext()
    {
        ...
        $contextOptions = array(
            'http' => array('method' => "GET", 'header' => $setHeaders));
