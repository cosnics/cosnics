<?php
/*
 * pChart - a PHP class to build charts! Copyright (C) 2008 Jean-Damien POGOLOTTI Version 1.27d last updated on 09/30/08
 * http://pchart.sourceforge.net This program is free software: you can redistribute it and/or modify it under the terms
 * of the GNU General Public License as published by the Free Software Foundation, either version 1,2,3 of the License,
 * or (at your option) any later version. This program is distributed in the hope that it will be useful, but WITHOUT
 * ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU
 * General Public License for more details. You should have received a copy of the GNU General Public License along with
 * this program. If not, see <http://www.gnu.org/licenses/>. Class initialisation : pChart($XSize,$YSize) Draw methods :
 * drawBackground($R,$G,$B) drawRectangle($X1,$Y1,$X2,$Y2,$R,$G,$B)
 * drawFilledRectangle($X1,$Y1,$X2,$Y2,$R,$G,$B,$DrawBorder=TRUE,$Alpha=100)
 * drawRoundedRectangle($X1,$Y1,$X2,$Y2,$Radius,$R,$G,$B) drawFilledRoundedRectangle($X1,$Y1,$X2,$Y2,$Radius,$R,$G,$B)
 * drawCircle($Xc,$Yc,$Height,$R,$G,$B,$Width=0) drawFilledCircle($Xc,$Yc,$Height,$R,$G,$B,$Width=0)
 * drawEllipse($Xc,$Yc,$Height,$Width,$R,$G,$B) drawFilledEllipse($Xc,$Yc,$Height,$Width,$R,$G,$B)
 * drawLine($X1,$Y1,$X2,$Y2,$R,$G,$B,$GraphFunction=FALSE) drawDottedLine($X1,$Y1,$X2,$Y2,$DotSize,$R,$G,$B)
 * drawAlphaPixel($X,$Y,$Alpha,$R,$G,$B) drawFromPNG($FileName,$X,$Y,$Alpha=100) drawFromGIF($FileName,$X,$Y,$Alpha=100)
 * drawFromJPG($FileName,$X,$Y,$Alpha=100) Graph setup methods : addBorder($Width=3,$R=0,$G=0,$B=0) clearScale()
 * clearShadow() createColorGradientPalette($R1,$G1,$B1,$R2,$G2,$B2,$Shades) drawGraphArea($R,$G,$B,$Stripe=FALSE)
 ...
 * getLegendBoxSize($DataDescription) loadColorPalette($FileName,$Delimiter=",") reportWarnings($Interface="CLI")
 * setGraphArea($X1,$Y1,$X2,$Y2) setLabel($Data,$DataDescription,$SerieName,$ValueName,$Caption,$R=210,$G=210,$B=210)
 * setColorPalette($ID,$R,$G,$B) setCurrency($Currency) setDateFormat($Format) setFontProperties($FontName,$FontSize)
 * setLineStyle($Width=1,$DotSize=0) setFixedScale($VMin,$VMax,$Divisions=5,$VXMin=0,$VXMin=0,$XDivisions=5)
 * setShadowProperties($XDistance=1,$YDistance=1,$R=60,$G=60,$B=60,$Alpha) writeValues($Data,$DataDescription,$Series)
 * Graphs methods : drawPlotGraph($Data,$DataDescription,$BigRadius=5,$SmallRadius=2,$R2=-1,$G2=-1,$B2=-1,$Shadow=FALSE)
 ...
 * drawOverlayBarGraph($Data,$DataDescription,$Alpha=50) drawBarGraph($Data,$DataDescription,$Shadow=FALSE)
 ...
 * Other methods : setImageMap($Mode=TRUE,$GraphID="MyGraph") getImageMap($MapName,$Flush=TRUE) Render($FileName)
 * Stroke()
 */
...
class pChart
{
    ...
    function drawRightScale($Data, $DataDescription, $ScaleMode, $R, $G, $B, $DrawTicks = TRUE, $Angle = 0, $Decimals = 1, 
        $WithMargin = FALSE, $SkipLabels = 1)
    {
        $this->drawScale(
            $Data, 
            $DataDescription, 
    ...
    function drawScale($Data, $DataDescription, $ScaleMode, $R, $G, $B, $DrawTicks = TRUE, $Angle = 0, $Decimals = 1, 
        $WithMargin = FALSE, $SkipLabels = 1, $RightScale = FALSE)
    {
        ...
        foreach ($Data as $Key => $Values)
        {
            if ($ID % $SkipLabels == 0)
            {
                ...
                {
                    /**
                     * This fixes a bug where the text wasnt shown correctly at the bottom of the graph
                     * changed from:
    ...
    function drawXYScale($Data, $DataDescription, $YSerieName, $XSerieName, $R, $G, $B, $WithMargin = 0, $Angle = 0, 
        $Decimals = 1)
    {
        /* Validate the Data and DataDescription array */
        $this->validateData("drawScale", $Data);
    ...
    function drawLegend($XPos, $YPos, $DataDescription, $R, $G, $B, $Rs = -1, $Gs = -1, $Bs = -1, $Rt = 0, $Gt = 0, $Bt = 0, 
        $Border = TRUE)
    {
        /* Validate the Data and DataDescription array */
    ...
    function drawTextBox($X1, $Y1, $X2, $Y2, $Text, $Angle = 0, $R = 255, $G = 255, $B = 255, $Align = ALIGN_LEFT, $Shadow = TRUE, $BgR = -1, 
        $BgG = -1, $BgB = -1, $Alpha = 100)
    {
        ...
        $Shadow = FALSE)
    {
        ...
        foreach ($DataDescription["Values"] as $Key2 => $ColName)
        {
            ...
            {
                $Is_Alpha = ((ord(file_get_contents($DataDescription["Symbol"][$ColName], false, null, 25, 1)) & 6) & 4) ==
                     4;
    ...
    function drawXYPlotGraph($Data, $DataDescription, $YSerieName, $XSerieName, $PaletteID = 0, $BigRadius = 5, 
        $SmallRadius = 2, $R2 = -1, $G2 = -1, $B2 = -1, $Shadow = TRUE)
    {
            ...
            foreach ($Data as $Key => $Values)
            {
                if (isset($Data[$Key][$ColName]))
                {
                    if (is_numeric($Data[$Key][$ColName]))
                    {
                        ...
                        {
                            $YPos = $this->GArea_Y2 -
                                 ((($Value + $LastValue[$Key]) - $this->VMin) * $this->DivisionRatio);
    ...
    function drawRadarAxis($Data, $DataDescription, $Mosaic = TRUE, $BorderOffset = 10, $A_R = 60, $A_G = 60, $A_B = 60, $S_R = 200, 
        $S_G = 200, $S_B = 200, $MaxValue = -1)
    {
        /* Validate the Data and DataDescription array */
    ...
    function drawBasicPieGraph($Data, $DataDescription, $XPos, $YPos, $Radius = 100, $DrawLabels = PIE_NOLABEL, $R = 255, $G = 255, $B = 255, 
        $Decimals = 0)
    {
        /* Validate the Data and DataDescription array */
    ...
    function drawFlatPieGraphWithShadow($Data, $DataDescription, $XPos, $YPos, $Radius = 100, $DrawLabels = PIE_NOLABEL, 
        $SpliceDistance = 0, $Decimals = 0)
    {
    ...
    /* This function draw a flat pie chart */
    function drawFlatPieGraph($Data, $DataDescription, $XPos, $YPos, $Radius = 100, $DrawLabels = PIE_NOLABEL, $SpliceDistance = 0, 
        $Decimals = 0, $AllBlack = FALSE)
    {
        /* Validate the Data and DataDescription array */
    ...
    function drawPieGraph($Data, $DataDescription, $XPos, $YPos, $Radius = 100, $DrawLabels = PIE_NOLABEL, $EnhanceColors = TRUE, 
        $Skew = 60, $SpliceHeight = 20, $SpliceDistance = 0, $Decimals = 0)
    {
        ...
        foreach ($iValues as $Key => $Value)
        {
            $XCenterPos = cos(($Angle - $CDev + ($Value * $SpliceRatio + $SpliceDistanceRatio) / 2) * 3.1418 / 180) *
                 $SpliceDistance + $XPos;
            $YCenterPos = sin(($Angle - $CDev + ($Value * $SpliceRatio + $SpliceDistanceRatio) / 2) * 3.1418 / 180) *
                 $SpliceDistance + $YPos;
            $XCenterPos2 = cos(($Angle + $CDev + ($Value * $SpliceRatio + $SpliceDistanceRatio) / 2) * 3.1418 / 180) *
                 $SpliceDistance + $XPos;
            $YCenterPos2 = sin(($Angle + $CDev + ($Value * $SpliceRatio + $SpliceDistanceRatio) / 2) * 3.1418 / 180) *
                 $SpliceDistance + $YPos;
    ...
    function addToImageMap($X1, $Y1, $X2, $Y2, $SerieName, $Value, $CallerFunction)
    {
        ...
        {
            $this->ImageMap[] = round($X1) . "," . round($Y1) . "," . round($X2) . "," . round($Y2) . "," . $SerieName .
                 "," . $Value;
