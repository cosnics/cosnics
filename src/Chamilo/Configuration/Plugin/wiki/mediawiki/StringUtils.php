...
class MediawikiStringUtils
{
    ...
    static function delimiterReplaceCallback($startDelim, $endDelim, $callback, $subject, $flags = '')
    {
        ...
        while ($inputPos < strlen($subject) &&
            ...
            if ($m[1][0] != '')
            {
                if ($foundStart && $strcmp($endDelim, substr($subject, $tokenOffset, $endLength)) == 0)
                {
    ...
    /*
     * Perform an operation equivalent to preg_replace( "!$startDelim(.*)$endDelim!$flags", $replace, $subject ) @param
     * string $startDelim Start delimiter regular expression @param string $endDelim End delimiter regular expression
     * @param string $replace Replacement string. May contain $1, which will be replaced by the text between the
     * delimiters @param string $subject String to search @return string The string with the matches replaced
     */
