...
class MediaWikiTitle
{
    ...
    /**
     * MediawikiTitle::newFromText maintains a cache to avoid expensive re-normalization of
    ...
    public static function newFromText($text, $defaultNamespace = NS_MAIN)
    {
        if (is_object($text))
        {
            throw new MWException('MediawikiTitle::newFromText given an object');
        }
    ...
    {
        $fname = 'MediawikiTitle::newFromID';
    ...
    public static function newFromRedirectArray($text)
    {
        ...
        while (-- $recurse > 0)
        {
            ...
            {
                // the new title passes the checks, so make that our current title so that further recursion can be
                // checked
                $title = $newtitle;
                $titles[] = $newtitle;
            }
    ...
    /*
     * Make a prefixed DB key from a DB key and a namespace index @param $ns \type{\int} numerical representation of the
     * namespace @param $title \type{\string} the DB key form the title @param $fragment \type{\string} The link
     * fragment (after the "#") @return \type{\string} the prefixed form of the title
     */
     ...
     * It includes the fragment, but does not include
     ...
     * @param $doExpensiveQueries \type{\bool} Set this to false to avoid doing unnecessary queries.
     * @param $ignoreErrors \type{\arrayof{\string}} Set this to a list of message keys whose corresponding errors may
     *            be ignored.
     ...
     * which checks ONLY that previously checked by userCan (i.e.
     * it leaves out
    ...
    private function getUserPermissionsErrorsInternal($action, $user, $doExpensiveQueries = true, $short = false)
    {
        ...
        if ($doExpensiveQueries && ! wfRunHooks(
        ...
        if ($this->isNamespaceProtected())
        {
            ...
            $errors[] = NS_MEDIAWIKI == $this->mNamespace ? array('protectedinterface') : array(
                'namespaceprotected', 
                $ns);
        }
    ...
    public function updateTitleProtection($create_perm, $reason, $expiry)
    {
        ...
        {
            $dbw->delete('protected_titles', array('pt_namespace' => $namespace, 'pt_title' => $title), __METHOD__);
        }
    ...
    {
        return $this->mNamespace == NS_MEDIAWIKI && preg_match('!\.(?:css|js)$!u', $this->mTextform) > 0;
    }

    /**
     * Is this a .
     * css or .js subpage of a user page?
     * 
     * @return \type{\bool} TRUE or FALSE
     */
    ...
    /**
     * Trim down a .
     * css or .js subpage title to get the corresponding skin name
     */
    ...
    {
        global $wgUser;
        return ($wgUser->isAllowed('editusercssjs') ||
     ...
     *         which cascading restrictions have come, false for none, or true if such restrictions exist, but
     *         $get_pages was not set.
    ...
    public function getCascadeProtectionSources($get_pages = true)
    {
        ...
        if (isset($this->mCascadeSources) && $get_pages)
        {
            ...
            if (isset($this->mHasCascadingRestrictions) && ! $get_pages)
            {
    ...
    private function loadRestrictionsFromRow($res, $oldFashionedRestrictions = NULL)
    {
        ...
        {
            $oldFashionedRestrictions = $dbr->selectField(
                'page', 
                'page_restrictions', 
    ...
    {
        $dbw = wfGetDB(DB_MASTER);
        $dbw->delete('page_restrictions', array('pr_expiry < ' . $dbw->addQuotes($dbw->timestamp())), __METHOD__);
        
        $dbw->delete('protected_titles', array('pt_expiry < ' . $dbw->addQuotes($dbw->timestamp())), __METHOD__);
    }
    ...
    public function isDeleted()
    {
        ...
        else
        {
            ...
            if ($this->getNamespace() == NS_FILE)
            {
                $n += $dbr->selectField('filearchive', 'COUNT(*)', array('fa_name' => $this->getDBkey()), __METHOD__);
            }
        }
        return (int) $n;
    }
    ...
    private function secureAndSplit()
    {
        ...
        if (strpos($dbkey, '.') !== false &&
             ($dbkey === '.' || $dbkey === '..' || strpos($dbkey, './') === 0 || strpos($dbkey, '../') === 0 ||
             strpos($dbkey, '/./') !== false || strpos($dbkey, '/../') !== false || substr($dbkey, - 2) == '/.' ||
             substr($dbkey, - 3) == '/..'))
        {
            return false;
        }
     ...
     * Removes the first character from the
     * specified fragment before setting, so it assumes you're passing it with
     * an initial "#".
     * Deprecated for public use, use MediawikiTitle::makeTitle() with fragment parameter.
    ...
    public function getSquidURLs()
    {
        ...
        $urls = array($this->getInternalURL(), $this->getInternalURL('action=history'));
    ...
    public function moveTo(&$nt, $auth = true, $reason = '', $createRedirect = true)
    {
        ...
        $dbw->update(
            'categorylinks', 
            array('cl_sortkey' => $nt->getPrefixedText(), 'cl_timestamp=cl_timestamp'), 
            array('cl_from' => $pageid, 'cl_sortkey' => $this->getPrefixedText()), 
            __METHOD__);
    ...
    private function moveOverExistingRedirect(&$nt, $reason = '', $createRedirect = true)
    {
        ...
        $fname = 'MediawikiTitle::moveOverExistingRedirect';
        ...
        if ($createRedirect || ! $wgUser->isAllowed('suppressredirect'))
        {
            ...
            $dbw->insert(
                'pagelinks', 
                array('pl_from' => $newid, 'pl_namespace' => $nt->getNamespace(), 'pl_title' => $nt->getDBkey()), 
                $fname);
    ...
    public function getParentCategories()
    {
        ...
        // NEW SQL
        $sql = "SELECT * FROM $categorylinks" . " WHERE cl_from='$titlekey'" . " AND cl_from <> '0'" .
             " ORDER BY cl_sortkey";
        ...
        {
            foreach ($res as $row)
                // $data[] = MediawikiTitle::newFromText($wgContLang->getNSText ( NS_CATEGORY ).':'.$row->cl_to);
    ...
    public function getPreviousRevisionID($revId, $flags = 0)
    {
        ...
        return $db->selectField(
            'revision', 
            'rev_id', 
            array('rev_page' => $this->getArticleId($flags), 'rev_id < ' . intval($revId)), 
            __METHOD__, 
    ...
    public function countRevisionsBetween($old, $new)
    {
        ...
        return $dbr->selectField(
            'revision', 
            'count(*)', 
            'rev_page = ' . intval($this->getArticleId()) . ' AND rev_id > ' . intval($old) . ' AND rev_id < ' .
                 intval($new), 
                __METHOD__);
    }
    ...
    public function equals(Title $title)
    {
        ...
        return $this->getInterwiki() === $title->getInterwiki() && $this->getNamespace() == $title->getNamespace() &&
    ...
    public function getNotificationTimestamp($user = NULL)
    {
        ...
        $this->mNotificationTimestamp[$uid] = $dbr->selectField(
            ...
            array('wl_namespace' => $this->getNamespace(), 'wl_title' => $this->getDBkey(), 'wl_user' => $user->getId()), 
            __METHOD__);
        return $this->mNotificationTimestamp[$uid];
    }
...
-->";
    ...
    public function getRedirectsHere($ns = null)
    {
        ...
        $res = $dbr->select(array('redirect', 'page'), array('page_namespace', 'page_title'), $where, __METHOD__);
