                ...
                if ($remaining = $tailBytes[$c = $str[++ $i]])
                {
                    ...
                    do
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                $replace[] = array(UTF8_REPLACEMENT, $base + $i - strlen($sequence), strlen($sequence));
                    ...
                    if (isset($checkit[$head]))
                    {
                        ...
                        else
                        {
                            ...
                            ($n < 0xc2 && $sequence <= UTF8_OVERLONG_A) || ($n == 0xe0 && $sequence <= UTF8_OVERLONG_B) ||
        ...
        for ($i = 0; $i < $len; $i ++)
        {
            ...
            if ($lastClass == 0)
            {
                ...
                if ($n >= $x1 && $n <= $x2)
                {
                    ...
                    //
                    if ($c >= UTF8_HANGUL_VBASE && $c <= UTF8_HANGUL_VEND && $startChar >= UTF8_HANGUL_LBASE &&
                         $startChar <= UTF8_HANGUL_LEND)
                    {
                        ...
                        $startChar = chr($hangulPoint >> 12 & 0x0f | 0xe0) . chr($hangulPoint >> 6 & 0x3f | 0x80) .
                             chr($hangulPoint & 0x3f | 0x80);
                        $lastHangul = 0;
                        continue;
                    }
                    elseif ($c >= UTF8_HANGUL_TBASE && $c <= UTF8_HANGUL_TEND && $startChar >= UTF8_HANGUL_FIRST &&
