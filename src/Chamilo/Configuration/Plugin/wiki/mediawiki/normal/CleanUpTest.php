...
class CleanUpTest extends PHPUnit_Framework_TestCase
{
    ...
    function XtestAllChars()
    {
        ...
        for ($i = 0x0; $i < UNICODE_MAX; $i ++)
        {
            ...
            if ($i % 0x1000 == 0)
                echo "U+$x\n";
            if ($i == 0x0009 || $i == 0x000a || $i == 0x000d || ($i > 0x001f && $i < UNICODE_SURROGATE_FIRST) ||
                 ($i > UNICODE_SURROGATE_LAST && $i < 0xfffe) || ($i > 0xffff && $i <= UNICODE_MAX))
            {
    ...
    function doTestBytes($head, $tail)
    {
        for ($i = 0x0; $i < 256; $i ++)
        {
            ...
            {
                $this->assertEquals(bin2hex($char), bin2hex($clean), "ASCII byte $x should be intact");
                if ($char != $clean)
                    return;
            }
            else
            {
                ...
                $this->assertEquals(bin2hex($norm), bin2hex($clean), "Forbidden byte $x should be rejected");
                if ($norm != $clean)
                    return;
            }
        }
        ...
        for ($first = 0xc0; $first < 0x100; $first ++)
        {
            for ($second = 0x80; $second < 0x100; $second ++)
            {
                ...
                elseif ($first > 0xfd || $second > 0xbf)
                {
                    ...
                    $this->assertEquals(bin2hex($norm), bin2hex($clean), "Forbidden pair $x should be rejected");
                    if ($norm != $clean)
                        return;
                }
                else
            ...
            for ($second = 0x80; $second < 0x100; $second ++)
            {
                ...
                for ($third = 0x80; $third < 0x81; $third ++)
                {
                    ...
                    elseif (($first > 0xfd || $second > 0xfd) &&
                         (($second > 0xbf && $third > 0xbf) || ($second < 0xc0 && $third < 0xc0) || ($second > 0xfd) ||
                         ($third > 0xfd)))
                    {
    ...
    function testChunkRegression()
    {
        ...
        $text = "\x46\x55\xb8" . "\xdc\x96" . "\xee" . "\xe7" . "\x44" . "\xaa" . "\x2f\x25";
    ...
    {
        $text = "\x4e\x30" . "\xb1" .         // bad tail
        "\x3a" . "\x92" .         // bad tail
        "\x62\x3a" . "\x84" .         // bad tail
        "\x43" . "\xc6" .         // bad head
        "\x3f" . "\x92" .         // bad tail
        "\xad" .         // bad tail
        "\x7d" . "\xd9\x95";
