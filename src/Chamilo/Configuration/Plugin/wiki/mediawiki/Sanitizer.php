...
$wgHtmlEntityAliases = array('רלמ' => 'rlm', 'رلم' => 'rlm');
...
class MediawikiSanitizer
{
                            ...
                            if (in_array($t, $tagstack) && ! isset($htmlnest[$t]))
                            {
                                ...
                                elseif (isset($htmlsingleonly[$t]))
                                {
                                    ...
                                    if (isset($htmlsingle[$t]))
                                    {
                                        ...
                                        if (isset($tabletags[$t]) && in_array($t, $tagstack))
                                        {
     ...
     * Conflicting items in the second set
    ...
    {
        $out = array_merge($a, $b);
        if (isset($a['class']) && isset($b['class']) && is_string($a['class']) && is_string($b['class']) &&
             $a['class'] !== $b['class'])
        {
    ...
    static function safeEncodeAttribute($text)
    {
        ...
        $encValue = preg_replace_callback(
            '/(' . wfUrlProtocols() . ')/', 
            array('MediawikiSanitizer', 'armorLinksCallback'), 
            $encValue);
        return $encValue;
    }
    ...
    static function escapeId($id, $options = array())
    {
        ...
        if (! preg_match("/^[$nameStartChar]/u", $id) && ! in_array('noninitial', $options))
        {
            $id = "_$id";
        }
     ...
     * a partial tag string.
     * Attribute names are forces to lowercase,
     ...
     * for XML and XHTML specifically.
     * Any stray bits will be
    ...
    {
        return preg_replace_callback(MW_CHAR_REFS_REGEX, array('Sanitizer', 'normalizeCharReferencesCallback'), $text);
    }
     ...
     * return the named entity reference as is.
     * If the entity is a
    ...
    public static function decodeCharReferences($text)
    {
        return preg_replace_callback(
            MW_CHAR_REFS_REGEX, 
            array('MediawikiSanitizer', 'decodeCharReferencesCallback'), 
            $text);
    }
    ...
    /**
     * If the named entity is defined in the HTML 4.0/XHTML 1.0 DTD,
     * return the UTF-8 encoding of that character.
     * Otherwise, returns
    ...
    static function setupAttributeWhitelist()
    {
        ...
        $tablecell = array('abbr', 'axis', 'headers', 'scope', 'rowspan', 'colspan', 'nowrap',         // deprecated
        'width',         // deprecated
        'height',         // deprecated
        'bgcolor'); // deprecated
        ...
        $whitelist = array( // 7.5.4
            ...
            // 9.2.2
            'blockquote' => array_merge($common, array('cite')),  // q
            ...
            'table' => array_merge(
                $common, 
                array('summary', 'width', 'border', 'frame', 'rules', 'cellspacing', 'cellpadding', 'align', 'bgcolor')), 
            ...
            // 15.2.2
            'font' => array_merge($common, array('size', 'color', 'face')),  // basefont
