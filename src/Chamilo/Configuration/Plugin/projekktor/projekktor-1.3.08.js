...
jQuery(function($) {
	playerModel.prototype = {
		...
		getPoster : function() {
			var type = 'poster', result = this.pp.getConfig(type), qual = 'default', quals = [];
...
jQuery(function($) {
	$p
			.newModel({
				...
				applySrc : function() {
					var ref = this, wasAwakening = ref.getState('AWAKENING');
	...
	$p.newModel({
		...
		applyMedia : function(destContainer) {
			...
			var mediaContainer = $('#' + this.pp.getMediaId()
					+ '_audio_container');

			if (mediaContainer.length === 0) {
				mediaContainer = $('<div/>').css({
					width : '1px',
					height : '1px',
					marginBottom : '-1px'
...
jQuery(function($) {
	$p
			.newModel({
				...
				// should require an additional config option to activate in
				// order consider strobe.swf variants
				// {ext:'m3u8', type:'application/mpegURL', platform:'flash',
				// fixed: true, streamType: ['*']},
				// {ext:'m3u8', type:'application/x-mpegURL', platform:'flash',
				// fixed: true, streamType: ['*']},
				// {ext:'m3u8', type:'application/vnd.apple.mpegurl',
				// platform:'flash', fixed: true, streamType: ['*']},
				// {ext:'manifest', type:'application/vnd.ms-ss',
				// platform:'flash', fixed: true, streamType: ['*']}
				],
...
jQuery(function($) {
	projekktorControlbar.prototype = {
		...
		startHandler : function() {
			this.setVolumeClass(this._getVolume());
			this.pp.setVolume(this._getVolume());
		...
		vsliderClk : function(evt) {
			...
			this.setVolumeClass(result);

			this.pp.setVolume(result);
		},

		setVolumeClass : function(volume) {
			$(this.controlElements['mute']).removeClass(
					'volumeMute volumeLow volumeNormal volumeFull');

			if (volume == 0) {
				$(this.controlElements['mute']).addClass('volumeMute');
			} else if (volume < 0.25) {
				$(this.controlElements['mute']).addClass('volumeLow');
			} else if (volume < 0.6) {
				$(this.controlElements['mute']).addClass('volumeNormal');
			} else {
				$(this.controlElements['mute']).addClass('volumeFull');
			}
		...
		vknobStartDragListener : function(event, domObj) {
			...
			var ref = this, sliderIdx = (this.pp.getInFullscreen() === true && this.controlElements['vslider'].length > 1) ? 1
					: 0, knob = $(domObj[sliderIdx]), slider = $(this.controlElements['vslider'][sliderIdx]), slider = $(this.controlElements['vslider'][sliderIdx]), dx = Math
					.abs(parseInt(knob.position().left) - event.clientX), volume = 0, mouseUp = function(
					mouseupevt) {
			...
			mouseMove = function(dragevent) {
				...
				newXPos = (newXPos > slider.width() - knob.width() / 2) ? slider
						.width()
						- (knob.width() / 2)
						: newXPos;
