...
use Chamilo\Configuration\Package\Properties\Dependencies\Dependency\RegistrationDependency;
...
class Sequencer
{
    ...
    {
        $this->process_dependency($this->get_package($package_context)->get_pre_depends());
    }
    ...
    public function process_dependency($dependency)
    {
        if ($dependency instanceof Dependencies)
        {
            ...
            {
                $this->process_dependency($sub_dependency);
            }
        }
