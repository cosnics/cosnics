(function($) {
	function form_submitted(evt, ui) {
		...
		if (selectedClass == 'confirm') {
			return confirm(getTranslation(selectedValue + 'Confirm', null, $(
					'input[name="' + table_name + '_namespace"]').attr('value')));
		}
	}
	...
	function checkboxClicked(e) {
		...
		if (e.shiftKey && lastSelectedCheckbox) {
			...
			var lastSelectedCheckboxIndex = allCheckboxes
					.index(lastSelectedCheckbox);

			// Determine the lowest and the highest selected index
			var start = Math.min(currentCheckboxIndex,
					lastSelectedCheckboxIndex);
	...
	function selectAll(e, ui) {
		e.preventDefault();
		$(':checkbox', $(this).parentsUntil('form').parent()).attr('checked',
				true);
	}
