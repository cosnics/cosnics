...
class XmlGroupFeedComponent extends Manager
{
    ...
    public function dump_groups_tree($groups)
    {
        ...
        foreach ($groups as $group)
        {
            ...
            {
                echo '<node id="group_' . $group['group']->get_id() . '" classes="' . $glyph->render() . '" title="' .
                    htmlspecialchars($group['group']->get_name()) . '" description="' .
                    htmlspecialchars($group['group']->get_name()) . '">', PHP_EOL;
                $this->dump_groups_tree($group['children']);
                echo '</node>', PHP_EOL;
            }
            else
            {
                echo '<leaf id="group_' . $group['group']->get_id() . '" classes="' . $glyph->render() . '" title="' .
                    htmlspecialchars($group['group']->get_name()) . '" description="' .
                    htmlspecialchars($group['group']->get_name()) . '"/>' . PHP_EOL;
            }
        }
