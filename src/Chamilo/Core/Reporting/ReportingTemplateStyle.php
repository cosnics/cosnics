 ...
 *
 * Configuration (PDF) Reports: Administrators can configure PDF reports centrally via Administration -> Settings -> Reporting. @see
 * src/Chamilo/Core/Reporting/Resources/Settings/settings.xml. Individual reporing templates and blocks can override the central settings
 ...
 * 
 * - PDF report orientation: By overriding the function getStyle() in concrete reporing template classes, templates can return an
 *   ReportingTemplateStyle object with custom propery values. Among others the paper orientation property can be set.
 *
 * - Report column styles: Concrete reporting templates and blocks can overrule the configured style. In reporting blocks ReportingDataStyle
 * ojects can be assigned to data rows. The following example shows how to define the heading colors and define the column widths:
 *
