<?php
/*
 * pBarcode128 - class to create barcodes (128B) Version : 2.1.3 Made by : Jean-Damien POGOLOTTI Last Update : 09/09/11
 ...
 class pBarcode128
{
    ...
    public function draw($Object, $Value, $X, $Y, $Format = "")
    {
        ...
        if ($DrawArea)
        {
            ...
            {
                $X3 = $X2 +
                     cos(($Angle + 90) * PI / 180) * ($Height + $LegendOffset + $this->pChartObject->FontSize + 10);
                $Y3 = $Y2 +
                     sin(($Angle + 90) * PI / 180) * ($Height + $LegendOffset + $this->pChartObject->FontSize + 10);
            }
            else
            {
