<?php
/*
 * pStock - class to draw stock charts Version : 2.1.3 Made by : Jean-Damien POGOLOTTI Last Update : 09/09/11 This file
 * can be distributed under the license you can find at : http://www.pchart.net/license You can find the whole class
...
class pStock
{
    ...
    public function drawStockChart($Format = "")
    {
        ...
        if (! isset($Data["Series"][$SerieOpen]) || ! isset($Data["Series"][$SerieClose]) ||
             ! isset($Data["Series"][$SerieMin]) || ! isset($Data["Series"][$SerieMax]))
        ...
        {
            $Point = "";
            if (isset($Data["Series"][$SerieClose]["Data"][$Key]) || isset($Data["Series"][$SerieMin]["Data"][$Key]) ||
        ...
        foreach ($Plots as $Key => $Points)
        {
            ...
            $Values = "Open :" . $Data["Series"][$SerieOpen]["Data"][$Key] . "<BR>Close : " .
                 $Data["Series"][$SerieClose]["Data"][$Key] . "<BR>Min : " . $Data["Series"][$SerieMin]["Data"][$Key] .
                ...
                if ($ExtremityWidth == 1)
                {
                    ...
                    if ($RecordImageMap)
                    {
                        $this->pChartObject->addToImageMap(
                            "RECT", 
                            floor($X - $ExtremityLength) . "," . floor($PosArray[2]) . "," . floor(
                                $X + $ExtremityLength) . "," . floor($PosArray[3]), 
                            $ImageMapColor, 
                            $ImageMapTitle, 
                            $Values);
                    }
                    ...
                    if ($RecordImageMap)
                    {
                        $this->pChartObject->addToImageMap(
                            ...
                            floor($X - $ExtremityLength) . "," . floor($PosArray[2] - $ExtremityWidth) . "," .
                                 floor($X + $ExtremityLength) . "," . floor($PosArray[3] + $ExtremityWidth), 
                                $ImageMapColor, 
                                $ImageMapTitle, 
                                $Values);
                    }
            ...
            elseif ($Data["Orientation"] == SCALE_POS_TOPBOTTOM)
            {
                ...
                if ($ExtremityWidth == 1)
                {
                    ...
                    if ($RecordImageMap)
                    {
                        $this->pChartObject->addToImageMap(
                            "RECT", 
                            floor($PosArray[2]) . "," . floor($Y - $ExtremityLength) . "," . floor($PosArray[3]) . "," .
                ...
                else
                {
                    ...
                    if ($RecordImageMap)
                    {
                        $this->pChartObject->addToImageMap(
                            "RECT", 
                            floor($PosArray[2] - $ExtremityWidth) . "," . floor($Y - $ExtremityLength) . "," .
                                 floor($PosArray[3] + $ExtremityWidth) . "," . floor($Y + $ExtremityLength), 
                                $ImageMapColor, 
                                $ImageMapTitle, 
                                $Values);
                    }
