<?php
/*
 * pPie - class to draw pie charts Version : 2.1.3 Made by : Jean-Damien POGOLOTTI Last Update : 09/09/11 This file can
...
class pPie
{
    ...
    public function draw3DPie($X, $Y, $Format = "")
    {
        ...
        foreach ($Slices as $SliceID => $Plots)
        {
                ...
                if (isset($SliceAngle[$SliceID][1]) ) /* Empty error handling */
          {
                ...
                if (isset($SliceAngle[$SliceID][1]) && $SliceAngle[$SliceID][1] > 270 &&
                ...
                if (isset($SliceAngle[$SliceID][1]) && $SliceAngle[$SliceID][1] > 90 &&
    ...
    public function writePieLabel($X, $Y, $Label, $Angle, $Settings, $Stacked, $Xc = 0, $Yc = 0, $Radius = 0, 
        $Reversed = FALSE)
    {
        $LabelOffset = 30;
        ...
        else
        {
            ...
            if ($this->LabelPos != "")
            {
                ...
                foreach ($this->LabelPos as $Key => $Settings)
                {
                    if (! $Done)
                    {
                        if ($Angle <= 90 && (($YTop >= $Settings["YTop"] && $YTop <= $Settings["YBottom"]) ||
                             ($YBottom >= $Settings["YTop"] && $YBottom <= $Settings["YBottom"])))
                        {
                            $this->shift(0, 180, - ($Height + 2), $Reversed);
                            $Done = TRUE;
                        }
                        if ($Angle > 90 && $Angle <= 180 && (($YTop >= $Settings["YTop"] && $YTop <= $Settings["YBottom"]) ||
                             ($YBottom >= $Settings["YTop"] && $YBottom <= $Settings["YBottom"])))
                        {
                            $this->shift(0, 180, - ($Height + 2), $Reversed);
                            $Done = TRUE;
                        }
                        if ($Angle > 180 && $Angle <= 270 && (($YTop >= $Settings["YTop"] &&
                             $YTop <= $Settings["YBottom"]) ||
                             ($YBottom >= $Settings["YTop"] && $YBottom <= $Settings["YBottom"])))
                        {
                            $this->shift(180, 360, ($Height + 2), $Reversed);
                            $Done = TRUE;
                        }
                        if ($Angle > 270 && $Angle <= 360 && (($YTop >= $Settings["YTop"] &&
                             $YTop <= $Settings["YBottom"]) ||
                             ($YBottom >= $Settings["YTop"] && $YBottom <= $Settings["YBottom"])))
                        {
                            $this->shift(180, 360, ($Height + 2), $Reversed);
