<?php
/*
 * pSurface - class to draw surface charts Version : 2.1.3 Made by : Jean-Damien POGOLOTTI Last Update : 09/09/11 This
        ...
        for ($X = 0; $X <= $this->GridSizeX; $X ++)
        {
            for ($Y = 0; $Y <= $this->GridSizeY; $Y ++)
            {
                ...
                if ($Value != UNKNOWN && $Value != IGNORED && $Value >= $Threshold)
                {
                    ...
                    if ($X > 0 && $this->Points[$X - 1][$Y] != UNKNOWN && $this->Points[$X - 1][$Y] != IGNORED &&
                         $this->Points[$X - 1][$Y] < $Threshold)
                        $this->pChartObject->drawLine($X1, $Y1, $X1, $Y2, $Color);
                    if ($Y > 0 && $this->Points[$X][$Y - 1] != UNKNOWN && $this->Points[$X][$Y - 1] != IGNORED &&
                         $this->Points[$X][$Y - 1] < $Threshold)
                        $this->pChartObject->drawLine($X1, $Y1, $X2, $Y1, $Color);
                    if ($X < $this->GridSizeX && $this->Points[$X + 1][$Y] != UNKNOWN &&
                         $this->Points[$X + 1][$Y] != IGNORED && $this->Points[$X + 1][$Y] < $Threshold)
                        $this->pChartObject->drawLine($X2, $Y1, $X2, $Y2, $Color);
                    if ($Y < $this->GridSizeY && $this->Points[$X][$Y + 1] != UNKNOWN &&
                         $this->Points[$X][$Y + 1] != IGNORED && $this->Points[$X][$Y + 1] < $Threshold)
        ...
        foreach ($Missing as $Key => $Pos)
        {
            ...
            if ($this->Points[$X][$Y] == UNKNOWN)
            {
                ...
                for ($Xi = $X - $NearestNeighbor; $Xi <= $X + $NearestNeighbor; $Xi ++)
                {
                    ...
                    {
                        if ($Xi >= 0 && $Yi >= 0 && $Xi <= $this->GridSizeX && $Yi <= $this->GridSizeY &&
                             $this->Points[$Xi][$Yi] != UNKNOWN && $this->Points[$Xi][$Yi] != IGNORED)
                        {
