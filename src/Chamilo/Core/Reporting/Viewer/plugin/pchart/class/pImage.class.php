<?php
/*
 * pDraw - pChart core class Version : 2.1.3 Made by : Jean-Damien POGOLOTTI Last Update : 09/09/11 This file can be
 * distributed under the license you can find at : http://www.pchart.net/license You can find the whole class
...
class pImage extends pDraw
{
    ...
    public function addToImageMap($Type, $Plots, $Color = NULL, $Title = NULL, $Message = NULL, $HTMLEncode = FALSE)
    {
        ...
        elseif ($this->ImageMapStorageMode == IMAGE_MAP_STORAGE_FILE)
        {
            ...
            fwrite(
                $Handle, 
                $Type . IMAGE_MAP_DELIMITER . $Plots . IMAGE_MAP_DELIMITER . $Color . IMAGE_MAP_DELIMITER . $Title .
                     IMAGE_MAP_DELIMITER . $Message . "\r\n");
            fclose($Handle);
        }
    }
        ...
        elseif ($this->ImageMapStorageMode == IMAGE_MAP_STORAGE_FILE)
        {
            ...
            if ($Handle)
            {
                ...
                foreach ($TempArray as $Key => $Settings)
                {
                    fwrite(
                        $Handle, 
                        $Settings[0] . IMAGE_MAP_DELIMITER . $Settings[1] . IMAGE_MAP_DELIMITER . $Settings[2] .
                             IMAGE_MAP_DELIMITER . $Settings[3] . IMAGE_MAP_DELIMITER . $Settings[4] . "\r\n");
                }
                fclose($Handle);
            }
        }
    ...
    public function dumpImageMap($Name = "pChart", $StorageMode = IMAGE_MAP_STORAGE_SESSION, $UniqueID = "imageMap", $StorageFolder = "tmp")
    {
        ...
        if ($this->ImageMapStorageMode == IMAGE_MAP_STORAGE_SESSION)
        {
            ...
            if ($_SESSION[$Name] != NULL)
            {
                ...
                {
                    echo $Params[0] . IMAGE_MAP_DELIMITER . $Params[1] . IMAGE_MAP_DELIMITER . $Params[2] .
                         IMAGE_MAP_DELIMITER . $Params[3] . IMAGE_MAP_DELIMITER . $Params[4] . "\r\n";
                }
            }
        }
        elseif ($this->ImageMapStorageMode == IMAGE_MAP_STORAGE_FILE)
