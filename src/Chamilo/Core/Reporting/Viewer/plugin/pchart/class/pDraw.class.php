<?php
/*
 * pDraw - class extension with drawing methods Version : 2.1.3 Made by : Jean-Damien POGOLOTTI Last Update : 09/09/11
 * This file can be distributed under the license you can find at : http://www.pchart.net/license You can find the whole
 * class documentation on the pChart web site.
 */
...
class pDraw
{
    ...
    public function drawPolygon($Points, $Format = "")
    {
        ...
        if (! $NoBorder)
        {
            ...
            for ($i = 0; $i <= count($Points) - 1; $i = $i + 2)
            {
                if (isset($Points[$i + 2]))
                {
                    if (! ($Points[$i] == $Points[$i + 2] && $Points[$i] == $SkipX) &&
    ...
    public function drawFilledCircle($X, $Y, $Radius, $Format = "")
    {
        ...
        if ($BorderR != - 1)
            $this->drawCircle(
                ...
                $Radius, 
                $Radius, 
                array("R" => $BorderR, "G" => $BorderG, "B" => $BorderB, "Alpha" => $BorderAlpha, "Ticks" => $Ticks));
    ...
    public function drawScale($Format = "")
    {
        ...
        foreach ($Data["Axis"] as $AxisID => $AxisParameter)
        {
            ...
            elseif ($Mode == SCALE_MODE_ADDALL || $Mode == SCALE_MODE_ADDALL_START0)
            {
                ...
                foreach ($Data["Series"] as $SerieID => $SerieParameter)
                {
                    if ($SerieParameter["Axis"] == $AxisID && $SerieParameter["isDrawable"] &&
                         $Data["Abscissa"] != $SerieID)
                    {
            ...
            while (! $Found)
            {
                foreach ($Factors as $Key => $Factor)
                {
                    if (! $Found)
                    {
                        ...
                        {
                            $XMaxRescaled = floor($XMax / ($Factor * $Scaled10Factor)) * $Factor * $Scaled10Factor +
        ...
        if ($Data["Orientation"] == SCALE_POS_LEFTRIGHT)
        {
            ...
            for ($i = 0; $i <= count($BoundsA) - 4; $i = $i + 2)
            {
                $this->drawLine(
                    ...
                    $BoundsA[$i + 2], 
                    $BoundsA[$i + 3], 
                    array("R" => $LineR, "G" => $LineG, "B" => $LineB, "Alpha" => $LineAlpha, "Ticks" => $LineTicks));
                $this->drawLine(
                    $BoundsB[$i], 
                    $BoundsB[$i + 1], 
                    $BoundsB[$i + 2], 
                        ...
                        if ($Y != VOID && $LastX != NULL && $LastY != NULL)
                        {
                            ...
                            if ($ReCenter && $X + $XStep < $this->GraphAreaX2 - $XMargin)
                            {
                                ...
                                if ($RecordImageMap)
                                {
                                    $this->addToImageMap(
                                        "RECT", 
                                        floor($X - $ImageMapPlotSize) . "," . floor($Y - $ImageMapPlotSize) . "," .
                                             floor($X + $XStep + $ImageMapPlotSize) . "," . floor(
                                                $Y + $ImageMapPlotSize), 
                                            $this->toHTMLColor($R, $G, $B), 
                                            $SerieDescription, 
                                            $this->scaleFormat($Serie["Data"][$Key], $Mode, $Format, $Unit));
                            ...
                            else
                            {
                                if ($RecordImageMap)
                                {
                                    $this->addToImageMap(
                                        "RECT", 
                                        floor($LastX - $ImageMapPlotSize) . "," . floor($LastY - $ImageMapPlotSize) . "," .
                                             floor($X + $ImageMapPlotSize) . "," . floor($LastY + $ImageMapPlotSize), 
                                            $this->toHTMLColor($R, $G, $B), 
                                            $SerieDescription, 
                                            $this->scaleFormat($Serie["Data"][$Key], $Mode, $Format, $Unit));
                                }
                                ...
                                if ($RecordImageMap)
                                {
                                    $this->addToImageMap(
                                        ...
                                        floor($LastGoodX + $XStep - $ImageMapPlotSize) . "," .
                                             floor($LastGoodY - $ImageMapPlotSize) . "," . floor($X + $ImageMapPlotSize) .
                            ...
                            if ($RecordImageMap)
                            {
                                $this->addToImageMap(
                                    ...
                                    floor($this->GraphAreaX1 + $XMargin - $ImageMapPlotSize) . "," .
                                         floor($Y - $ImageMapPlotSize) . "," . floor($X + $ImageMapPlotSize) . "," .
                    ...
                    foreach ($PosArray as $Key => $X)
                    {
                        ...
                        if ($X != VOID && $LastX != NULL && $LastY != NULL)
                        {
                            ...
                            if ($RecordImageMap)
                            {
                                $this->addToImageMap(
                                    "RECT", 
                                    floor($LastX - $ImageMapPlotSize) . "," . floor($LastY - $ImageMapPlotSize) . "," .
                                         floor($LastX + $XStep + $ImageMapPlotSize) . "," . floor(
                                            $Y + $ImageMapPlotSize), 
                                        $this->toHTMLColor($R, $G, $B), 
                                        $SerieDescription, 
                                        $this->scaleFormat($Serie["Data"][$Key], $Mode, $Format, $Unit));
                        ...
                        if ($X != VOID && $LastX == NULL && $LastGoodY != NULL && ! $BreakVoid)
                        {
                            ...
                            if ($RecordImageMap)
                            {
                                $this->addToImageMap(
                                    "RECT", 
                                    floor($LastGoodX - $ImageMapPlotSize) . "," . floor($LastGoodY - $ImageMapPlotSize) .
                                         "," . floor($LastGoodX + $ImageMapPlotSize) . "," .
                            ...
                            if ($RecordImageMap)
                            {
                                $this->addToImageMap(
                                    ...
                                    floor($LastGoodX - $ImageMapPlotSize) . "," .
                                         floor($LastGoodY + $YStep - $ImageMapPlotSize) . "," .
                                         floor($LastGoodX + $ImageMapPlotSize) . "," . floor($YStep + $ImageMapPlotSize), 
                                        $this->toHTMLColor($R, $G, $B), 
                                        $SerieDescription, 
                                        $this->scaleFormat($Serie["Data"][$Key], $Mode, $Format, $Unit));
                            }
                    ...
                    foreach ($PosArray as $Key => $Y2)
                    {
                        ...
                        if ($Y2 != VOID)
                        {
                                ...
                                if ($RecordImageMap)
                                {
                                    $this->addToImageMap(
                                        ...
                                        floor($X + $XOffset + $XSpace) . "," . floor($Y1 - 1) . "," .
                                             floor($X + $XOffset + $XSize - $XSpace) . "," . floor($Y1 + 1), 
                                            $this->toHTMLColor($R, $G, $B), 
                                            $SerieDescription, 
                                            $this->scaleFormat($Serie["Data"][$Key], $Mode, $Format, $Unit));
                                }
                                ...
                                if ($RecordImageMap)
                                {
                                    $this->addToImageMap(
                                        ...
                                        floor($X + $XOffset + $XSpace) . "," . floor($Y1) . "," .
                                             floor($X + $XOffset + $XSize - $XSpace) . "," . floor($Y2), 
                                            $this->toHTMLColor($R, $G, $B), 
                                            $SerieDescription, 
                                            $this->scaleFormat($Serie["Data"][$Key], $Mode, $Format, $Unit));
                                }
                            ...
                            if ($DisplayValues && $Serie["Data"][$Key] != VOID)
                            {
                                ...
                                {
                                    $CenterX = (($X + $XOffset + $XSize - $XSpace) - ($X + $XOffset + $XSpace)) / 2 + $X +
                                         $XOffset + $XSpace;
                                    $CenterY = ($Y2 - $Y1) / 2 + $Y1;
                        ...
                        if ($X2 != VOID)
                        {
                            ...
                            if ($Serie["Data"][$Key] == 0)
                            {
                                ...
                                if ($RecordImageMap)
                                {
                                    $this->addToImageMap(
                                        "RECT", 
                                        floor($X1 - 1) . "," . floor($Y + $YOffset + $YSpace) . "," . floor($X1 + 1) .
                            ...
                            else
                            {
                                if ($RecordImageMap)
                                {
                                    $this->addToImageMap(
                                        "RECT", 
                                        floor($X1) . "," . floor($Y + $YOffset + $YSpace) . "," . floor($X2) . "," .
                                             floor($Y + $YOffset + $YSize - $YSpace), 
                                            $this->toHTMLColor($R, $G, $B), 
                                            $SerieDescription, 
                                            $this->scaleFormat($Serie["Data"][$Key], $Mode, $Format, $Unit));
                            ...
                            if ($DisplayValues && $Serie["Data"][$Key] != VOID)
                            {
                                ...
                                {
                                    $CenterX = ($X2 - $X1) / 2 + $X1;
                                    $CenterY = (($Y + $YOffset + $YSize - $YSpace) - ($Y + $YOffset + $YSpace)) / 2 +
                                         ($Y + $YOffset + $YSpace);
                    ...
                    foreach ($PosArray as $Key => $Height)
                    {
                        if ($Height != VOID && $Serie["Data"][$Key] != 0)
                        {
                            ...
                            if ($RecordImageMap)
                            {
                                $this->addToImageMap(
                                    "RECT", 
                                    floor($X + $XOffset) . "," . floor($Y1 - $YSpaceUp + $YSpaceDown) . "," .
                    ...
                    foreach ($PosArray as $Key => $Width)
                    {
                        if ($Width != VOID && $Serie["Data"][$Key] != 0)
                        {
                            ...
                            if ($RecordImageMap)
                            {
                                $this->addToImageMap(
                                    "RECT", 
                                    floor($X1 + $XSpaceLeft) . "," . floor($Y + $YOffset) . "," . floor(
                                        $X2 - $XSpaceRight) . "," . floor($Y + $YOffset + $YSize), 
                                    $this->toHTMLColor($R, $G, $B), 
                                    $SerieDescription, 
                                    $this->scaleFormat($Serie["Data"][$Key], $Mode, $Format, $Unit));
                            }
                    ...
                    foreach ($PosArray as $Key => $Y)
                    {
                        if ($Y != VOID && $LastY != NULL)
                        {
                            ...
                            if ($Slope >= 0)
                            {
                                ...
                                $R = (($PositiveSlopeEndR - $PositiveSlopeStartR) / 100) * $SlopeIndex +
                                     $PositiveSlopeStartR;
                                $G = (($PositiveSlopeEndG - $PositiveSlopeStartG) / 100) * $SlopeIndex +
                                     $PositiveSlopeStartG;
                                $B = (($PositiveSlopeEndB - $PositiveSlopeStartB) / 100) * $SlopeIndex +
                                     $PositiveSlopeStartB;
                            }
                            elseif ($Slope < 0)
                            {
                                $SlopeIndex = (100 / abs($MinSlope)) * abs($Slope);
                                $R = (($NegativeSlopeEndR - $NegativeSlopeStartR) / 100) * $SlopeIndex +
                                     $NegativeSlopeStartR;
                                $G = (($NegativeSlopeEndG - $NegativeSlopeStartG) / 100) * $SlopeIndex +
                                     $NegativeSlopeStartG;
    ...
    public function drawLabelBox($X, $Y, $Title, $Captions, $Format = "")
    {
        ...
        foreach ($Captions as $Key => $Caption)
        {
            ...
            $CaptionHeight = $CaptionHeight + max(($TxtPos[0]["Y"] - $TxtPos[2]["Y"]), ($SerieBoxSize + 2)) +
                 $HorizontalMargin;
        }
        ...
        $this->drawPolygon(
            $Poly, 
            array("R" => $GradientEndR, "G" => $GradientEndG, "B" => $GradientEndB, "NoBorder" => TRUE));
        ...
        if ($TitleMode == LABEL_TITLE_NOBACKGROUND && ! $NoTitle)
        {
            ...
            $this->drawLine(
                ...
                $XMax - $XMargin, 
                $YPos + 1, 
                array("R" => $GradientEndR, "G" => $GradientEndG, "B" => $GradientEndB));
            $this->drawLine(
                $XMin + $XMargin, 
                $YPos, 
                $XMax - $XMargin, 
                $YPos, 
                array("R" => $GradientStartR, "G" => $GradientStartG, "B" => $GradientStartB));
        }
        elseif ($TitleMode == LABEL_TITLE_BACKGROUND)
        {
            $this->drawFilledRectangle(
                ...
                array("R" => $TitleBackgroundR, "G" => $TitleBackgroundG, "B" => $TitleBackgroundB));
            imageline(
                $this->Picture, 
                $XMin + 1, 
        ...
        if (! $NoTitle)
            $this->drawText(
                ...
                $Title, 
                array("Align" => TEXT_ALIGN_BOTTOMLEFT, "R" => $TitleR, "G" => $TitleG, "B" => $TitleB));
    ...
    /* Draw a basic shape */
    public function drawShape($X, $Y, $Shape, $PlotSize, $PlotBorder, $BorderSize, $R, $G, $B, $Alpha, $BorderR, 
        $BorderG, $BorderB, $BorderAlpha)
    {
            ...
            for ($Y = $MinY; $Y <= $MaxY; $Y = $Y + $YStep)
            {
                ...
                foreach ($Segments as $Key => $Coords)
                {
                    ...
                    if (min($Y1, $Y2) <= $Y && max($Y1, $Y2) >= $Y)
                    {
                        ...
                        if (is_array($Intersections) && in_array($X, $Intersections) && $LastSlope == "=" &&
                             ($Slope == "-"))
                        {
                            $Intersections[] = $X;
                        }
