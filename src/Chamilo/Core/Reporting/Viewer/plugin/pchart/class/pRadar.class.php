<?php
/*
 * pRadar - class to draw radar charts Version : 2.1.3 Made by : Jean-Damien POGOLOTTI Last Update : 09/09/11 This file
...
class pRadar
{
    ...
    public function drawRadar($Object, $Values, $Format = "")
    {
        ...
        if ($Layout == RADAR_LAYOUT_STAR)
        {
            for ($j = 1; $j <= $Segments; $j ++)
            {
                for ($i = 0; $i < 360; $i = $i + (360 / $Points))
                {
                    ...
                    $EdgeX2 = cos(deg2rad($i + $AxisRotation + (360 / $Points))) * ($EdgeHeight / $Segments) * $j +
                         $CenterX;
                    $EdgeY2 = sin(deg2rad($i + $AxisRotation + (360 / $Points))) * ($EdgeHeight / $Segments) * $j +
                         $CenterY;
        ...
        for ($i = 0; $i < 360; $i = $i + (360 / $Points))
        {
            ...
            if ($WriteLabels)
            {
                $LabelX = cos(deg2rad($i + $AxisRotation + $Axisoffset)) * ($EdgeHeight + $LabelPadding + $TicksLength) +
                     $CenterX;
                $LabelY = sin(deg2rad($i + $AxisRotation + $Axisoffset)) * ($EdgeHeight + $LabelPadding + $TicksLength) +
                     $CenterY;
