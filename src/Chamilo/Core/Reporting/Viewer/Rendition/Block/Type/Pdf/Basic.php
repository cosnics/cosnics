...
class Basic extends Pdf
{
        ...
        foreach ($reportingData->get_categories() as $category_index => $category_name)
        {
            ...
            foreach ($reportingData->get_rows() as $row_index => $row_name)
            {
                ...
                $rowData[] = $this->transcode_string(
                    $reportingDataData[$category_index][$row_index], false, 'UTF-8', 'CP1252'
                );
            }
    ...
    private function renderBlockTitle($pdfMcTable)
    {
        ...
        $pdfMcTable->MultiCell(
            0, $pdfMcTable->FontSize * 1.8,
            $this->transcode_string($this->get_block()->get_title(), false, 'UTF-8', 'CP1252'), 0
        );
    }
    ...
    {
        preg_match_all(
            "/<li(\s+class=\".*\")?(\s+style=\".*\")?\s*>(.*)<\/li>/isU", $unordered_list_match[3], $list_matches
        );
        $result = [];
    ...
    )
    {
        ...
        // Handle ordered list
        $text = preg_replace_callback('/<ol>(.*)<\/ol>/isU', ['self', 'replace_ordered_list'], $text);

        // Handle unordered list
        $text = preg_replace_callback(
            "/<ul(\s+class=\".*\")?(\s+style=\".*\")?\s*>(.*)<\/ul>/isU", ['self', 'replace_unordered_list'], $text
        );
        ...
        return preg_replace_callback(
            '/<img\s+src="(.+)".*\/>/isU', function ($matches) {
            return null;
        }, $text
        );
    }
