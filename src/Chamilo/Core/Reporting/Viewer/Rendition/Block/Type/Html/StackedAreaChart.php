...
class StackedAreaChart extends Chart
{
    ...
    public function get_path()
    {
        ...
        $md5 = md5(serialize(['stacked_area_chart', $reporting_data]));
        ...
        if (!file_exists($path))
        {
            ...
            if ($number_of_rows > 1)
            {
                ...
                {
                    $width = 282 + (36 * $number_of_categories) + 20;
                    $graph_area_left = 160;
                }
            }
            else
            {
                if ($number_of_categories <= 14)
                {
                    $width = 600;
                    $graph_area_left = 70;
                }
                ...
                {
                    $width = 122 + (36 * $number_of_categories) + 20;
                    $graph_area_left = 70;
                }
            }
            ...
            $chart_canvas->setFontProperties(
                [
                    'FontName' => $this->getVendorPath() . 'szymach/c-pchart/src/Resources/fonts/verdana.ttf',
                    'FontSize' => 8,
                    'R' => 0,
                    'G' => 0,
                    'B' => 0
            ...
            $chart_canvas->drawFilledRectangle(
                $graph_area_left, 20, $width - 21, $graph_area_bottom - 1,
                ['R' => 0, 'G' => 0, 'B' => 0, 'Surrounding' => - 200, 'Alpha' => 3]
            );
            ...
            $chart_canvas->setShadow(true, ['X' => 1, 'Y' => 1, 'R' => 0, 'G' => 0, 'B' => 0, 'Alpha' => 10]);
            $chart_canvas->drawStackedAreaChart(
                [
                    'DisplayValues' => true,
