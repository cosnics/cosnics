...
class ReportingData
{
     ...
     *  @param $style RepostingDataStyle Null or style of given row.
     *
     *  @note $style is cloned before storing it, so that any change to $style will not influence the stored values.
     */
     ...
     *
     *  Relative width of style objects: maximum length of (stripped) content of each each row is collected. Row Width are proportional to
     *  the content length.
     */
    public function setStyleAutomatically()
    {
        ...
        $relative_widths = array_map(function($item) use ($sum_of_relative_widths) { return $item / $sum_of_relative_widths; }, $relative_widths);
     ...
     *
     *  @return array: keys: row names values: maximum data lengths. If category is visible the key "#CATEGORY#" contains the maximum
     *  category length.
     */
