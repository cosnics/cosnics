...
class AdvancedRightsEditorComponent extends RightsEditorComponent implements TableSupport
{
    ...
    public function display_entity_table($selected_entity)
    {
        ...
        if ($selected_entity instanceof NestedRightsEntity)
        {
            ...
            $this->table_conditions[self::TAB_ENTITY] = $this->get_condition(
                $selected_entity, $search_query, $selected_id
            );
            ...
            $this->table_conditions[self::TAB_SUB_ENTITIES] = $this->get_condition(
                $selected_entity, $search_query, null, $selected_id
            );
    ...
    public function getButtonToolbarRenderer()
    {
        if (!isset($this->buttonToolbarRenderer))
        {
            ...
            $commonActions->addButton(
                new Button(
                    Translation::get('SimpleRightsEditor'), new FontAwesomeGlyph('cog'),
            ...
            if (count($locations) == 1)
            {
                ...
                if ($location != null && $location->get_parent())
                {
                    ...
                    if ($location->inherits())
                    {
                        ...
                        $commonActions->addButton(
                            new Button(
                                Translation::get('NoInherit'), new FontAwesomeGlyph('level-up', array('text-danger')),
                                $url, ToolbarItem::DISPLAY_ICON_AND_LABEL
                            )
                        );
                    }
                    else
                    {
                        ...
                        $commonActions->addButton(
                            new Button(
                                Translation::get('Inherit'), new FontAwesomeGlyph('level-up', array('text-success')),
                                $url, ToolbarItem::DISPLAY_ICON_AND_LABEL
                            )
                        );
                    }
    ...
    public function get_condition($selected_entity, $search_query, $entity_id = null, $parent_id = null)
    {
        ...
        if (isset($entity_id))
        {
            $conditions[] = new EqualityCondition(
                new PropertyConditionVariable(
                    $selected_entity->data_class_class_name(), $selected_entity->get_id_property()
        ...
        if (isset($parent_id))
        {
            ...
            $conditions[] = new EqualityCondition(
                new PropertyConditionVariable(
                    $selected_entity->data_class_class_name(), $selected_entity->get_parent_property()
