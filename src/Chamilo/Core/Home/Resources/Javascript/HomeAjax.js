...
$(function() {
	...
	function showTab(e, ui) {
		...
		$("div.portal_tab:not(#portal_tab_" + tab[2] + ")").css('display',
				'none');
	...
	function deleteItem(e) {
		...
		confirmation = confirm(getTranslation('Confirm', null,
				translationContext));
		if (confirmation) {
	...
	function getEmptyBlocksContainer() {
		...
		packageSearch.val(getTranslation('SearchForWidgets', null,
				translationContext));
		...
		componentTitle.html(getTranslation('BrowseBlocks', null,
				translationContext));
	...
	function clearWidgetSearch(e, ui) {
		...
		if (search.val() == '') {
			search.val(getTranslation('SearchForWidgets', null,
					translationContext));
			search.removeClass('query');

			$('#component-title').html(
					getTranslation('BrowseBlocks', null, translationContext))
		}
	}
	...
	function addComponents() {
		...
		for ( var i in availableBlocks) {
			for ( var j in availableBlocks[i].components) {
				...
				availableComponent.attr('id', i + '-'
						+ availableBlocks[i].components[j].id);
				availableComponent.attr('title',
						availableBlocks[i].components[j].name);
				...
				componentImage.css('background-image', 'url('
						+ availableBlocks[i].components[j].image + ')');
	...
	function showBlockScreen(e, ui) {
		...
		if (blockContainer.length == 0) {
			...
			$("#portal-block-container input").bind('focusin',
					prepareWidgetSearch);
			$("#portal-block-container input").bind('focusout',
					clearWidgetSearch);
			$("#portal-block-container input").bind('change', hideComponents);

			$(document).on('click', "#packages-list span.package span.title",
					filterComponents);

			$(document).on('click',
					"#packages-list span.package a.remove-package-filter",
					removePackageFilter);
		} else {
			$("#portal-block-container").show();
			$("#tab_menu li.current").addClass('current-no-border');
	...
	function filterComponents(e, ui) {
		...
		var search = $("#portal-block-container input");
		search
				.val(getTranslation('SearchForWidgets', null,
						translationContext));
		search.removeClass('query');
		...
		$('#component-title')
				.html(
						getTranslation('BrowseBlockCategory', null,
								translationContext));
	}
	...
	function hideComponents() {
		...
		if (typeof (searchQuery) !== 'undefined') {
			if (searchQuery == '') {
				$('#component-title')
						.html(
								getTranslation('BrowseBlocks', null,
										translationContext));
			} else {
				$('#component-title')
						.html(
								getTranslation('SearchBlocks', null,
										translationContext)
										+ ': ' + searchQuery)
				$("#packages-list .package").removeClass('selected-package');
				$("#packages-list .package .selected-package-indicator").hide();
		...
		var selectedPackage = $('#packages-list .package.selected-package')
				.attr('id');
		if (typeof (selectedPackage) == 'string'
				&& selectedPackage !== 'all_packages') {
		...
		for ( var i in availableBlocks) {
			...
			for ( var j in availableBlocks[i].components) {
				...
				var block = $('#' + str_replace('\\', '\\\\', i) + '-'
						+ availableBlocks[i].components[j].id);

				if (typeof (searchQuery) !== 'undefined'
						&& blockName.indexOf(searchQuery) === -1) {
		...
		if ($('#components .component').length == 0) {
			...
			if (typeof (searchQuery) !== 'undefined') {
				noMatches.html('Sorry, your search for "' + searchQuery
						+ '" does not match any widgets.');
			} else {
	...
	function addColumn(e, ui) {
		...
		row = $(".portal_tab:visible .portal_row:first");
		rowId = row.attr('id');
		...
		var response = $.ajax({
			...
			if (lastColumn.length > 0) {
				...
				$("div.portal_column", row).each(function(i) {
					var newWidth = newWidths[this.id] + '%';
					this.style.width = newWidth;
				});

				$("div.portal_column:last", row).after(columnHtml);
			} else {
				...
				.bind(
						...
						function(e) {
							...
							if (code === 13) {
								...
								tabId = $('#tab_menu li.current').attr('id')
										.split("_");
								tabId = tabId[2];
								...
								var response = $
										...
										.success(
												function(json) {
													if (json.result_code == 200) {
														$(
																'#tab_menu li.current a.tabTitle')
																.html(
																		$(
																				'#current-tab-title')
																				.val());
				...
				.success(
						function(json) {
							if (json.result_code == 200) {
								...
								columnWidth = parseInt(columnWidth.replace('%',
										''), 10);
								column.remove();
								...
								if (otherColumn.length > 0) {
									...
									otherColumnWidth = parseInt(
											otherColumnWidth.replace('%', ''),
											10);

									// Calculate the new width
									newColumnWidth = Math
											.max(
													101 - (total_width - otherColumnWidth),
													otherColumnWidth);
									newColumnWidth = Math.min(100,
											newColumnWidth);
									newColumnWidth = columns.length === 1 ? 100
											: newColumnWidth; // i.e.
				...
				.success(
						function(json) {
							if (json.result_code == 200) {
								...
								$('form#home_block button[name="submit"]')
										.bind(
												'click',
												function(e, ui) {
													e.preventDefault();
	...
	function bindIcons() {
		...
		$(document).on('click', "#tab_menu li.current a.tabTitle",
				function(e, ui) {
					e.preventDefault();
				});
	...
	function processDroppedBlock(e, ui) {
		...
		newColumn = $(
				"#portal_tab_" + newTabId
						+ " .portal_row:first .portal_column:first").attr('id');
				...
				.success(
						function(json) {
							if (json.result_code == 200) {
								...
								$("#tab_menu ul li.current").removeClass(
										'current');
								$("#tab_menu ul li").addClass('normal');
								$("#tab_menu ul li#tab_select_" + newTabId)
										.addClass('current');
								$("#tab_menu ul li#tab_select_" + newTabId)
										.removeClass('normal');
								$(".portal_tab").css('display', 'none');
								$("#portal_tab_" + newTabId).css('display',
										'block');
