...
$(function() {
    ...
    function hideComponents() {
        ...
        $('#component-title').html(getTranslation('SearchBlocks', null, translationContext) + ': ' + searchQuery)
        $("#packages-list .package").removeClass('selected-package');
        $("#packages-list .package .selected-package-indicator").hide();
        $("#packages-list .package .remove-package-filter").hide();
        }
    ...
    function editTab(e, ui) {
        ...
        $("#current-tab-title").bind('keypress', {}, function(e) {
    ...
    function configureBlock(e, ui) {
        ...
        var response = $.ajax({
            ...
            if (json.result_code == 200) {
            var form = $('<div />');
            form.append($('<div />').append($('<h3 />').html(getTranslation('ConfigureBlock', {
    ...
    function __NEWaddBlock(event, interface) {
        var column, columnId, order, isEmpty;

        column = $(".portal-column:visible:last");
