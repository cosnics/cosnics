    ...
    public function run()
    {
        ...
        if ($id)
        {
            ...
            $form = new UserViewForm(
                UserViewForm::TYPE_EDIT, $user_view, $this->get_url([self::PARAM_USER_VIEW_ID => $id]),
                $this->get_user()
            );
            ...
            if ($form->validate())
            {
                ...
                $message = Translation::get(
                    $success ? 'ObjectUpdated' : 'ObjectNotUpdated', ['OBJECT' => Translation::get('UserView')],
                ...
                $this->redirectWithMessage(
                    $message, !$success, [self::PARAM_ACTION => self::ACTION_BROWSE]
                );
            }
            else
            {
        ...
        else
        {
            return $this->display_error_page(
                htmlentities(
                    Translation::get(
                        'NoObjectSelected', ['OBJECT' => Translation::get('UserView')], StringUtilities::LIBRARIES
                    )
                )
            );
        }
