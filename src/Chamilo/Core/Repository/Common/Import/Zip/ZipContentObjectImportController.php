        ...
        if (self::is_available())
        {
            if (in_array($file->get_extension(), self::get_allowed_extensions()))
            {
                ...
                else
                {
                    ...
                    {
                        $this->add_message(
                            Translation::get('NotImportedCategoryWithSameNameExists'), self::TYPE_ERROR
                        );
                    }
                }
            }
    ...
    protected function get_category_by_path($path)
    {
        ...
        $category->set_name(
            DataManager::create_unique_category_name(
                $this->get_parameters()->getWorkspace(), $parent_category->get_id(), $base_name
            )
        );
