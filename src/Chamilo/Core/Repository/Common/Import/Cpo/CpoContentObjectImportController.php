...
class CpoContentObjectImportController extends ContentObjectImportController
{
    ...
    public function create_sub_items($content_object_node)
    {
        ...
        foreach ($sub_item_list as $key => $sub_item_node)
        {
            ...
            if (!$this->get_complex_content_object_item_id_cache_id($id))
            {
                ...
                $complex_content_object_item->set_user_id($this->getSession()->get(Manager::SESSION_USER_ID));
                $complex_content_object_item->set_parent(
                    $this->get_content_object_id_cache_id($content_object_node->getAttribute('id'))
                );
                $complex_content_object_item->set_display_order($key + 1);
                foreach ($complex_content_object_item->getAdditionalPropertyNames() as $additional_property)
                {
                    $complex_content_object_item->setAdditionalProperty(
                        $additional_property, $sub_item_node->getAttribute($additional_property)
                    );
                }
    ...
    public function determine_content_object_type($xpath_value)
    {
         ...
         */
        if (!str_contains($xpath_value, '\\'))
        {
            $context = 'Chamilo\Core\Repository\ContentObject\\' .
                $this->getStringUtilities()->createString($xpath_value)->upperCamelize();
        }
        else
        {
            $context = $classnameUtilities->getNamespaceParent($xpath_value, 3);
        }

        $registration = $registrationConsulter->getRegistrationForContext($context);
        ...
        return $registration[Registration::PROPERTY_CONTEXT] . '\Storage\DataClass\\' .
            $classnameUtilities->getPackageNameFromNamespace(
    ...
    public function get_complex_content_object_item_id_cache_id($old_item_id)
    {
        return $this->get_cache_id(
            ComplexContentObjectItem::class, ComplexContentObjectItem::PROPERTY_ID, $old_item_id
        );
    }
    ...
    {
        return $this->get_cache_id(
            ContentObject::class, ContentObject::PROPERTY_OBJECT_NUMBER, $old_object_number
        );
    }
    ...
    public function process_content_object($content_object_node)
    {
        ...
        if (($this->getConfigurationConsulter()->getSetting(['Chamilo\Core\Repository', 'description_required']) ==
                '1') && $content_object->get_description() == '')
        {
    ...
    {
        $this->set_cache_id(
            ComplexContentObjectItem::class, ComplexContentObjectItem::PROPERTY_ID, $old_item_id, $new_item_id
        );
    }
