...
class TagsFormBuilder
{
    ...
    {
        $resource_manager = $this->form->getResourceManager();
        $plugin_path = $this->form->getWebPathBuilder()->getJavascriptPath('Chamilo\Core\Repository') .
    ...
    public function build_form($available_tags, $default_tags = [])
    {
        ...
        $this->form->add_textfield(
            self::PROPERTY_TAGS, Translation::get('Tags'), false, ['id' => 'tagsinput', 'style' => 'width: 95%']
        );
