...
class DeleterComponent extends Manager
{
    ...
    public function run()
    {
        ...
        try
        {
            ...
            {
                $entityRelationIdentifiers = [$entityRelationIdentifiers];
            }
            ...
            foreach ($entityRelationIdentifiers as $entityRelationIdentifier)
            {
                ...
                if ($this->getRightsService()->hasWorkspaceCreatorRights(
                    $this->get_user(), $this->getCurrentWorkspace()
                ))
                {
                    if (!$entityRelationService->deleteEntityRelation($entityRelation))
                    {
                        throw new Exception(
                            Translation::get(
                                'ObjectNotDeleted', ['OBJECT' => Translation::get('Workspace')],
            ...
            $message = Translation::get(
                'ObjectDeleted', ['OBJECT' => Translation::get('Workspace')], StringUtilities::LIBRARIES
            );
        }
        catch (Exception $ex)
        {
