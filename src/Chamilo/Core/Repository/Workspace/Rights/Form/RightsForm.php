...
class RightsForm extends FormValidator
{
    ...
    public const PROPERTY_ACCESS = 'targets';

    public const PROPERTY_COPY = 'right_copy';

    public const PROPERTY_MANAGE = 'right_manage';

    public const PROPERTY_USE = 'right_use';

    public const PROPERTY_VIEW = 'right_view';
    ...
    public function addSaveResetButtons()
    {
        ...
        if ($this->mode == self::MODE_CREATE)
        {
            $buttons[] = $this->createElement(
                'style_submit_button', 'submit', Translation::get('SaveAndAddNew', [], Manager::CONTEXT),
                ['class' => 'positive']
            );
        }
    ...
    public function build_form()
    {
        if ($this->entityRelation instanceof WorkspaceEntityRelation)
        {
            if ($this->entityRelation->get_entity_type() == UserEntity::ENTITY_TYPE)
            {
                ...
                $entityName = DataManager::retrieve_by_id(
                    User::class, $this->entityRelation->get_entity_id()
                )->get_fullname();
            }
            else
            {
                ...
                $entityName = DataManager::retrieve_by_id(
                    Group::class, $this->entityRelation->get_entity_id()
                )->get_name();
            }
        ...
        else
        {
            ...
            $this->addElement(
                'advanced_element_finder', self::PROPERTY_ACCESS, Translation::get('UsersGroups'), $types
            );
        }

        $this->addElement(
            'radio', self::PROPERTY_VIEW, Translation::get('ContentRight'), Translation::get('ViewRight'),
            RightsService::RIGHT_VIEW
        );

        $this->addElement(
            'radio', self::PROPERTY_VIEW, null, Translation::get('AddRight'),
            RightsService::RIGHT_VIEW | RightsService::RIGHT_ADD
        );

        $this->addElement(
            'radio', self::PROPERTY_VIEW, null, Translation::get('EditRight'),
            RightsService::RIGHT_VIEW | RightsService::RIGHT_ADD | RightsService::RIGHT_EDIT
        );

        $this->addElement(
            'radio', self::PROPERTY_VIEW, null, Translation::get('DeleteRight'),
            RightsService::RIGHT_VIEW | RightsService::RIGHT_ADD | RightsService::RIGHT_EDIT |
            RightsService::RIGHT_DELETE
        );

        $this->addElement(
            'checkbox', self::PROPERTY_USE, Translation::get('UseRight'), null, null, RightsService::RIGHT_USE
        );

        $this->addElement(
            'checkbox', self::PROPERTY_COPY, Translation::get('CopyRight'), null, null, RightsService::RIGHT_COPY
        );

        $this->addElement(
            'checkbox', self::PROPERTY_MANAGE, Translation::get('ManageRight'), null, null,
            RightsService::RIGHT_VIEW | RightsService::RIGHT_ADD | RightsService::RIGHT_EDIT |
        ...
        $this->addElement(
            ...
            $this->getWebPathBuilder()->getJavascriptPath('Chamilo\Core\Repository\Workspace\Rights') . 'RightsForm.js'
        )
        );
    ...
    public function setDefaults($defaults = [], $filter = null)
    {
        if ($this->entityRelation instanceof WorkspaceEntityRelation)
        {
            ...
            $defaults[self::PROPERTY_VIEW] =
                $givenRights & ~RightsService::RIGHT_USE & ~RightsService::RIGHT_COPY & ~RightsService::RIGHT_MANAGE;
