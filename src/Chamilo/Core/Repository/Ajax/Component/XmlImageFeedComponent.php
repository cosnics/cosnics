...
class XmlImageFeedComponent extends Manager
{
    ...
    function run()
    {
        ...
        foreach ($image_types as $type)
        {
            $image_type_conditions[] = new PatternMatchCondition(
                new PropertyConditionVariable(File::class_name(), File::PROPERTY_FILENAME), '*.' . $type
            );
        }
        ...
        $objects = DataManager::retrieve_active_content_objects(
            File::class_name(), new DataClassRetrievesParameters($condition)
        );
    ...
    public function dump_tree($tree, $objects)
    {
        ...
        foreach ($tree as $node)
        {
            ...
            echo '<node id="category_', $id, '" classes="' . $glyph->getClassNamesString() . '" title="', htmlentities(
                $title
            ), '">', PHP_EOL;
            ...
            foreach ($objects[$id] as $lo)
            {
                ...
                echo '<leaf id="lo_' . $id . '" classes="' . $value['classes'] . '" title="' .
                    htmlspecialchars($value['title']) . '" description="' . htmlspecialchars($value['description']) .
                    '" />' . PHP_EOL;
            }
