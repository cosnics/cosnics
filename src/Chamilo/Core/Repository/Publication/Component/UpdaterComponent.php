    ...
    public function run()
    {
        ...
        if (!empty($application) && !empty($publication_id))
        {
            $publication_attributes = DataManager::get_content_object_publication_attribute(
                $publication_id, $application, $publicationContext
            );
            ...
            $this->redirectWithMessage(
                Translation::get(
                    $success ? 'ObjectUpdated' : 'ObjectNotUpdated', ['OBJECT' => Translation::get('Publication')],
        ...
        else
        {
            $this->display_warning_page(
                htmlentities(
                    Translation::get(
                        'NoObjectSelected', ['OBJECT' => Translation::get('Publication')], StringUtilities::LIBRARIES
                    )
                )
            );
        }
