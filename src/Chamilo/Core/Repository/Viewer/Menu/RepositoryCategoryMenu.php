...
class RepositoryCategoryMenu extends ContentObjectCategoryMenu
{
     ...
     * @param string $url_format              The format to use for the URL of a category. Passed to sprintf().
     *                                        Defaults to the string "?category=%s".
     * @param array $extra_items              An array of extra tree items, added to the root.
     * @param string[] $filter_count_on_types - Array to define the types on which the count on the categories should
     *                                        be
     *                                        filtered
     */
    public function __construct(
        $parent, $owner, Workspace $currentWorkspace, $current_category = null, $url_format = '?category=%s',
        $extra_items = [], $filter_count_on_types = [], $exclude_types = []
    )
    {
        $this->parent = $parent;
        parent::__construct(
            $currentWorkspace, $current_category, $url_format, $extra_items, $filter_count_on_types, $exclude_types
        );
    }
