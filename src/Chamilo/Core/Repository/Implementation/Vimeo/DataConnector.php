...
class DataConnector extends \Chamilo\Core\Repository\External\DataConnector
{
    ...
    public function update_external_repository_object($values)
    {
        ...
        {
            $response = $this->vimeo->request('/videos/' . $values['id'], array('title' => $values['title']), 'PATCH');
            if (! $response->stat == 'ok')
            {
                return false;
            }
            ...
            {
                $response = $this->vimeo->request('/videos/' . $values['id']);
                if ($response->stat == 'ok')
                {
                    $response = $this->vimeo->request(
                        '/videos/' . $values['id'],
                        array('tags' => $values['tags']),
                        'PUT');
                    if (! $response->stat == 'ok')
                    {
                        return false;
    ...
    public function create_external_repository_object($values, $video_path)
    {
        ...
        {
            $response = $this->vimeo->request('/videos/' . $video_id, array('name' => $values['title']), 'PATCH');
            if (! $response->stat == 'ok')
            {
                return false;
            }
            ...
            {
                $response = $this->vimeo->request('/videos/' . $video_id);
                if ($response->stat == 'ok')
                {
                    $response = $this->vimeo->request(
                        '/videos/' . $values['id'],
    ...
    public function export_external_repository_object($content_object)
    {
        ...
        $response = $this->vimeo->request(
            '/videos/' . $video_id,
            array('description' => $content_object->get_description()),
            'PATCH');
        if (! $response->stat == 'ok')
        {
            return false;
        ...
        else
        {
            $response = $this->vimeo->request(
                '/videos/' . $video_id,
                array('name' => $content_object->get_title()),
                PATCH);
            if (! $response->stat == 'ok')
            {
                return false;
