...
jQuery(function($) {
	...
	// apply IE8 html5 fix - thanx to Remy Sharp -
	...
	projekktor = $p = function() {
		...
		if (typeof arg == 'string') {
			...
			for ( var i = 0; i < projekktors.length; i++) {
				...
				try {
					for ( var j = 0; j < $(arg).length; j++) {
						if (projekktors[i].env.playerDom.get(0) == $(arg)
								.get(j)) {
							instances.push(projekktors[i]);
							continue;
						}
			...
			if (instances.length > 0) {
				return (instances.length == 1) ? instances[0] : new Iterator(
						instances);
			}
		}
		...
		function PPlayer(srcNode, cfg, onReady) {
			...
			this._reelUpdate = function(obj) {
				var ref = this, itemIdx = null, data = obj || [ {} ], files = data.playlist
						|| data;
				...
				try {
					for ( var props in data.config) {
						if (data.config.hasOwnProperty(props)) {
							if (typeof data.config[props]
									.indexOf('objectfunction') > -1) {
								continue; // IE SUCKZ
							}
					...
					if (data.config != null) {
						$p.utils.log('Updated config var: ' + props + ' to '
								+ this.config[props]);
			...
			this._canPlay = function(mediaType, platform, streamType) {
				var ref = this, checkIn = [], checkFor = [], st = streamType
						|| 'http', pltfrm = (typeof platform == 'object') ? platform
						: [ platform ], type = (mediaType) ? mediaType.replace(
						/x-/, '') : undefined, tm = ref._testMediaSupport();

				$.each(pltfrm, function(nothing, plt) {
					$.each($.extend(tm[st], tm['*'] || []) || [], function(
							thisPlatform, val) {
						if (plt != null) {
							if (thisPlatform != plt) {
								return true;
			...
			this._prepareMedia = function(data) {
				var ref = this, types = [], mediaFiles = [], qualities = [], extTypes = {}, typesModels = {}, modelSets = [], result = {}, extRegEx = [], bestMatch = 0;

				// build regex string and filter dublicate extensions and more
				// ...
				for ( var mmapIndex in $p.mmap) {
					if ($p.mmap.hasOwnProperty(mmapIndex)) {
						platforms = (typeof $p.mmap[mmapIndex].platform == 'object') ? $p.mmap[mmapIndex].platform
								: [ $p.mmap[mmapIndex].platform ];
						$
								.each(
										platforms,
										function(_na, platform) {
											var k = 0, streamType = 'http';

											for ( var j in data.file) {
												if (data.file.hasOwnProperty(j)) {
													if (j === 'config')
														continue;
													streamType = data.file[j].streamType
															|| ref
																	.getConfig('streamType')
															|| 'http';

													if (ref
															._canPlay(
																	$p.mmap[mmapIndex].type,
																	platform,
																	streamType)) {
														k++;
													}
													...
													// support any of the
													// provided streamtypes:
													if (k === 0) {
														continue;
													}

													// set priority level
													$p.mmap[mmapIndex].level = $
															.inArray(
																	platform,
																	ref.config._platforms);
													$p.mmap[mmapIndex].level = ($p.mmap[mmapIndex].level < 0) ? 100
															: $p.mmap[mmapIndex].level;

													// upcoming fun:
													extRegEx
															.push('.'
																	+ $p.mmap[mmapIndex].ext);

													// build extension2filetype
													// map
													if (!extTypes[$p.mmap[mmapIndex].ext]) {
														extTypes[$p.mmap[mmapIndex].ext] = [];
													}
													extTypes[$p.mmap[mmapIndex].ext]
															.push($p.mmap[mmapIndex]);

													if ($p.mmap[mmapIndex].streamType === null
															|| $p.mmap[mmapIndex].streamType == '*'
															|| $
																	.inArray(
																			streamType
																					|| [],
				...
				if ($.isEmptyObject(data) || data.file === false
						|| data.file === null) {
					data.file = [ {
						'src' : null
					} ];
				...
				for ( var index in data.file) {
					if (data.file.hasOwnProperty(index)) {
						...
						// get file extension:
						/**
						 * try { data.file[index].ext =
						 * data.file[index].src.match( new RegExp(extRegEx))[1];
						 * data.file[index].ext = (!data.file[index].ext) ?
						 * 'NaN' : data.file[index].ext.replace('.',''); }
						 * catch(e) { data.file[index].ext='NaN'; }
						 */

						// if type is set, get rid of the codec mess
						if (data.file[index].type != null
								&& data.file[index].type !== '') {
							try {
								var codecMatch = data.file[index].type.split(
										' ').join('').split(
										/[\;]codecs=.([a-zA-Z0-9\,]*)[\'|\"]/i);
								if (codecMatch[1] != null) {
									data.file[index].codec = codecMatch[1];
								}
								data.file[index].type = codecMatch[0].replace(
										/x-/, '');
								data.file[index].originalType = codecMatch[0];
							} catch (e) {
							}
						} else {
							data.file[index].type = this
									._getTypeFromFileExtension(data.file[index].src);
						}

						if (typesModels[data.file[index].type]
								&& typesModels[data.file[index].type].length > 0) {

							typesModels[data.file[index].type].sort(function(a,
									b) {
								return a.level - b.level;
							});

							modelSets
									.push(typesModels[data.file[index].type][0]);
						}
					}
				}
				...
				for (index in data.file) {
					if (data.file.hasOwnProperty(index)) {
						...
						if (($.inArray(data.file[index].type.replace(/x-/, ''),
								types) < 0)
								&& modelSet.type != 'none/none') {
							continue;
						}

						// make srcURL absolute for non-RTMP files
						if ($.isEmptyObject(data.config)
								|| data.config.streamType == null
								|| data.config.streamType.indexOf('rtmp') == -1) {
							data.file[index].src = $p.utils
									.toAbsoluteURL(data.file[index].src);
						}
				...
				result = {
					...
					platforms : platforms,
					qualities : $p.utils.intersect($p.utils.unique(_setQual),
							$p.utils.unique(qualities)),
			...
			this._modelUpdateListener = function(type, value) {
				var ref = this, modelRef = this.playerModel;

				if (!this.playerModel.init)
					return;
				if (type != 'time' && type != 'progress') {
					$p.utils.log("Update: '" + type, this.playerModel.getSrc(),
							this.playerModel.getModelName(), value);
				}

				switch (type) {
				case 'state':
					this._promote('state', value); // IMPORTANT: STATES must be
													// promoted before being
													// processed!

					var classes = $.map(this.getDC().attr("class").split(" "),
							function(item) {
								return item.indexOf(ref.getConfig('ns')
										+ "state") === -1 ? item : "";
							});

					classes.push(this.getConfig('ns') + "state"
							+ value.toLowerCase());
					...
					case 'COMPLETED':
						...
						if (this._currentItem + 1 >= this.media.length
							...
							if (this.getConfig('leaveFullscreen')) {
								this.reset(); // POW, crowba-method for
												// Firefox!
								return;
							}
						}
				...
				/*
				 * case 'durationChange': console.log(
				 * this.getConfig('start')>0, this.playerModel.allowRandomSeek )
				 * if (this.playerModel.allowRandomSeek==true) { if
				 * (this.getConfig('start')>0) { console.log("setPLay")
				 * this.setPlayhead(this.getConfig('start'));
				 * this.setConfig({start: 0}); } } console.log(
				 * this.getConfig('start') ) break;
				 */
			...
			this._MD = function(event) {
				projekktor('#' + event.currentTarget.id.replace(/_media$/, ''))
						._playerFocusListener(event);
			};
			...
			this._addGUIListeners = function() {
				...
				if (this.getDC().get(0).addEventListener) {
					this.getDC().get(0).addEventListener("mousedown", this._MD,
							true);
				} else {
					// IE *sigh*
			...
			this._removeGUIListeners = function() {
				...
				if (this.getDC().get(0).removeEventListener) {
					this.getDC().get(0).removeEventListener("mousedown",
							this._MD, true);
				} else {
			...
			this._registerPlugins = function() {
				var plugins = $.merge($.merge([], this.config._plugins),
						this.config._addplugins), pluginName = '', pluginObj = null;
				...
				for ( var i = 0; i < plugins.length; i++) {
					pluginName = "projekktor"
							+ plugins[i].charAt(0).toUpperCase()
							+ plugins[i].slice(1);
					try {
						typeof eval(pluginName);
					} catch (e) {
						alert("Projekktor Error: Plugin '" + plugins[i]
								+ "' malicious or not available.");
						continue;
					}

					pluginObj = $.extend(true, {},
							new projekktorPluginInterface(),
							eval(pluginName).prototype);
					pluginObj.name = plugins[i].toLowerCase();
					pluginObj.pp = this;
					pluginObj.playerDom = this.env.playerDom;
					pluginObj._init(this.config['plugin_'
							+ plugins[i].toLowerCase()]
							|| {});

					if (this.config['plugin_' + pluginObj.name] == null)
						this.config['plugin_' + pluginObj.name] = {};

					this.config['plugin_' + pluginObj.name] = $.extend(true,
							{}, pluginObj.config || {});
			...
			this.removePlugins = function(rmvPl) {
				...
				var pluginsToRemove = rmvPl
						|| $.merge($.merge([], this.config._plugins),
								this.config._addplugins), pluginsRegistered = this._plugins.length;

				for ( var j = 0; j < pluginsToRemove.length; j++) {
					for ( var k = 0; k < pluginsRegistered; k++) {
						if (this._plugins[k] != undefined) {
							if (this._plugins[k].name == pluginsToRemove[j]
									.toLowerCase()) {
								...
								for ( var events in this._pluginCache) {
									for ( var shortcuts = 0; shortcuts < this._pluginCache[events].length; shortcuts++) {
										if (this._pluginCache[events][shortcuts].name == pluginsToRemove[j]
												.toLowerCase()) {
											this._pluginCache[events].splice(
													shortcuts, 1);
										}
									}
								}
			...
			this.__promote = function(evt, value) {
				...
				if (typeof event == 'object') {
					if (!event._plugin)
						return;
					event = 'plugin_' + event._plugin
							+ $p.utils.capitalise(event._event.toUpperCase());
				}

				if (event != 'time' && event != 'progress'
						&& event != 'mousemove')
					$p.utils.log("Event: [" + event + "]", value,
							this.listeners);

				// fire on plugins
				if (this._pluginCache[event + "Handler"]
						&& this._pluginCache[event + "Handler"].length > 0) {
					for ( var i = 0; i < this._pluginCache[event + "Handler"].length; i++) {
						if (this.getConfig('debug')) {
							try {
								this._pluginCache[event + "Handler"][i][event
										+ "Handler"](value, this);
							} catch (e) {
								$p.utils.log(e)
							}
				...
				if (this._pluginCache["eventHandler"]
						&& this._pluginCache["eventHandler"].length > 0) {
					for ( var i = 0; i < this._pluginCache["eventHandler"].length; i++) {
						if (this.getConfig('debug')) {
							try {
								this._pluginCache["eventHandler"][i]["eventHandler"]
										(event, value, this);
							} catch (e) {
								$p.utils.log(e)
							}
			...
			this._playerFocusListener = function(evt) {
				...
				case 'mousedown':
					...
					if ("|TEXTAREA|INPUT".indexOf('|'
							+ evt.target.tagName.toUpperCase()) > -1) {
						return;
					}
			...
			this._keyListener = function(evt) {
				...
				var ref = this, set = (this.getConfig('keys').length > 0) ? this
						.getConfig('keys')
						: [ {
							27 : function(player) {
							...
							84 : function(player) {
								$p.utils.log('Cuepoints Dump', player
										.getCuePoints());
							} // T
						} ];
			...
			this._enterFullViewport = function(forcePlayer) {
				...
				var win = this.getIframeParent() || $(window), target = this
						.getIframe()
						|| this.getDC(), overflow = $(win[0].document.body)
						.css('overflow');
				...
				target.data('fsdata', {
					...
					bodyOverflow : (overflow == 'visible') ? 'auto' : overflow, // prevent
																				// IE7
																				// crash
					bodyOverflowX : $(win[0].document.body).css('overflow-x'), // prevent
																				// IE7
																				// crash
					bodyOverflowY : $(win[0].document.body).css('overflow-y'), // prevent
																				// IE7
																				// crash
			...
			this._exitFullViewport = function(forcePlayer) {
				...
				var win = this.getIframeParent() || $(window), target = this
						.getIframe()
						|| this.getDC(), fsData = target.data('fsdata') || null;
				...
				if (fsData != null) {
					...
					win.scrollTop(fsData.scrollTop).scrollLeft(
							fsData.scrollLeft);
					$(win[0].document.body)
							.css('overflow', fsData.bodyOverflow);
					$(win[0].document.body).css('overflow-x',
							fsData.bodyOverflowX);
					$(win[0].document.body).css('overflow-y',
							fsData.bodyOverflowY);

					// rebuild iframe:
					if (fsData.iframeWidth > 0 && !forcePlayer) {
						target.attr('width', fsData.iframeWidth + "px").attr(
								'height', fsData.iframeHeight + "px");
					} else {
						target.width(fsData.targetWidth).height(
								fsData.targetHeight);
					}
					target.attr(
							'style',
							(fsData.targetStyle == null) ? ''
									: fsData.targetStyle).data('fsdata', null);
				}
			};
			...
			this.pluginAPI = function() {
				var args = Array.prototype.slice.call(arguments) || null, dest = args
						.shift(), func = args.shift();
			...
			this.getConfig = function(name, itemIdx) {
				var idx = itemIdx || this._currentItem, result = (this.config['_'
						+ name] != null) ? this.config['_' + name]
						: this.config[name];
				...
				if (name.indexOf('plugin_') > -1) {
					try {
						if (this.media[idx]['config'][name]) {
							result = $.extend(true, {}, this.config[name],
			...
			this.getItemCount = function() {
				// ignore NA dummy
				return (this.media.length == 1 && this.media[0].mediaModel == 'na') ? 0
						: this.media.length;
			};
			...
			this.getItem = function() {
				...
				default:
					return $.extend(true, {}, this.media[arguments[0]
							|| this._currentItem]
							|| {});
				}
			};
			...
			this.getVolume = function() {
				return (this.getConfig('fixedVolume') === true) ? this.config.volume
					...
					if (this.env.playerDom.find('.' + this.getNS() + 'display').length > 0) {
						...
						.css({
							...
							display : 'block'
								...
								this.env.playerDom.find('.' + this.getNS()
										+ 'display'));
					}
			...
			this.getMediaType = function() {
				...
				try {
					return this._getTypeFromFileExtension(this.playerModel
							.getSrc())
							|| 'na/na';
				} catch (e) {
					return 'na/na';
			...
			this.getIframeAllowFullscreen = function() {
				var result = null;
				try {
					result = window.frameElement.attributes.allowFullscreen
			...
			this.getIsMobileClient = function(what) {
				...
				var mobileAgents = [ 'android', "windows ce", 'blackberry',
						'palm', 'mobile' ];

				for ( var i = 0; i < mobileAgents.length; i++) {
					if (uagent.indexOf(mobileAgents[i]) > -1) {
						// if (uagent.indexOf('webkit')>-1) return false;
						return (what) ? (mobileAgents[i].toUpperCase() == what
								.toUpperCase()) : true;
					}
				}
				return false;
			...
			this.getPlatforms = function() {
				var ref = this, platforms = this._testMediaSupport(true), cfg = this
						.getConfig('platforms'), tmp = [], result = [];

				try {
					for ( var i in this.media[this._currentItem].file) {
						if (this.media[this._currentItem].file
								.hasOwnProperty(i)) {
							for ( var j in platforms) {
								if (this
										._canPlay(
												this.media[this._currentItem].file[i].type
														.replace(/x-/, ''),
												platforms[j].toLowerCase(),
												this.getConfig('streamType'))) {
									if ($.inArray(platforms[j].toLowerCase(),
											result) == -1) {
			...
			this.getNativeFullscreenSupport = function() {
				var ref = this, fullScreenApi = {
					supportsFullScreen : 'viewport', // viewport=full
				...
				if (typeof document.cancelFullScreen != 'undefined') {
					...
					for ( var i = 0, il = browserPrefixes.length; i < il; i++) {
						...
						// media element only
						if (typeof document.createElement('video')[fullScreenApi.prefix
						...
						if (typeof document[fullScreenApi.prefix
							...
							if (fullScreenApi.prefix == 'moz'
									&& typeof document[fullScreenApi.prefix
				...
				if (fullScreenApi.supportsFullScreen == 'viewport'
						|| (fullScreenApi.supportsFullScreen == 'dom' && this
								.getConfig('forceFullViewport'))) {
					return fullScreenApi;
				}

				// MEDIA ONLY:
				// the browser supports true fullscreen for the media element
				// only - this is semi cool
				if (fullScreenApi.supportsFullScreen == 'mediaonly') {
					fullScreenApi.requestFullScreen = function(el) {
						ref.playerModel.getMediaElement().get(0)[this.prefix
								+ 'EnterFullscreen']();
					}
					fullScreenApi.dest = {};
				...
				fullScreenApi.isFullScreen = function(esc) {
					...
					var dest = (ref.getIframe()) ? parent.window.document
							: document;
					switch (this.prefix) {
					case '':
					...
					case 'moz':
						return dest[this.prefix + 'FullScreen']
								|| (ref.getDC().hasClass('fullscreen') && esc !== true);
					default:
						return dest[this.prefix + 'FullScreen'];
					}
				}

				// the browser supports true fullscreen for any DOM container -
				// this is ubercool:
				fullScreenApi.requestFullScreen = function() {
					if (this.isFullScreen())
						return;

					var win = ref.getIframeParent() || $(window), target = (ref
							.getIframe()) ? ref.getIframe().get(0) : null
							|| ref.getDC().get(0), apiRef = this, dest = (ref
							.getIframe()) ? parent.window.document : document, win = ref
							.getIframeParent()
							|| $(window);
					...
					$(dest).bind(
							...
							function(evt) {
								if (!apiRef.isFullScreen(true)) {
									apiRef.ref.playerModel.applyCommand(
											'fullscreen', false);
									var win = apiRef.ref.getIframeParent()
											|| $(window), fsData = win
											.data('fsdata');
									if (fsData != null) {
				...
				fullScreenApi.cancelFullScreen = function() {
					...
					var target = ref.getIframe() ? parent.window.document
							: document, win = ref.getIframeParent()
							|| $(window), fsData = win.data('fsdata');

					$((ref.getIframe()) ? parent.window.document : document)
							.unbind(this.prefix + "fullscreenchange.projekktor");
			...
			this.getAppropriateQuality = function(qualities) {
				...
				var wid = this.env.playerDom.width(), hei = this.env.playerDom
						.height(), ratio = $p.utils.roundNumber(wid / hei, 2), temp = {};
				...
				return ($.inArray('auto', this.getPlaybackQualities()) > -1) ? 'auto'
						: temp.key || 'default';
			};
			...
			this.getFromUrl = function(url, dest, callback, customParser,
				...
				if (dataType) {
					if ($.parseJSON == undefined
							&& dataType.indexOf('json') > -1) {
						this
								._raiseError("Projekktor requires at least jQuery 1.4.2 in order to handle JSON playlists.");
						return this;
					}
					dataType = (dataType.indexOf('/') > -1) ? dataType
							.split('/')[1] : dataType;
				}
				...
				var ajaxConf = {
					...
					complete : function(xhr, status) {
						...
						data = $p.utils.cleanResponse(xhr.responseText,
								dataType)
					...
					async : aSync,
					dataType : dataType,
					jsonpCallback : (callback.substr(0, 1) != '_') ? false
							: "projekktor('" + this.getId() + "')._jsonp"
									+ callback,
			...
			this.setActiveItem = function(mixedData) {
				...
				// regular "autoplay" on:
				if (newItem === 0
						&& (lastItem == null || lastItem == newItem)
						&& (this.config._autoplay === true || 'DESTROYING|AWAKENING'
								.indexOf(this.getState()) > -1)) {
					ap = true;
				}
				// "continuous" playback?
				else if (this.getItemCount() > 1 && newItem != lastItem
						&& lastItem != null && this.config._continuous === true
						&& newItem < this.getItemCount()) {
					ap = true;
				}
				...
				if (!$p.models[newModel]) {
					...
					if (this.getConfig('className', null) != null) {
						this.getDC().addClass(
								this.getNS() + this.getConfig('className'))
					}
					this.getDC().addClass(
							this.getNS()
									+ (this.getConfig('streamType') || 'http'));
				...
				// persistent: (ap || this.config._continuous) &&
				// (newModel==nextUp)
				});
				return this;
			};
			...
			this.setFullscreen = function(goFull) {
				...
				goFull = (goFull == null) ? !nativeFullscreen.isFullScreen()
						: goFull;
			...
			this.setSize = function(data) {
				var target = this.getIframe() || this.getDC(), fsdata = target
						.data('fsdata')
						|| null, w = (data && data.width != null) ? data.width
						: (this.getConfig('width') != null) ? this
								.getConfig('width') : false, h = (data && data.height != null) ? data.height
						: (this.getConfig('height') == null && this
								.getConfig('ratio')) ? Math.round((w || this
								.getDC().width())
								/ this.getConfig('ratio')) : (this
								.getConfig('height') != null) ? this
								.getConfig('height') : false;
			...
			this._addListener = function(event, callback) {
				var evt = (event.indexOf('.') > -1) ? event.split('.') : [
						event, 'default' ];
				this.listeners.push({
					event : evt[0],
					ns : evt[1],
			...
			this.removeListener = function(event, callback) {
				var len = this.listeners.length, evt = (event.indexOf('.') > -1) ? event
						.split('.')
						: [ event, '*' ];
				...
				for ( var i = 0; i < len; i++) {
					...
					if (this.listeners[i].event != evt[0] && evt[0] !== '*')
						continue;
					if ((this.listeners[i].ns != evt[1] && evt[1] !== '*')
			...
			this.setFile = function() {
				...
				var fileNameOrObject = arguments[0] || '', dataType = arguments[1]
						|| this._getTypeFromFileExtension(fileNameOrObject), result = [];
				...
				if (typeof fileNameOrObject == 'object') {
					$p.utils.log('Applying incoming JS Object',
							fileNameOrObject);
				...
				result[0].file.type = dataType
						|| this._getTypeFromFileExtension(splt[0]);

				// incoming playlist
				if (result[0].file.type.indexOf('/xml') > -1
						|| result[0].file.type.indexOf('/json') > -1) {
					$p.utils.log('Loading external data from '
							+ result[0].file.src + ' supposed to be '
							+ result[0].file.type);
					this._playlistServer = result[0].file.src;
					this.getFromUrl(result[0].file.src, this, '_reelUpdate',
							this.getConfig('reelParser'), result[0].file.type);
					return this;
				}

				// incoming single file:
				$p.utils.log('Applying incoming single file:'
						+ result[0].file.src, result);
			...
			this.setPlaybackQuality = function(quality) {
				...
				if ($.inArray(qual, this.media[this._currentItem].qualities
						|| []) > -1) {
			 ...
			 */
					...
					this._reset = function() {
						...
						$(this).unbind();
						$(
								(this.getIframe()) ? parent.window.document
										: document).unbind(".projekktor");
						...
						for ( var i in this.config) {
							cleanConfig[(i.substr(0, 1) == '_') ? i.substr(1)
									: i] = this.config[i];
						}
					...
					this.setCuePoint = function(obj, opt) {
						var item = (obj.item !== undefined) ? obj.item : this
								.getItemIdx(), options = $.extend(true, {
							offset : 0
							...
							on : ($p.utils.toSeconds(obj.on) || 0)
									+ options.offset,
							off : ($p.utils.toSeconds(obj.off)
									|| $p.utils.toSeconds(obj.on) || 0)
									+ options.offset,
							value : obj.value || null,
							callback : obj.callback || function() {
							},
							precision : (obj.precision == null) ? 1
									: obj.precision,
							...
							_stateListener : function(state, player) {
								if ('STOPPED|COMPLETED|DESTROYING'
										.indexOf(state) > -1) {
									if (this._active)
										try {
							...
							_timeListener : function(time, player) {
								...
								if (player.getItemIdx() !== this.item
										&& this.item != '*')
									return;

								var timeIdx = (this.precision == 0) ? Math
										.round(time) : $p.utils.roundNumber(
										time, this.precision), ref = this;

								// are we already unlocked?
								// consider buffer state to unlock future
								// cuepoints for user interactions
								if (this._unlocked === false) {
									var approxMaxTimeLoaded = player
											.getDuration()
											* player.getLoadProgress() / 100;

									if (this.on <= approxMaxTimeLoaded
											|| this.on <= timeIdx) {
								...
								var nat = (timeIdx - this._lastTime <= 1 && timeIdx
										- this._lastTime > 0);

								// trigger ON
								if (((timeIdx >= this.on && timeIdx <= this.off) || (timeIdx >= this.on
										&& this.on == this.off && timeIdx <= this.on + 1))
										&& this._active !== true) {

									this._active = true;
									$p.utils.log("Cue Point: [ON " + this.on
											+ "] at " + timeIdx, this);
									try {
										this.callback({
											id : this.id,
								...
								// trigger OFF
								else if ((timeIdx < this.on || timeIdx > this.off)
										&& this.off != this.on
										&& this._active == true) {
									this._active = false;
									$p.utils.log("Cue Point: [OFF] at "
											+ this.off, this);
									try {
										this.callback({
											id : this.id,
								...
								if (this.off == this.on
										&& this._active
										&& new Number(timeIdx - this.on)
												.toPrecision(this.precision) >= 1) {
									this._active = false;
								}
					...
					this.getCuePoints = function(idx) {
						return this._cuePoints[idx || this.getItemIdx()]
								|| this._cuePoints || {};
					},
					...
					this.removeCuePoints = function(idx, group) {
						var cuePoints = this.getCuePoints(idx || 0) || {}, kill = [];
						for ( var cIdx = 0; cIdx < cuePoints.length; cIdx++) {
							if (cuePoints[cIdx].group == group) {
								this.removeListener('time',
										cuePoints[cIdx].timeEventHandler);
								this.removeListener('state',
										cuePoints[cIdx].stateEventHandler);
								kill.push(cIdx);
							}
						}
					...
					this._applyCuePoints = function(resync) {
						...
						if (this._cuePoints[this._currentItem] == null
								&& this._cuePoints['*'] == null) {
							return;
						}

						$.each($.merge(
								this._cuePoints[this._currentItem] || [],
								this._cuePoints['*'] || []), function(key,
								cuePointObj) {
							try {
								ref.removeListener('time',
										cuePointObj.timeEventHandler);
								ref.removeListener('state',
										cuePointObj.stateEventHandler);
							} catch (e) {
							}

							cuePointObj.timeEventHandler = function(time,
									player) {
								try {
							...
							cuePointObj.stateEventHandler = function(state,
									player) {
								try {
			...
			this._getTypeFromFileExtension = function(url) {
				var fileExt = '', extRegEx = [], extTypes = {}, extRegEx = [], plt = null, on = true;
						...
						for ( var j = 0; j < plt.length; j++) {
							if (plt[j] != null) {
								if (this.getConfig('enable'
										+ plt[j].toUpperCase() + 'Platform') === false
										|| $.inArray(plt[j], this
												.getConfig('platforms')) === -1) {
									on = false;
								}
							}
						}
			...
			/*
			 * generates an array of mediatype=>playertype relations depending
			 * on browser capabilities
			 */
			this._testMediaSupport = function(getPlatforms) {
				var result = {}, resultPlatforms = [], streamType = '', ref = this;
						...
						$
								.each(
										...
										function(_na, platform) {
											...
											streamType = $p.mmap[i]['streamType']
													|| [ 'http' ];
													...
													.each(
															...
															function(key, st) {
																...
																if ($
																		.inArray(
																				$p.mmap[i]['type'],
																				result[st][platform]) > -1)
																	return true;

																var reqPlatformVersion = ($p.models[$p.mmap[i]['model']
																		.toUpperCase()].prototype[(platform
																		.toLowerCase())
																		+ 'Version'] || "1")
																		.toString();
				...
				return (getPlatforms) ? $p._platformTableCache
						: $p._compTableCache;
			};
			...
			this._readMediaTag = function(domNode) {
				...
				if (!this.getConfig('ignoreAttributes')) {
					result = {
						autoplay : ((domNode.attr('autoplay') !== undefined || domNode
								.prop('autoplay') !== undefined) && domNode
								.prop('autoplay') !== false) ? true : false,
						controls : ((domNode.attr('controls') !== undefined || domNode
								.prop('controls') !== undefined) && domNode
								.prop('controls') !== false) ? true : false,
						loop : ((domNode.attr('autoplay') !== undefined || domNode
								.prop('loop') !== undefined) && domNode
								.prop('loop') !== false) ? true : false,
						title : (domNode.attr('title') !== undefined && domNode
								.attr('title') !== false) ? domNode
								.attr('title') : '',
						poster : (domNode.attr('poster') !== undefined && domNode
								.attr('poster') !== false) ? domNode
								.attr('poster') : '',
						width : (domNode.attr('width') !== undefined && domNode
								.attr('width') !== false) ? domNode
								.attr('width') : null,
						height : (domNode.attr('height') !== undefined && domNode
								.attr('height') !== false) ? domNode
								.attr('height') : null
					};
				}

				// IE7+8 and some other idiots do not keep attributes w/o
				// values:
				...
				if (domNode.attr('src')) {
					result.playlist[0].push({
						...
						type : domNode.attr('type')
								|| this._getTypeFromFileExtension(domNode
										.attr('src'))
					});
				}
								...
								result.playlist[0]
										.push({
											...
											type : childNode.attr('type')
													|| this
															._getTypeFromFileExtension(childNode
																	.attr('src')),
											quality : childNode
													.attr('data-quality')
													|| ''
							...
							case 'TRACK':
								if ($(this).attr('src')) {
									result.playlist[0]['config']['tracks']
											.push({
												src : childNode.attr('src'),
												kind : childNode.attr('kind')
														|| 'subtitle',
												lang : childNode
														.attr('srclang')
														|| null,
												label : childNode.attr('label')
														|| null
											});
								}
								break;
			...
			this._init = function(customNode, customCfg) {
				...
				var theNode = customNode || srcNode, theCfg = customCfg || cfg, cfgByTag = this
						._readMediaTag(theNode);
				...
				if (this.config._theme) {
					...
					case 'string':
						...
						// {id:this.config._theme, ver:this.config._version}),
			...
			this._start = function(data) {
				...
				var ref = this, iframeParent = this.getIframeParent();
				...
				if (typeof onReady === 'function') {
					// this._queue.unshift({command:function()
					// {onReady(ref);}});
					this._enqueue(function() {
						onReady(ref);
					});
				...
				for ( var i in this.config._playlist[0]) {
					...
					if (this.config._playlist[0][i].type) {
						if (this.config._playlist[0][i].type.indexOf('/json') > -1
								|| this.config._playlist[0][i].type
										.indexOf('/xml') > -1) {
							this.setFile(this.config._playlist[0][i].src,
									this.config._playlist[0][i].type);
							return this;
						}
					}
			...
			this._applyTheme = function(data) {
				...
				if (data === false) {
					this
							._raiseError('The Projekktor theme-set specified could not be loaded.')
					return false;
				}

				/*
				 * // check projekktor version if (typeof data. == 'string') {
				 * if ( (parseInt(this.config._version.split('.')[0]) || 0) <
				 * (parseInt(data.version.split('.')[0]) || 0) ||
				 * (parseInt(this.config._version.split('.')[1]) || 0) <
				 * (parseInt(data.version.split('.')[1]) || 0) ){
				 * this._raiseError('You are using Projekktor
				 * V'+this.config._version+' but the applied theme requires at
				 * least V'+data.version+'.'); return false; } }
				 */
				...
				if (typeof data.css == 'string') {
					$('head').append(
							'<style type="text/css">'
									+ $p.utils.parseTemplate(data.css, {
										'rp' : data.baseURL
									}) + '</style>');
				}
				...
				this.env.className = this.env.className
						&& this.env.className.length !== 0 ? this.env.className
						+ " " + data.id : data.id;
				...
				if (typeof data.config == 'object') {
					...
					if (typeof data.config.plugins == 'object') {
						for ( var i = 0; i < data.config.plugins.length; i++) {
							try {
								typeof eval('projekktor'
										+ data.config.plugins[i]);
							} catch (e) {
								this
										._raiseError('The applied theme requires the following Projekktor plugin(s): <b>'
												+ data.config.plugins
														.join(', ') + '</b>');
								return false;
							}
	...
	$p.newModel = function(obj, ext) {
		...
		$p.mmap = $
				.grep(
						$p.mmap,
						function(iLove) {
							var doesNotExist = iLove.model != ((obj.replace) ? obj.replace
									.toLowerCase()
...
/*
 * this file is part of: projekktor zwei http://www.projekktor.com
 * 
 * Copyright 2010-2012, Sascha Kluger, Spinning Airwhale Media,
 * http://www.spinningairwhale.com under GNU General Public License
...
projekktorConfig.prototype = {
	...
	/*
	 * Plugins to load on instance initialization, plugins are automatically
	 * extening the projekktorPluginInterface class. The order how the plugins
	 * are set here is important because they are added from z-index 0 to n one
	 * by one to the player DOM. As such it is usefull to add the "Display"
	 * plugin always first.
	 */
	_plugins : [ 'display', 'controlbar', 'contextmenu' ],

	/*
	 * Add one plugin or more plugins to the player. Alternative to "plugins"
	 * above. Will be merged with it.
	 */
	_addplugins : [],
	...
	/*
	 * Prefix prepended to all css-Classnames and data-attributes used by the
	 * player in order to avoid conflicts with existing layouts and scripts
	 */
	_ns : 'pp',
	...
	/*
	 * if set to true, projekktor assumes to live within an iframe and will act
	 * accordingly (used for embedding)
	 */
	_iframe : false,

	/*
	 * if set to true projekktor will discard native media tag attributes
	 * (loop,controls,autoplay,preload etc.pp)
	 */
	_ignoreAttributes : false,

	/*
	 * looping scheduled media elements - will be overwritten by loop-attribute
	 * of a replaced <video> tag.
	 */
	_loop : false,

	/*
	 * automatically start playback once page has loaded - will be overwritten
	 * by autoplay-attribute of a replaced <video> tag.
	 */
	_autoplay : false,

	/*
	 * if more than one item is scheduled, true will automatically start
	 * playback of the next item in line once current one completed
	 */
	_continuous : true,

	/*
	 * "true" will stop all other player instances but the one the user clicked
	 * play on.
	 */
	_thereCanBeOnlyOne : true,

	/*
	 * on "true" try to leave fullscreen on player "complete" - does not seem to
	 * work properly in Firefox... yeah!
	 */
	_leaveFullscreen : true,

	/*
	 * An array of items to be played. Check
	 * http://www.projekktor.com/docs/playlists to learn more
	 */
	_playlist : [],
	...
	/*
	 * the width of the player - >0= overwrite destNodes width, 0= keep dest
	 * node width, false=maintain ratio
	 */
	_width : null,

	/*
	 * guess what.... the hight of the player - >0= overwrite destNodes height,
	 * 0 = keep height , false=maintain ratio
	 */
	_height : null,
	...
	/*
	 * An array of objects featuring keycode=>function sets for
	 * keyboard-controls-customizing
	 */
	_keys : [],
	...
	/*
	 * foce full viewport if browser supports native media-element-fullscreen
	 * (e.g. iPad)
	 */
	...
	/*
	 * unique itemID for the item currently played - dynamically generated if
	...
	/*
	 * URL to poster image - will be overwritten by poster-attribute of the
	 * replaced media tag.
	 */
	poster : null,

	/*
	 * enable/disable controls - will be overwritten by controls-attribute of
	 * the replaced <video> tag.
	 */
	controls : true,
	...
	/*
	 * enable/disable the possibility to skip the video by hitting NEXT or using
	 * the SCRUBBER
	 */
	disallowSkip : false,

	/*
	 * if set to TRUE users can not change the volume of the player - neither
	 * via API nor through controls
	 */
	fixedVolume : false,

	/*
	 * scaling used for images (playlist items and posters) "fill",
	 * "aspectratio" or "none"
	 */
	imageScaling : 'aspectratio',

	/*
	 * scaling used for videos (flash and native, not youtube) "fill",
	 * "aspectratio" or "none"
	 */
	...
	/*
	 * defines the streamtype of the current item. 'http': http streaming
	...
	/*
	 * Youtube offers two different player APIs: fLaSh and "iFrame" for HTML5 .
	 * Make your choice here: For mobile devices this is forced to TRUE
	 */
	useYTIframeAPI : true,
	...
	/*
	 * small: Player height is 240px, and player dimensions are at least 320px
	 * by 240px for 4:3 aspect ratio. medium: Player height is 360px, and player
	 * dimensions are 640px by 360px (for 16:9 aspect ratio) or 480px by 360px
	 * (for 4:3 aspect ratio). large: Player height is 480px, and player
	 * dimensions are 853px by 480px (for 16:9 aspect ratio) or 640px by 480px
	 * (for 4:3 aspect ratio). hd720: Player height is 720px, and player
	 * dimensions are 1280px by 720px (for 16:9 aspect ratio) or 960px by 720px
	 * (for 4:3 aspect ratio). hd1080: Player height is 1080px, and player
	 * dimensions are 1920px by 1080px (for 16:9 aspect ratio) or 1440px by
	 * 1080px (for 4:3 aspect ratio). highres: Player height is greater than
	 * 1080px, which means that the player's aspect ratio is greater than 1920px
	 * by 1080px.
	 */
	...
	/*
	 * if testcard is disabled, the player will force a filedowload in case no
	 * native- or flashplayer is available. oterhwise (enableTestcard=true) a
	 * testcard with an errormessage is shown in case of issues
	 */
	enableTestcard : true,

	/*
	 * if the scheduled playlist holds more than one item an "skipTestcard" is
	 * set to TRUE in case of an error the player will proceed to the next item
	 * without showing a testcard
	 */
	skipTestcard : false,

	/*
	 * sets the duration for media items without a duration (images & html
	 * pages)
	 */
	duration : 0,
	...
	$p.utils = {
		...
		roundNumber : function(rnum, rlength) {
			if (rnum <= 0 || isNaN(rnum))
				return 0;
			return Math.round(rnum * Math.pow(10, rlength))
					/ Math.pow(10, rlength);
		},
		 ...
		 * @public
		 * @param (String)
		 *            Da human readable time to parse
		...
		toTimeString : function(secs, noSecs) {
			var hours = Math.floor(secs / (60 * 60)), divisor_for_minutes = secs
					% (60 * 60), minutes = Math.floor(divisor_for_minutes / 60), divisor_for_seconds = divisor_for_minutes % 60, seconds = Math
					.floor(divisor_for_seconds);
			...
			return (noSecs === true) ? hours + ':' + minutes : hours + ':'
					+ minutes + ':' + seconds;
		},
		...
		embeddFlash : function(destObj, domOptions, shield, shrinkShield) {
			...
			var flashVars = domOptions.FlashVars || {}, result = '', htmlEmbedObj = '', htmlEmbed = '', tmpStr = '', dest = destObj, id = '';
			...
			for ( var key in flashVars) {
				if (typeof flashVars[key] != 'function') {
					...
					/*
					 * // support "{tags}" to add media properties for(var i in
					 * this.media) { if (typeof tmpStr != 'string') continue;
			...
			htmlEmbedObj = '<object id="'
					+ domOptions.id
					+ '" codebase="https://fpdownload.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=9,0,0,0"  name="'
					+ domOptions.name + '" width="' + domOptions.width
					+ '" height="' + domOptions.height
					+ '" classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000">'
					+ '<param name="movie" value="' + domOptions.src
					+ '"></param>' + '<param name="allowScriptAccess" value="'
					+ domOptions.allowScriptAccess + '"></param>'
					+ '<param name="allowFullScreen" value="'
					+ domOptions.allowFullScreen + '"></param>'
					+ '<param name="wmode" value="' + domOptions.wmode
					+ '"></param>';
			...
			if (shield !== false) {
				dest
						.append($('<div/>').attr('id', domOptions.id + '_cc')
								.css(
										{
											width : (shrinkShield) ? '1px'
													: '100%',
											height : (shrinkShield) ? '1px'
													: '100%',
											backgroundColor : ($p.utils
													.ieVersion() < 9) ? '#000'
													: 'transparent',
		 ...
		 *            Da string to get processed
		 * @param (Object)
		 *            Object holding data to fill in
		...
		parseTemplate : function(template, data, encode) {
			...
			for ( var i in data) {
				template = template.replace(new RegExp('%{' + i + '}', 'gi'),
						((encode === true) ? window.encodeURIComponent(data[i])
								: data[i]))
			}
		...
		/**
		 * stretches target to fit into specified dimensions keeping apsect
		 * ratio
		 * 
		 * @public
		 * @param (String)
		 *            "fill" or "aspectratio" (default)
		 * @param (Object)
		 *            the Dom-Obj to scale
		 * @param (Float)
		 *            The maximum available width in px
		 * @param (Float)
		 *            The maximum available height in px
		 * @param (Float)
		 *            A forced asumed with of the target object (optional)
		 * @param (Float)
		 *            A forced asumed height of the target object (optional)
		 * @return (Boolean) Returns TRUE if <target> was resized in any way,
		 *         otherwise FALSE
		 */
		stretch : function(stretchStyle, target, wid, hei, twf, thf) {
			...
			var tw = (twf !== undefined) ? twf : target.data('od').width, th = (thf !== undefined) ? thf
					: target.data('od').height, xsc = (wid / tw), ysc = (hei / th), rw = wid, rh = hei;
			...
			if (target.data('od').width != target.width()
					|| target.data('od').height != target.height()) {
				return true;
			}
		...
		log : function() {
			...
			this.history = this.history || []; // store logs to an array for
												// reference
		...
		cssTransitions : function() {
			var m = document.createElement('z'), s = m.style;
			...
			function test_props_all(prop) {
				var d = 'Webkit Moz O ms Khtml'.split(' '), u = prop.charAt(0)
						.toUpperCase()
						+ prop.substr(1), e = (prop + ' ' + d.join(u + ' ') + u)
						.split(' ');
				return test_props(e);
			}
		...
		versionCompare : function(installed, required) {
			var a = installed.split('.'), b = required.split('.'), i = 0;
		...
		/**
		 * serializes a simple object to a JSON formatted string. Note:
		 * stringify() is different from jQuery.serialize() which URLEncodes
		 * form elements CREDITS:
...
jQuery(function($) {
	...
	$p.platforms = {
		...
		/*
		 * returns the version of the flash player installed on client. returns
		 * 0 on none.
		 */
		FLASH : function(typ) {
			try {
		...
		IOS : function(type) {
			var agent = navigator.userAgent.toLowerCase(), start = agent
					.indexOf('os ');
		...
		NATIVE : function(type) {
			try {
				var testObject = $(
						(type.indexOf('video') > -1) ? '<video/>' : '<audio/>')
						.get(0);
	 ...
	 * this file is part of: projekktor zwei http://www.projekktor.com
	 * 
	 * Copyright 2010-2013 Sascha Kluger, Spinning Airwhale Media,
	 * http://www.spinningairwhale.com under GNU General Public License
	 * http://www.filenew.org/projekktor/license/
	 */
...
jQuery(function($) {
	projekktorPluginInterface.prototype = {
		...
		_init : function(pluginConfig) {
			...
			if (this.reqVer != null) {
				if (!$p.utils.versionCompare(this.pp.getPlayerVer(),
						this.reqVer)) {
					alert("Plugin '"
							+ this.name
							+ "' requires Projekktor v"
							+ this.reqVer
							+ " or later! Please visit http://www.projekktor.com and get the most recent version.");
		...
		applyToPlayer : function(element, fu, visible) {
			...
			var func = fu || 'container', tmpClass = '', ref = this;
			...
			this._pageDOMContainer[func] = $("[" + this.getDA('host') + "='"
					+ this.pp.getId() + "'][" + this.getDA('func') + "='"
					+ func + "']");
			this._childDOMContainer[func] = this.playerDom.find("["
					+ this.getDA('func') + "='" + func + "'],."
					+ this.getCN(tmpClass) + ":not([" + this.getDA('func')
					+ "=''])");

			// check if this element aleady exists somewhere on page
			if (this._pageDOMContainer[func].length > 0) {
				this._pageDOMContainer[func].removeClass('active').addClass(
						'inactive');
			...
			if (this._childDOMContainer[func].length == 0) {
				element.removeClass(tmpClass).addClass(
						this.pp.getNS() + tmpClass).removeClass('active')
						.addClass('inactive').attr(this.getDA('func'), func)
			...
			if (visible === true) {
				this._childDOMContainer[func].addClass('active').removeClass(
						'inactive');
			}
		...
		setInactive : function() {
			$(this._pageDOMContainer['container']).removeClass('active')
					.addClass('inactive');
			$(this._childDOMContainer['container']).removeClass('active')
					.addClass('inactive');
			this.sendEvent('inactive', $.extend(true, {},
					this._pageDOMContainer['container'],
					this._childDOMContainer['container']));
		},
		...
		setActive : function(elm, on) {
			...
			if (elm == null) {
				this._pageDOMContainer['container'].removeClass('inactive')
						.addClass('active');
				this._childDOMContainer['container'].removeClass('inactive')
						.addClass('active');
				this.sendEvent('active', $.extend(true, {},
						this._pageDOMContainer['container'],
						this._childDOMContainer['container']));
				return dest;
			}
		...
		clickHandler : function(what) {
			try {
				this.pp[this.getConfig(what + 'Click').callback](this
						.getConfig(what + 'Click').value);
			} catch (e) {
				try {
					this.getConfig(what + 'Click')(
							this.getConfig(what + 'Click').value);
				} catch (e) {
				}
			}
			return false;
		...
		cookie : function(key, value, ttl) {
			...
			var t = new Date(), result = null, cookieString = '', tmp = storedData = jQuery
					.parseJSON(eval(result = new RegExp('(?:^|; )'
							+ encodeURIComponent(this.getConfig('cookieName')
									+ "_" + this.name) + '=([^;]*)')
							.exec(document.cookie)) ? decodeURIComponent(result[1])
							: null);
			...
			cookieString = encodeURIComponent(this.getConfig('cookieName',
					'projekktor')
					+ "_" + this.name)
					+ '='
					+ encodeURIComponent(storedData)
					+ '; expires='
					+ ((ttl == false) ? "Thu, 01 Jan 1970 00:00:01 GMT" : t
							.toUTCString())
	 ...
	 * this file is part of: projekktor zwei http://www.projekktor.com
	 * 
	 * Copyright 2010, 2011, Sascha Kluger, Spinning Airwhale Media,
	 * http://www.spinningairwhale.com under GNU General Public License
	 * http://www.filenew.org/projekktor/license/
	 */
	...
	playerModel.prototype = {
		...
		displayItem : function(showMedia) {
			...
			if (showMedia !== true || this.getState('STOPPED')) {
				this._setState('idle');
				this.applyImage(this.getPoster(), this.pp.getMediaContainer()
						.html(''));
				this._isPoster = true;
				this.displayReady();
				return;
		...
		reInit : function() {
			// no FF:
			if (this.flashVersion !== false || !this._isFF()
					|| this.getState('ERROR')
					|| this.pp.getConfig('bypassFlashFFFix') === true) {
				return;
			}
		...
		getVolume : function() {
			...
			return (this.mediaElement.prop('muted') === true) ? 0
					: this.mediaElement.prop('volume');
		},
		...
		getPlaybackQuality : function() {
			return ($.inArray(this._quality, this.media.qualities) > -1) ? this._quality
					: 'default';
		},
		...
		getState : function(isThis) {
			var result = (this._currentState == null) ? 'IDLE'
					: this._currentState;
			if (isThis != null)
		...
		getBufferState : function(isThis) {
			var result = (this._currentBufferState == null) ? 'NONE'
					: this._currentBufferState;
			if (isThis != null)
				return (result == isThis.toUpperCase());
			return result;
		},
		...
		getSeekState : function(isThis) {
			var result = (this._currentSeekState == null) ? 'NONE'
					: this._currentSeekState;
			if (isThis != null)
				return (result == isThis.toUpperCase());
			return result;
		},
		...
		getPoster : function() {
			var type = 'poster', result = null, cfg = this.pp.getConfig(type), qual = 'default', quals = [];
		...
		getSource : function() {
			...
			var resultSrc = [], offset = this.media.offset
					|| this.media.position || false, ref = this, pseudoQuery = (this.pp
					.getConfig('streamType') == 'pseudo') ? this.pp
					.getConfig('startParameter') : false;
			...
			$.each(this.media.file || [], function() {
				...
				var u = $p.utils.parseUri(this.src), src = u.protocol + '://'
						+ u.host + u.path, query = [];
				...
				src += (query.length > 0) ? '?' + query.join('&') + "&"
						+ pseudoQuery + "=" + offset : "?" + pseudoQuery + "="
						+ offset;
				this.src = src;
		...
		timeListener : function(obj) {
			...
			var position = parseFloat((obj.position || obj.currentTime
					|| this.media.position || 0).toFixed(2)), duration = parseFloat((obj.duration || 0)
					.toFixed(2));
			...
			if (duration != 0
					&& (duration != this.media.duration && !this.isPseudoStream)
					|| (this.isPseudoStream && this.media.duration == 0)) {

				this.media.duration = duration;
				this.sendUpdate('durationChange', duration);
			}

			// remember values & concider pseudo stream position offset, bypass
			// some strange position hopping effects during pseudostream:
			if (position == this.media.position)
				return;

			if (this.isPseudoStream
					&& Math.round(position * 100) / 100 == Math
							.round(this.media.offset * 100) / 100) {
				this.media.position = this.media.offset;
			} else {
				this.media.position = this.media.offset + position;
			}

			this.media.maxpos = Math.max(this.media.maxpos || 0,
					this.media.position || 0);
			this.media.playProgress = parseFloat((this.media.position > 0 && this.media.duration > 0) ? this.media.position
					* 100 / this.media.duration
					: 0);
		...
		loadProgressUpdate : function() {
			...
			var me = this.mediaElement.get(0), progress = 0;
			...
			if (me.seekable && me.seekable.length > 0) {
				progress = Math.round(me.seekable.end(0) * 100
						/ this.media.duration);
			} else {
				progress = Math
						.round(me.buffered.end(me.buffered.length - 1) * 100)
						/ this.media.duration;
			}
			...
			this.media.loadProgress = (this.allowRandomSeek === true) ? 100
					: -1;
			this.media.loadProgress = (this.media.loadProgress < 100 || this.media.loadProgress === undefined) ? progress
					: 100;
		...
		progressListener : function(obj, evt) {
			...
			// we prefer timeranges but keep catching "progress" events by
			// default
			// for historical and compatibility reasons:
			if (this.mediaElement instanceof jQuery) { // fix this - make sure
														// all instances are
														// jquery objects
			...
			try {
				if (!isNaN(evt.loaded / evt.total)) {
					...
					total = evt.total;
						...
						&& !isNaN(evt.originalEvent.loaded
								/ evt.originalEvent.total)) {
			...
			var loadedPercent = (current > 0 && total > 0) ? current * 100
					/ total : 0;
			...
			loadedPercent = (this.media.loadProgress !== 100) ? loadedPercent
					: 100;
			loadedPercent = (this.allowRandomSeek === true) ? 100 : 5 * Math
					.round(loadedPercent / 5);
		...
		setTestcard : function(no, txt) {
			var destContainer = this.pp.getMediaContainer().html('').css({
				width : '100%',
				height : '100%'
			}), messages = $.extend(this.pp.getConfig('messages'), this.pp
					.getConfig('msg')), code = (messages[no] == null) ? 0 : no, msgTxt = (txt !== undefined && txt !== '') ? txt
					: messages[code];
			...
			this.mediaElement = $('<div/>').addClass(
					this.pp.getNS() + 'testcard').attr('id',
					this.pp.getId() + '_testcard_media').html(
					'<p>' + msgTxt + '</p>').appendTo(destContainer);
		...
		applyImage : function(url, destObj) {
			...
			var imageObj = $('<img/>').hide(), ref = this;
			...
			imageObj.load(function(event) {
				...
				imageObj.show();
				$p.utils.stretch(ref.pp.getConfig('imageScaling'), imageObj,
						destObj.width(), destObj.height());
			});
			...
			var onReFull = function(imgObj, destObj) {
				...
				var tw = destObj.width(), th = destObj.height(), wid = imgObj
						.width(), hei = imgObj.height();

				if ($p.utils.stretch(ref.pp.getConfig('imageScaling'), imgObj,
						destObj.width(), destObj.height())) {
					try {
		...
		createFlash : function(domOptions, destObj, shield) {
			this.mediaElement = $p.utils.embeddFlash(destObj.html(''),
					domOptions, shield, true);
			this._waitforPlayer();
		},
		...
		_waitforPlayer : function() {
			...
			(function() {
				...
				if (counter > 6 && ref._isFF()) {
					counter = 0;
					var dest = $(ref.mediaElement).parent(), clone = $(
		...
		_setState : function(state) {
			...
			if (this._currentState != state && this._currentState != 'ERROR') {
				...
				if ((this._currentState == 'IDLE' || this._currentState == 'STARTING')
						&& state == 'PLAYING') {
		...
		_scaleVideo : function(promote) {
			...
			if (this.pp.getIsMobileClient())
				return;
			try {
				var wid = destContainer.width(), hei = destContainer.height(), tw = this.media.videoWidth, th = this.media.videoHeight;

				if ($p.utils.stretch(this.pp.getConfig('videoScaling'),
						this.mediaElement, wid, hei, tw, th)) {
					this.sendUpdate('scaled', {
						realWidth : tw,
						realHeight : th,
						displayWidth : wid,
...
jQuery(function($) {
			...
			.newModel({
				...
				applyMedia : function(destContainer) {
					...
					if (this.pp.getConfig('enableTestcard')
							&& !this.pp.getIsMobileClient()) {
						this.pp.addListener('mousedown', mouseClick);
						this._setState('error');
						this
								.setTestcard((this.media.file[0].src != null && this.media.errorCode === 7) ? 5
										: this.media.errorCode);
					} else {
						// this.applyImage(this.media.config.poster,
						// destContainer);
	 ...
	 * this file is part of: projekktor zwei http://www.projekktor.com
	 * 
	 * Copyright 2010, Sascha Kluger, Spinning Airwhale Media,
	 * http://www.spinningairwhale.com under GNU General Public License
	 * http://www.filenew.org/projekktor/license/
	 */
jQuery(function($) {
	$p
			.newModel({
				...
				init : function() {
					var ua = navigator.userAgent; // TODO: global platform and
													// feature detection
				...
				applyMedia : function(destContainer) {
					...
					if ($('#' + this.pp.getMediaId() + "_html").length === 0) {
						...
						destContainer.html('').append($('<video/>').attr({
				...
				applySrc : function() {
					var ref = this, media = this.getSource(), wasAwakening = ref
							.getState('AWAKENING');

					/*
					 * Using 'src' attribute directly in <video> element is
					 * safer than using it inside <source> elements. Some of the
					 * mobile browsers (e.g. Samsung Galaxy S2, S3 Android
					 * native browsers <= 4.2.2) will not initialize video
					 * playback with <source> elements at all, displaying only
					 * gray screen instead. HLS stream on iOS and Android will
					 * not work if its URL is defined through <source> 'src'
					 * attribute instead of <video> 'src' attribute.
					 */
					...
					/*
					 * Some of the mobile browsers (e.g. Android native browsers <=
					 * 4.2.x, Opera Mobile) have by default play/pause actions
					 * bound directly to click/mousedown events of <video>. That
					 * causes conflict with display plugin play/pause actions,
					 * which makes it impossible to pause the currently playing
					 * video. Precisely _setState is called twice: first by
					 * pauseListener triggered by <video> default
					 * click/mousedown action, secondly by display plugin
					 * actions bound to mousedown events. The result is that the
					 * video is paused by native <video> events and then
					 * immediately started by display plugin that uses the
					 * setPlayPause function. setPlayPause function toggles
					 * between "PAUSED" and "PLAYING" states, so when a video is
					 * being played, the function causes its pausing.
					 */
					this.mediaElement.bind('mousedown.projekktorqs'
							+ this.pp.getId(),
							this.disableDefaultVideoElementActions);
					this.mediaElement.bind('click.projekktorqs'
							+ this.pp.getId(),
							this.disableDefaultVideoElementActions);

					var func = function() {

						ref.mediaElement.unbind('loadstart.projekktorqs'
								+ ref.pp.getId());
						ref.mediaElement.unbind('loadeddata.projekktorqs'
								+ ref.pp.getId());
						ref.mediaElement.unbind('canplay.projekktorqs'
								+ ref.pp.getId());
						...
						ref.mediaElement = $('#' + ref.pp.getMediaId()
								+ "_html");
					...
					this.mediaElement.bind('loadstart.projekktorqs'
							+ this.pp.getId(), func);
					this.mediaElement.bind('loadeddata.projekktorqs'
							+ this.pp.getId(), func);
					this.mediaElement.bind('canplay.projekktorqs'
							+ this.pp.getId(), func);

					this.mediaElement[0].load(); // important especially for
													// iOS devices
				...
				detachMedia : function() {
					try {
						...
						this.mediaElement.unbind('mousedown.projekktorqs'
								+ this.pp.getId());
						this.mediaElement.unbind('click.projekktorqs'
								+ this.pp.getId());
				...
				addListeners : function(evtId, subId) {
					if (this.mediaElement == null)
						return;
					var id = (subId != null) ? '.projekktor' + subId
							+ this.pp.getId() : '.projekktor' + this.pp.getId(), ref = this, evt = (evtId == null) ? '*'
							: evtId;
				...
				removeListener : function(evt, subId) {
					...
					var id = (subId != null) ? '.projekktor' + subId
							+ this.pp.getId() : '.projekktor' + this.pp.getId(), ref = this;
				...
				_ended : function() {
					var dur = this.mediaElement[0].duration, // strange
																// android
																// behavior
																// workaround
					complete = (Math.round(this.media.position) === Math
							.round(dur)), fixedEnd = ((dur - this.media.maxpos) < 2)
							&& (this.media.position === 0) || false;
						...
						(function() {
							try {
								if (ref.getDuration() === 0) {
									if (ref.mediaElement.get(0).currentSrc !== ''
											&& ref.mediaElement.get(0).networkState == ref.mediaElement
													.get(0).NETWORK_NO_SOURCE) {
					...
					if (this.pp.getConfig('streamType') == 'pseudo') {
						$
								.each(
										...
										function() {
											if (this.src
													.indexOf(ref.mediaElement[0].currentSrc) > -1) {
	...
	$p.newModel({
		...
		applyMedia : function(destContainer) {
			...
			// create cover image
			this.imageElement = this.applyImage(this.getPoster('cover')
					|| this.getPoster('poster'), destContainer);
			...
			if ($('#' + this.pp.getMediaId() + "_html").length === 0) {
				...
				destContainer.html('').append(
						$((this.isGingerbread) ? '<video/>' : '<audio/>').attr(
								{
									"id" : this.pp.getMediaId() + "_html",
									"poster" : $p.utils.imageDummy(),
									"loop" : false,
		...
		setPosterLive : function() {
			if (this.imageElement.parent) {
				var dest = this.imageElement.parent(), ref = this;

				if (this.imageElement.attr('src') == this.getPoster('cover')
						|| this.getPoster('poster'))
					return;

				this.imageElement.fadeOut('fast', function() {
					$(this).remove();
					ref.imageElement = ref.applyImage(ref.getPoster('cover')
							|| ref.getPoster('poster'), dest);
				});
			}
		}
	 ...
	 * this file is part of: projekktor zwei http://www.projekktor.com
	 * 
	 * Copyright 2014, Sascha Kluger, Spinning Airwhale Media,
	 * http://www.spinningairwhale.com under GNU General Public License
	 * http://www.filenew.org/projekktor/license/
	 */
jQuery(function($) {
...
jQuery(function($) {
	$p.newModel({
					...
					if (navigator.plugins[i].name.indexOf("VLC") > -1) {
						for ( var j = 0; j < navigator.plugins[i].length; j++) {
							...
							if (ref.suffixes != null && ref.type != null) {
								$.each(ref.suffixes.split(','), function(key,
										value) {
									model.iLove.push({
										ext : value,
		...
		applyMedia : function(destContainer) {
			destContainer.html('').append($('<embed/>').attr({
			...
			/*
			 * destContainer.append( $('<div/>').attr('id',
			 * this.pp.getMediaId()+"_vlc_cc" ) .css({ width: '100%', height:
			 * '100%', backgroundColor: 'rgba(255,0,0,0.1)', opacity: '0.1',
			 * filter: 'alpha(opacity = 0.1)', position: 'absolute', top: 0,
			 * left: 0 }) )
			 */
		...
		applySrc : function() {
			...
			var ref = this, sources = this.getSource();
			...
			$.each(this._eventMap, function(event, value) {
				try {
					if (ref.mediaElement.get(0).attachEvent) {
						// Microsoft
						ref.mediaElement.get(0).attachEvent(event,
								function(evt) {
									ref[value](this, evt);
								});
					} else if (ref.mediaElement.get(0).addEventListener) {
						// Mozilla: DOM level 2
						ref.mediaElement.get(0).addEventListener(event,
								function(evt) {
									ref[value](this, evt);
								}, false);
					} else {
		...
		removeListener : function(evt, subId) {
			...
			var id = (subId != null) ? '.projekktor' + subId + this.pp.getId()
					: '.projekktor' + this.pp.getId(), ref = this;
		...
		setSeek : function(newpos) {
			this.mediaElement.get(0).input.position = newpos
					/ this.media.duration;
	 ...
	 * this file is part of: projekktor zwei http://filenew.org/projekktor/
	 * 
	 * Copyright 2010, 2011, Sascha Kluger, Spinning Airwhale Media,
	 * http://www.spinningairwhale.com under GNU General Public License
	 * http://www.filenew.org/projekktor/license/
...
jQuery(function($) {
	$p.newModel({
		...
		applyMedia : function(destContainer) {
			this.mediaElement = this.applyImage(this.media.file[0].src,
					destContainer.html(''));
			...
			.newModel({
				...
				applyMedia : function(destContainer) {
					...
					window['projekktorOSMFReady' + this.pp.getId()] = function() {
						projekktor(ref.pp.getId()).playerModel
								._OSMFListener(arguments);
					};
					...
					var domOptions = {
						...
						menu : false,
						allowFullScreen : 'true',
						wmode : ($p.utils.ieVersion()) ? 'transparent'
								: 'opaque', // must be either transparent (ie)
											// or opaque in order to allow HTML
											// overlays
				...
				applySrc : function() {
					...
					this.mediaElement.get(0)
							.setMediaResourceURL(sources[0].src);

					this.streamType = sources[0].streamType
							|| this.pp.getConfig('streamType') || 'http';
					if (this.getState('PLAYING')) {
						this.setPlay();
						if (ref.isPseudoStream !== true
								&& this.media.position > 0) {
				...
				_OSMFListener : function() {
					var event = arguments[0][1], value = arguments[0][2], ref = this;

					this.mediaElement = $('#' + this.pp.getMediaId() + "_flash"); // IE
																					// 10
																					// sucks
					...
					case 'onJavaScriptBridgeCreated':
						if (this.mediaElement !== null
								&& this.getState('AWAKENING')) {
							$.each(this._eventMap, function(key, value) {
								ref.mediaElement.get(0).addEventListener(
										key,
										"projekktor('" + ref.pp.getId()
												+ "').playerModel." + value);
							});
				...
				OSMF_currentTimeChange : function(value) {
					if (this._isDVR) {
						this.sendUpdate('isLive',
								(value + 20 >= this.media.duration)); // 20 =>
																		// default
																		// dvr
																		// buffer
																		// of
				...
				OSMF_loadStateChange : function(state) {
					...
					case 'ready':
						...
						if (this.mediaElement.get(0).getStreamType().indexOf(
								'dvr') > -1) {
							this.allowRandomSeek = true;
							this.media.loadProgress = 100;
						}
						break;
					case 'loadError':
						// causes false positive in case of dynamically loaded
						// plugins
				...
				/*
				 * catching playStateChange and playerStateChange and
				 * playerStateChange aaaand... and playerStateChange
				 */
				OSMF_playerStateChange : function(state) {
					...
					if (!this._isDVR
							&& this.mediaElement.get(0).getStreamType() == 'dvr') {
						this._isDVR = true;
				...
				OSMF_updateDynamicStream : function() {
					var dynamicStreams = this.mediaElement.get(0)
							.getStreamItems(), name = '', result = [];
					// switchMode =
					// this.mediaElement.get(0).getAutoDynamicStreamSwitch() ?
					// "Auto" : "Manual";

					for ( var index in dynamicStreams) {
						if (dynamicStreams.hasOwnProperty(index)
								&& dynamicStreams[index].bitrate !== undefined) {
							name = dynamicStreams[index].width + "x"
									+ dynamicStreams[index].height;
							if (this.pp.getConfig('OSMFQualityMap')
									&& this.pp.getConfig('OSMFQualityMap')[name]) {
								this.availableQualities[this.pp
										.getConfig('OSMFQualityMap')[name]] = index;
							}
					...
					this._isDynamicStream = true; // important: set this
				...
				switchDynamicStreamIndex : function(index) {
					if (index == -1) {
						this.mediaElement.get(0).setAutoDynamicStreamSwitch(
								true);
					} else {
						if (this.mediaElement.get(0)
								.getAutoDynamicStreamSwitch()) {
							this.mediaElement.get(0)
									.setAutoDynamicStreamSwitch(false);
						}
						this.mediaElement.get(0)
								.switchDynamicStreamIndex(index);
					}
				},

				errorListener : function() {
					/*
					 * todo OSMF MediaErrorCodes mapping
					 * http://help.adobe.com/en_US/FlashPlatform/reference/actionscript/3/org/osmf/events/MediaErrorCodes.html
					 */
					switch (arguments[0]) {
					case 15:
						this.sendUpdate('error', 5);
				...
				setQuality : function(quality) {
					...
					if (this._isDynamicStream === true) {
						this.switchDynamicStreamIndex((quality == 'auto') ? -1
			...
			.newModel(
					{
						...
						applyMedia : function(destContainer) {
							...
							this.imageElement = this.applyImage(this
									.getPoster('cover')
									|| this.getPoster('poster'), destContainer);

							var flashContainer = $('#' + this.pp.getMediaId()
									+ '_flash_container');
...
jQuery(function($) {
	$p
			.newModel({
				...
				init : function(params) {
					...
					this.useIframeAPI = this.pp.getConfig('useYTIframeAPI')
							|| this.pp.getIsMobileClient();
				...
				applyMedia : function(destContainer) {
					...
					var ref = this, width = (this.modelId == 'YTAUDIO') ? 1
							: '100%', height = (this.modelId == 'YTAUDIO') ? 1
							: '100%';

					if (this.modelId == 'YTAUDIO')
						this.imageElement = this.applyImage(
								this.pp.getPoster(), destContainer);

					if (this.useIframeAPI) {

						destContainer.html('').append(
								$('<div/>').attr('id',
										this.pp.getId() + '_media_youtube')
										.css({
											width : '100%',
											height : '100%',
											position : 'absolute',
						...
						var shield = $('<div/>').attr('id',
								this.pp.getId() + '_media_youtube_cc')
								.css(
										{
											width : '100%',
											height : '100%',
						...
						this.mediaElement = new YT.Player(
								this.pp.getId() + '_media_youtube',
								{
									width : (this.pp.getIsMobileClient()) ? this.pp.config._width
											: width,
									height : (this.pp.getIsMobileClient()) ? this.pp.config._height
											: height,
									playerVars : {
										autoplay : 0,
										disablekb : 0,
										version : 3,
										start : 0,
										controls : (this.pp.getIsMobileClient()) ? 1
												: 0,
										showinfo : 0,
										enablejsapi : 1,
				...
				flashReadyListener : function() {
					...
					this.mediaElement.cueVideoById(this.youtubeGetId(),
							this.media.position || 0, this._playbackQuality);
				},

				/* OLD API - workaround for youtube video resize bug: */
				_youtubeResizeFix : function() {
					/*
					 * $(this.mediaElement).attr({ width: '99.99999%', height:
					 * '99.9999%' });
					 */
					this.applyCommand('volume', this.pp.getConfig('volume'));
				},
				...
				addListeners : function() {
					...
					this.mediaElement.addEventListener("onStateChange",
							"projekktor('" + this.pp.getId()
									+ "').playerModel.stateChange");
					this.mediaElement.addEventListener("onError",
							"projekktor('" + this.pp.getId()
									+ "').playerModel.errorListener");
					this.mediaElement.addEventListener(
							"onPlaybackQualityChange", "projekktor('"
									+ this.pp.getId()
									+ "').playerModel.qualityChangeListener");
				},
				...
				getPoster : function() {
					return this.media['config']['poster']
							|| this.pp.config.poster
							|| 'http://img.youtube.com/vi/'
							+ this.youtubeGetId() + '/0.jpg';
				},
				...
				stateChange : function(eventCode) {
					...
					if (this.mediaElement === null
						...
						return;
					switch ((eventCode.data == undefined) ? eventCode
							: eventCode.data) {
					case -1:
						this.setPlay();
						this.ffFix = true;
					...
					case 1:
						...
						if ((this.media.position || 0) > 0 && this._isFF()
								&& this.ffFix) {
							this.ffFix = false;
							this.setSeek(this.media.position);
						}
				...
				onReady : function() {
					...
					$('#' + this.pp.getId() + '_media').attr(
							'ALLOWTRANSPARENCY', true).attr({
						scrolling : 'no',
						frameborder : 0
					}).css({
						'overflow' : 'hidden',
					...
					$.ajax({
						url : 'http://gdata.youtube.com/feeds/api/videos/'
								+ this.youtubeGetId() + '?v=2&alt=jsonc',
						async : false,
						complete : function(xhr, status) {
				...
				youtubeGetId : function() {
					return encodeURIComponent(this.media.file[0].src.replace(
							/^[^v]+v.(.{11}).*/, "$1"));
				},
				...
				updateInfo : function() {
					...
					(function() {
						...
						try {
							if (ref.getState('PLAYING')) {
								ref.timeListener({
									position : ref.mediaElement
											.getCurrentTime(),
									duration : ref.mediaElement.getDuration()
								});
								ref.progressListener({
									loaded : ref.mediaElement
											.getVideoBytesLoaded(),
									total : ref.mediaElement
											.getVideoBytesTotal()
								});
								ref._updateTimer = setTimeout(arguments.callee,
										500);
							}
						} catch (e) {
						}
	 ...
	 * features startbutton, logo-overlay and buffering indicator Copyright
	 * 2010-2013, Sascha Kluger, Spinning Airwhale Media,
	 * http://www.spinningairwhale.com
	 * 
	 * under GNU General Public License http://www.projekktor.com/license/
	 */
...
jQuery(function($) {
	projekktorDisplay.prototype = {
		...
		config : {
			...
			/*
			 * time to delay buffering-icon-overlay once "waiting" event has
			 * been triggered
			 */
			bufferIconDelay : 1000,

			/*
			 * if set the indicator animation is tinkered from a cssprite - must
			 * be horizontal
			 */
			spriteUrl : '',
			spriteWidth : 50,
		...
		initialize : function() {
			...
			this.startButton = this.applyToPlayer(
					$('<div/>').addClass('start'), 'startbtn');

			// create buffericon
			this.buffIcn = this.applyToPlayer(
					$('<div/>').addClass('buffering'), 'buffericn');

			this.imaContainer = this.applyToPlayer($('<div/>').addClass('ima'),
					'ima');
						...
						.css(
								{
									...
									marginLeft : ((this.buffIcn.width() - this.config.spriteWidth) / 2)
											+ "px",
									marginTop : ((this.buffIcn.height() - this.config.spriteHeight) / 2)
											+ "px",
		...
		plugineventHandler : function(data) {
			if (data.PLUGIN == 'controlbar' && data.EVENT == 'show'
					&& this.getConfig('staticControls')) {
		...
		showBufferIcon : function(instant) {
			...
			var startOffset = (ref.config.spriteCountUp === true) ? 0
					: (ref.config.spriteHeight + ref.config.spriteOffset)
							* (ref.config.spriteTiles - 1), spriteOffset = startOffset;
			ref.buffIcnSprite.addClass('active').removeClass('inactive');
			(function() {

				if (!ref.buffIcn.is(':visible'))
					return;
				ref.buffIcnSprite.css('backgroundPosition', '0px -'
						+ spriteOffset + "px");

				if (ref.config.spriteCountUp === true)
					spriteOffset += ref.config.spriteHeight
							+ ref.config.spriteOffset;
				else
					spriteOffset -= ref.config.spriteHeight
							+ ref.config.spriteOffset;

				if (spriteOffset > (startOffset + ref.config.spriteHeight)
						* ref.config.spriteTiles
						|| spriteOffset < ref.config.spriteOffset)
	 ...
	 * 
	 * DESC: Adds a fully features cb element to the player Copyright 2010-2013
	 * Sascha Kluger, Spinning Airwhale Media, http://www.spinningairwhale.com
	 * 
	 * under GNU General Public License http://www.projekktor.com/license/
	 */
...
jQuery(function($) {
	projekktorControlbar.prototype = {
		...
		controlElementsConfig : {
			...
			'loaded' : null, // { on:['touchstart', 'click'],
								// call:'scrubberClk'},
			'golive' : [ {
				on : [ 'touchstart', 'click' ],
				call : 'goliveClk'
			...
			'scrubber' : null, // { on:['touchstart', 'click'],
								// call:'scrubberClk'},
			'scrubbertip' : null,
			'scrubberknob' : null,
			'scrubberdrag' : [ {
...
//			controlsTemplate : '<ul class="left"><li><div %{play}></div><div %{pause}></div></li></ul><ul class="right"><li><div %{fsexit}></div><div %{fsenter}></div></li><li><div %{loquality}></div><div %{hiquality}></div></li><li><div %{tracksbtn}></div></li><li><div %{vmax}></div></li><li><div %{vslider}><div %{vmarker}></div><div %{vknob}></div></div></li><li><div %{mute}></div></li><li><div %{timeleft}>%{hr_elp}:%{min_elp}:%{sec_elp} | %{hr_dur}:%{min_dur}:%{sec_dur}</div></li><li><div %{next}></div></li><li><div %{prev}></div></li></ul><ul class="bottom"><li><div %{scrubber}><div %{loaded}></div><div %{playhead}></div><div %{scrubberknob}></div><div %{scrubberdrag}></div></div></li></ul><div %{scrubbertip}>%{hr_tip}:%{min_tip}:%{sec_tip}</div>'
			controlsTemplate : '<ul class="bottom"><li><div %{scrubber}><div %{loaded}></div><div %{playhead}></div><div %{scrubberknob}></div><div %{scrubberdrag}></div></div></li></ul>'
					+ '<ul class="left"><li><div %{play}></div><div %{pause}></div></li><li><div %{mute}></div></li><li id="volume"><div %{vslider}><div %{vmarker}></div><div %{vknob}></div></div></li><li><div %{timeleft}>%{hr_elp}:%{min_elp}:%{sec_elp} / %{hr_dur}:%{min_dur}:%{sec_dur}</div></li></ul>'
					+ '<ul class="right"><li><div %{fsexit}></div><div %{fsenter}></div></li><li><div %{quality}></div></li><li><div %{tracksbtn}></div></li><li><div %{next}></div></li><li><div %{prev}></div></li></ul>'
					+ '<div %{scrubbertip}>%{hr_tip}:%{min_tip}:%{sec_tip}</div>'
					+ '<div id="quality_options"><div %{qualityfullhd}></div><div %{qualityhd}></div><div %{qualitylarge}></div><div %{qualitymedium}></div><div %{qualitysmall}></div></div>'
		},

		initialize : function() {

			var ref = this, playerHtml = this.playerDom.html(), useTemplate = true, classPrefix = this.pp
					.getNS();
			...
			if (useTemplate) {
				this.cb = this
						.applyToPlayer($(('<div/>')).addClass('controls'));
			...
			for ( var i in this.controlElementsConfig) {
				this.controlElements[i] = $(this.playerDom).find(
						'.' + classPrefix + i);
		...
		applyTemplate : function(dest, templateString) {
			var ref = this, classPrefix = this.pp.getNS();
					...
					$.each(tagsUsed, function(key, value) {
						...
						if (value.match(/\_/gi)) {
							...
							templateString = templateString.replace(value,
									'<span class="' + classPrefix + cn
											+ '"></span>');
						} else {
							// replace with className
							templateString = templateString.replace(value,
									'class="' + classPrefix + cn + '"');
						}
					});
				}
		...
		updateDisplay : function() {
			var ref = this, state = this.pp.getState();
			...
			this.controlElements.loop.addClass(
					this.pp.getConfig('loop') ? 'on' : 'off').removeClass(
					!this.pp.getConfig('loop') ? 'on' : 'off');
							...
							function(key, elmCfg) {
								...
								for ( var subset = 0; subset < elmCfg.length; subset++) {
									...
									$
											.each(
													elmCfg[subset].on,
													function(evtKey, eventName) {

														// thanx to FF3.6 this
														// approach became a
														// little complicated:
														var isSupported = ("on"
																+ eventName in window.document), callback = elmCfg[subset].call;
															...
															el
																	.setAttribute(
																			"on"
																					+ eventName,
																			'return;');
															isSupported = (typeof el["on"
																	+ eventName] == 'function');

														}

														if (isSupported) {
															ref.controlElements[key]
																	.bind(
																			eventName,
																			function(
																					event) {
		...
		displayTime : function(pct, dur, pos) {
			...
			var percent = Math
					.round((pct || this.pp.getLoadPlaybackProgress() || 0) * 10) / 10, duration = dur
					|| this.pp.getDuration() || 0, position = pos
					|| this.pp.getPosition() || 0, times = $.extend({}, this
					._clockDigits(duration, 'dur'), this._clockDigits(position,
					'elp'), this._clockDigits(duration - position, 'rem'));
		...
		displayProgress : function() {
			...
			if (this.controlElements['loaded'].data('pct') != percent) {
				this.controlElements['loaded'].data('pct', percent).css(
						"width", percent + "%");
			}
			;
		},
		...
		displayVolume : function(volume) {
			...
			var isVisible = this.cb.hasClass('active'), ref = this, fixed = this
					.getConfig('fixedVolume'), toggleMute = (this.controlElements['mute']
					.hasClass('toggle')
					|| this.controlElements['unmute'].hasClass('toggle') || this
					.getConfig('toggleMute'));
			...
			if (this.controlElements['vslider'].width() > this.controlElements['vslider']
					.height()) {
				this.controlElements['vmarker']
			...
			// "li" hack
			var lis = this.controlElements['volume'].find('li'), set = lis.length
					- Math.ceil((volume * 100) / lis.length);
			...
			if (toggleMute) {
				...
				default:
					...
					// vknob.css('left',
					// volume*(vslider.width()-(vknob.width()/2))+"px");
					break;
				}
			}
		...
		displayCuePoints : function(duration) {
			...
			var ref = this, prefix = this.pp.getNS();
			...
			$
					.each(
							this.pp.getCuePoints() || [],
							function() {

								var blipWidth = Math.max(100 / duration, Math
										.round(duration / 100), 1),
								// Math.max( (this.off - this.off / duration,
								// Math.round(duration/100), 1),
								blipPos = (this.on * 100 / duration)
										- ((blipWidth / 2) * 100 / duration), that = this, player = ref.pp, blip = $(
										document.createElement('div'))
										.addClass(prefix + 'cuepoint')
										.addClass('inactive').css('left',
												blipPos + "%").css('width',
												blipWidth + "%").data('on',
												this.on);
								...
								this.addListener('unlock', function() {
									$(blip).removeClass('inactive').addClass(
											'active');
									blip.click(function() {
		...
		displayQualityToggle : function(qual) {
			...
			var qualsCfg = this.getConfig('playbackQualities'), qualsItm = this.pp
					.getPlaybackQualities(), classPrefix = this.pp.getNS();
			best = [];

			// off
			if (qualsItm.length < 2 || qualsCfg.length < 2) {
				// this.controlElements['loquality'].removeClass().addClass('inactive').addClass(classPrefix
				// + 'loquality').data('qual', '');
				// this.controlElements['hiquality'].removeClass().addClass('inactive').addClass(classPrefix
				// + 'hiquality').data('qual', '');
				this.controlElements['qualitysmall'].removeClass().addClass(
						'inactive').addClass(classPrefix + 'qualitysmall')
						.data('qual', '');
				this.controlElements['qualitymedium'].removeClass().addClass(
						'inactive').addClass(classPrefix + 'qualitymedium')
						.data('qual', '');
				this.controlElements['qualitylarge'].removeClass().addClass(
						'inactive').addClass(classPrefix + 'qualitylarge')
						.data('qual', '');
				this.controlElements['qualityhd'].removeClass().addClass(
						'inactive').addClass(classPrefix + 'qualityhd').data(
						'qual', '');
				this.controlElements['qualityfullhd'].removeClass().addClass(
						'inactive').addClass(classPrefix + 'qualityfullhd')
						.data('qual', '');
				return;
			}

			this._active('qualitysmall', true).addClass('qualsmall').data(
					'qual', 'small');
			this._active('qualitymedium', true).addClass('qualmedium').data(
					'qual', 'medium');
			this._active('qualitylarge', true).addClass('quallarge').data(
					'qual', 'large');
			this._active('qualityhd', true).addClass('qualhd720').data('qual',
					'hd720');
			this._active('qualityfullhd', true).addClass('qualhd1080').data(
					'qual', 'hd1080');

			if (this.pp.getPlaybackQuality() == 'small') {
				this._active('qualitysmall', true).removeClass('dimmed');
				this._active('qualitymedium', true).addClass('dimmed');
				this._active('qualitylarge', true).addClass('dimmed');
				this._active('qualityhd', true).addClass('dimmed');
				this._active('qualityfullhd', true).addClass('dimmed');
			} else if (this.pp.getPlaybackQuality() == 'medium') {
				this._active('qualitysmall', true).addClass('dimmed');
				this._active('qualitymedium', true).removeClass('dimmed');
				this._active('qualitylarge', true).addClass('dimmed');
				this._active('qualityhd', true).addClass('dimmed');
				this._active('qualityfullhd', true).addClass('dimmed');
			} else if (this.pp.getPlaybackQuality() == 'large') {
				this._active('qualitysmall', true).addClass('dimmed');
				this._active('qualitymedium', true).addClass('dimmed');
				this._active('qualitylarge', true).removeClass('dimmed');
				this._active('qualityhd', true).addClass('dimmed');
				this._active('qualityfullhd', true).addClass('dimmed');
			} else if (this.pp.getPlaybackQuality() == 'hd720') {
				this._active('qualitysmall', true).addClass('dimmed');
				this._active('qualitymedium', true).addClass('dimmed');
				this._active('qualitylarge', true).addClass('dimmed');
				this._active('qualityhd', true).removeClass('dimmed');
				this._active('qualityfullhd', true).addClass('dimmed');
			} else {
				this._active('qualitysmall', true).addClass('dimmed');
				this._active('qualitymedium', true).addClass('dimmed');
				this._active('qualitylarge', true).addClass('dimmed');
				this._active('qualityhd', true).addClass('dimmed');
				this._active('qualityfullhd', true).removeClass('dimmed');
			}
		},
		...
		isLiveHandler : function(islive) {
			if (islive) {
				this.controlElements['golive'].addClass('on')
						.removeClass('off');
			} else {
				this.controlElements['golive'].addClass('off')
						.removeClass('on');
			}
		},
		...
		showQualityClk : function(evt) {
			if ($('#quality_options').is(':visible')) {
		...
		openCloseClk : function(evt) {
			var ref = this;
			$($(evt.currentTarget).attr('class').split(/\s+/)).each(
					function(key, value) {
						if (value.indexOf('toggle') == -1)
							return;
						ref.playerDom.find('.' + value.substring(6))
		...
		vsliderClk : function(evt) {
			...
			var slider = $(this.controlElements['vslider']), orientation = slider
					.width() > slider.height() ? 'hor' : 'vert', totalDim = (orientation == 'hor') ? slider
					.width()
					: slider.height(), pageX = (evt.originalEvent.touches) ? evt.originalEvent.touches[0].pageX
					: evt.pageX, pageY = (evt.originalEvent.touches) ? evt.originalEvent.touches[0].pageY
					: evt.pageY, requested = (orientation == 'hor') ? pageX
					- slider.offset().left : pageY - slider.offset().top, result = 0;

			if (requested < 0 || isNaN(requested) || requested == undefined) {
				result = 0;
			} else {
				result = (orientation == 'hor') ? (requested / totalDim)
						: 1 - (requested / totalDim);
			}
		...
		scrubberdragTooltip : function(evt) {
			...
			var slider = $(this.controlElements['scrubberdrag'][0]), loaded = $(this.controlElements['loaded'][0]), tip = $(this.controlElements['scrubbertip']), pageX = (evt.originalEvent.touches) ? evt.originalEvent.touches[0].pageX
					: evt.pageX, pageY = (evt.originalEvent.touches) ? evt.originalEvent.touches[0].pageY
					: evt.pageY, newPos = pageX - slider.offset().left
					- (tip.outerWidth() / 2), timeIdx = this.pp.getDuration()
					/ 100
					* ((pageX - slider.offset().left) * 100 / slider.width()), times = this
					._clockDigits(timeIdx, 'tip');

			if (this._isDVR) {
				timeIdx = this.pp.getDuration() - timeIdx;
				var then = new Date(
						(new Date().getTime() / 1000 - timeIdx) * 1000), // date
																			// minus
																			// timeidx
				then = then.getSeconds() + (60 * then.getMinutes())
						+ (60 * 60 * then.getHours()); // second of today
			...
			newPos = (newPos > slider.width() - tip.outerWidth()) ? slider
					.width()
					- tip.outerWidth() : newPos;
		...
		scrubberdragStartDragListener : function(event) {
			...
			var ref = this, slider = $(this.controlElements['scrubberdrag'][0]), loaded = $(this.controlElements['loaded'][0]), second = 0, dx = Math
					.abs(parseInt(slider.offset().left) - event.clientX),
			...
			applyValue = function(event) {
				...
				newPos = (newPos < 0) ? 0 : newPos;
				newPos = Math.abs(newPos / slider.width())
						* ref.pp.getDuration();
		...
		vknobStartDragListener : function(event, domObj) {
			...
			var ref = this, sliderIdx = (this.pp.getInFullscreen() === true && this.controlElements['vslider'].length > 1) ? 1
					: 0, knob = $(domObj[sliderIdx]), slider = $(this.controlElements['vslider'][sliderIdx]), dx = Math
					.abs(parseFloat(knob.position().left) - event.clientX), dy = Math
					.abs(parseFloat(knob.position().top) - event.clientY),

			volume = 0, mouseUp = function(mouseupevt) {
			...
			mouseMove = function(dragevent) {
				clearTimeout(ref._cTimer);
				var newXPos = (dragevent.clientX - dx), newXPos = (newXPos > slider
						.width()
						- knob.width() / 2) ? slider.width()
						- (knob.width() / 2) : newXPos, newXPos = (newXPos < 0) ? 0
						: newXPos, newYPos = (dragevent.clientY - dy), newYPos = (newYPos > slider
						.height()
						- knob.height() / 2) ? slider.height()
						- (knob.height() / 2) : newYPos, newYPos = (newYPos < 0) ? 0
						: newYPos;

				if (ref.controlElements['vslider'].width() > ref.controlElements['vslider']
						.height()) {
					knob.css('left', newXPos + 'px');
					volume = Math.abs(newXPos
							/ (slider.width() - (knob.width() / 2)));
					$(ref.controlElements['vmarker'][sliderIdx]).css('width',
							volume * 100 + "%");
				} else {
					knob.css('top', newYPos + 'px');
					volume = 1 - Math.abs(newYPos
							/ (slider.height() - (knob.height() / 2)));
					$(ref.controlElements['vmarker'][sliderIdx]).css('height',
							volume * 100 + "%");
				}
			};
		...
		handleStartDragListener : function(evt, domObj) {
			...
			var ref = this, dx = Math.abs(parseInt(this.cb.position().left)
					- evt.clientX), dy = Math
					.abs(parseInt(this.cb.position().top) - evt.clientY);

			/*
			 * this._initalPosition = { top: this.cb.css('top'), bottom:
			 * this.cb.css('bottom'), left: this.cb.css('left'), right:
			 * this.cb.css('right')
			 *  };
			 */
			// this._initalPosition = $.extend({}, this.cb.attr('style'),
			// this.cb.css());
			...
			var mouseMove = function(evt) {
				...
				newXPos = (newXPos > ref.playerDom.width() - ref.cb.width()) ? ref.playerDom
						.width()
						- ref.cb.width()
						: newXPos;
				...
				newYPos = (newYPos > ref.playerDom.height() - ref.cb.height()) ? ref.playerDom
						.height()
						- ref.cb.height()
						: newYPos;
		...
		_getVolume : function() {
			...
			var volume = parseFloat(this.cookie('volume')
					|| this.getConfig('volume') || 0.5), muted = this
					.cookie('muted') || false;
		...
		_clockDigits : function(secs, postfix) {
			...
			var hr = Math.floor(secs / (60 * 60)), divisor_for_minutes = secs
					% (60 * 60), min = Math.floor(divisor_for_minutes / 60), min_abs = hr
					* 60 + min, divisor_for_seconds = divisor_for_minutes % 60, sec = Math
					.floor(divisor_for_seconds), sec_abs = secs, result = {};

			result['min_' + postfix] = (min < 10) ? "0" + min : min;
			result['min_abs_' + postfix] = (min_abs < 10) ? "0" + min_abs
					: min_abs;
			result['sec_' + postfix] = (sec < 10) ? "0" + sec : sec;
			result['sec_abs_' + postfix] = (sec_abs < 10) ? "0" + sec_abs
					: sec_abs;
...
jQuery(function($) {
	projekktorContextmenu.prototype = {
		...
		initialize : function() {
			var ref = this, target = this.pp.getIframeWindow() || $(window);

			this._dest = $p.utils
					.blockSelection(this.applyToPlayer($('<ul/>')))
			this._items['player'] = {
				getContextTitle : function() {
					return ref.getConfig('playerName') + ' V'
							+ ref.pp.getPlayerVer();
				},
				open : function() {
					if (ref.getConfig('playerHome') != null) {
						target.get(0).location.href = ref
								.getConfig('playerHome');
						ref.pp.setPause();
					}
				}
			}
		...
		mousedownHandler : function(evt) {
			switch (evt.which) {
			case 3:
				var parentOffset = this.pp.getDC().offset(), yPos = (evt.pageY - parentOffset.top), xPos = (evt.pageX - parentOffset.left);
		...
		displayReadyHandler : function() {
			...
			for ( var i in this._items) {
				...
				span = $('<span/>').data('plugin', i).html(
						this._items[i].getContextTitle() || i);
				...
				$('<li/>').append(span).data('plugin', i).appendTo(this._dest)
			}
		},

		popup : function(url, width, height) {
			centeredY = window.screenY
					+ (((window.outerHeight / 2) - (height / 2)));
