...
{ /*
   ...
   */
    ...
    /**
     * generates the basic string serialization of a token that a server would respond to request_token and access_token
     * calls with
     */
    ...
    { /*
       * {{{
       */
        return "oauth_token=" . OAuthUtil :: urlencode_rfc3986($this->key) . "&oauth_token_secret=" . OAuthUtil :: urlencode_rfc3986(
            $this->secret);
    } /*
       * }}}
       */
   ...
   */
    ...
    { /*
       ...
       */
        ...
        {
            // collect request parameters from query string (GET) and post-data (POST) if appropriate (note: POST vars
            // have priority)
            $req_parameters = $_GET;
    ...
    /**
     * Returns the normalized parameters of the request This will be all (except oauth_signature) parameters, sorted
     * first by key, and if duplicate keys, then by value.
     * The returned string will be all the key=value pairs concated
     * by &.
     * 
     * @return string
     */
    ...
    /**
     * Returns the base string of this request The base string defined as the method, the url and the parameters
     * (normalized), each urlencoded and the concated with &.
     */
    ...
    /**
     * builds the data one would send in a POST request TODO(morten.fangel): this function might be easily replaced with
     * http_build_query() and corrections for rfc3986 compatibility..
     * but not sure
     */
