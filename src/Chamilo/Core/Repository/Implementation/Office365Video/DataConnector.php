...
class DataConnector extends \Chamilo\Core\Repository\External\DataConnector
{
    ...
    public function __construct($externalRepositoryInstance)
    {
        ...
        $this->microsoftClientService = new MicrosoftClientService(new MicrosoftSharePointClientSettingsProvider($externalRepositoryInstance, $user));
    }
        ...
        if ($this->isUserLoggedIn())
        {
            try
            {
                $request = $this->microsoftClientService->createRequest(
                    'GET', 'VideoService/Channels(guid\'' . ExternalObject :: getChannelId($id) . '\')/Videos(guid\'' . ExternalObject :: getVideoId($id) . '\')');
                $responseXml = simplexml_load_string($this->microsoftClientService->sendRequest($request, false));
                $namespaces = $responseXml->getNamespaces(true);
                $videoProperties = $responseXml->content->children($namespaces['m'])->properties->children($namespaces['d']);
    ...
    public function getVideoEmbedCode($id, $width = 600, $height = 480)
    {
        $request = $this->microsoftClientService->createRequest(
            'GET', 'VideoService/Channels(guid\'' . ExternalObject :: getChannelId($id) . '\')/Videos(guid\'' . ExternalObject :: getVideoId($id) . 
            '\')/GetVideoEmbedCode?width=' . $width . '&height=' . $height . '&autoplay=false&showinfo=true');
        $responseXml = simplexml_load_string($this->microsoftClientService->sendRequest($request, false));
    ...
    private function getVideos($searchString = null, $orderProperty = null)
    {
        if (! isset($this->videos))
        {
            ...
            if (! empty($channelId))
            {
                ...
                foreach ($responseXml->entry as $entry)
                {
                    $this->videos[] = $entry->content->children($namespaces['m'])->properties->children($namespaces['d']);
                }
            }
        }
