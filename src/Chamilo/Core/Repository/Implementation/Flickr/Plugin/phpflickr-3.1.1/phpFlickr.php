...
/*
 * phpFlickr Written by Dan Coulter (dan@dancoulter.com) Project Home Page: http://github.com/dancoulter/phpflickr This
...
class phpFlickr
{
    ...
    /*
     * When your database cache table hits this many rows, a cleanup will occur to get rid of all of the old rows
     * and cleanup the garbage in the table. For most personal apps, 1000 rows should be more than enough. If your
     * site gets hit by a lot of traffic or you have a lot of disk space to spare, bump this number up. You should
     * try to set it high enough that the cleanup only happens every once in a while, so this will depend on the
     * growth of your table.
     */
    ...
    function activity_userPhotos($timeframe = null, $per_page = null, $page = null)
    {
        ...
        $this->request(
            'flickr.activity.userPhotos', array("timeframe" => $timeframe, "per_page" => $per_page, "page" => $page)
        );
    ...
    function auth($perms = "read", $remember_uri = true)
    {
        ...
        if (empty($_SESSION['phpFlickr_auth_token']) && empty($this->token))
        {
            ...
            {
                header(
                    "Location: http://www.23hq.com/services/auth/?api_key=" . $this->api_key . "&perms=" . $perms .
                    "&api_sig=" . $api_sig
                );
            }
            else
            {
                header(
                    "Location: https://www.flickr.com/services/auth/?api_key=" . $this->api_key . "&perms=" . $perms .
                    "&api_sig=" . $api_sig
                );
            }
            exit();
    ...
    function auth_url($frob, $perms = 'read')
    {
        ...
        return sprintf(
            'https://flickr.com/services/auth/?api_key=%s&perms=%s&frob=%s&api_sig=%s', $this->api_key, $perms, $frob,
            $sig
        );
    }
    ...
    function buildPhotoURL($photo, $size = "Medium")
    {
        ...
        {
            $url =
                "https://farm" . $photo['farm'] . ".static.flickr.com/" . $photo['server'] . "/" . $photo['id'] . "_" .
    ...
    function cache($request, $response)
    {
        ...
        if ($this->cache == 'db')
        {
            ...
            $sql = 'INSERT INTO ' . $this->cache_table . ' (request, response, expiration)
						VALUES (\'' . $reqhash . '\', \'' . $response . '\', TIMESTAMPADD(SECOND,' .
                $this->cache_expire . ',CURRENT_TIMESTAMP))
    ...
    /**
     * ***************************** To use the phpFlickr::call method, pass a string containing the API method you
     * want to use and an associative array of arguments.
     * For example: $result =
     * $f->call("flickr.photos.comments.getList", array("photo_id"=>'34952612')); This method will allow you to make
     * calls to arbitrary methods that haven't been implemented in phpFlickr yet. *****************************
     */
    ...
    function collections_getTree($collection_id = null, $user_id = null)
    {
        ...
        return $this->call(
            'flickr.collections.getTree', array('collection_id' => $collection_id, 'user_id' => $user_id)
        );
    }

    /*
     * These functions are the direct implementations of flickr calls. For method documentation, including
     * arguments, visit the address included in a comment in the function.
     */
    ...
    function contacts_getList($filter = null, $page = null, $per_page = null)
    {
        ...
        $this->request(
            'flickr.contacts.getList', array('filter' => $filter, 'page' => $page, 'per_page' => $per_page)
        );
    ...
    function contacts_getListRecentlyUploaded($date_lastupload = null, $filter = null)
    {
        ...
        return $this->call(
            'flickr.contacts.getListRecentlyUploaded', array('date_lastupload' => $date_lastupload, 'filter' => $filter)
        );
    }
    ...
    function contacts_getPublicList($user_id, $page = null, $per_page = null)
    {
        ...
        $this->request(
            'flickr.contacts.getPublicList', array('user_id' => $user_id, 'page' => $page, 'per_page' => $per_page)
        );
    ...
    function galleries_getList($user_id, $per_page = null, $page = null)
    {
        ...
        return $this->call(
            'flickr.galleries.getList', array('user_id' => $user_id, 'per_page' => $per_page, 'page' => $page)
        );
    }
    ...
    function galleries_getListForPhoto($photo_id, $per_page = null, $page = null)
    {
        ...
        return $this->call(
            'flickr.galleries.getListForPhoto', array('photo_id' => $photo_id, 'per_page' => $per_page, 'page' => $page)
        );
    }
    ...
    function getCached($request)
    {
        ...
        {
            $result = mysqli_query(
                $this->cache_db, "SELECT response FROM " . $this->cache_table . " WHERE request = '" . $reqhash .
    ...
    function people_getPhotos($user_id, $args = [])
    {
         ...
         * there are just so many arguments to this API method. What you'll need to do is pass an associative array
         * to the function containing the arguments you want to pass to the API. For example: $photos =
         * $f->photos_search(array("tags"=>"brown,cow", "tag_mode"=>"any")); This will return photos tagged with
         * either "brown" or "cow" or both. See the API documentation (link below) for a full list of arguments.
         */
    ...
    function photos_comments_addComment($photo_id, $comment_text)
    {
        ...
        $this->request(
            "flickr.photos.comments.addComment", array("photo_id" => $photo_id, "comment_text" => $comment_text), true
        );
    ...
    function photos_comments_editComment($comment_id, $comment_text)
    {
        ...
        $this->request(
            "flickr.photos.comments.editComment", array("comment_id" => $comment_id, "comment_text" => $comment_text),
            true
        );
    ...
    function photos_getFavorites($photo_id, $page = null, $per_page = null)
    {
        ...
        $this->request(
            "flickr.photos.getFavorites", array("photo_id" => $photo_id, "page" => $page, "per_page" => $per_page)
        );
    ...
    function photos_getWithGeoData($args = [])
    {
         ...
         * for this function. The only difference here is that this doesn't require any arguments. The
    ...
    function photos_licenses_setLicense($photo_id, $license_id)
    {
        ...
        /* Requires Authentication */
        $this->request(
            "flickr.photos.licenses.setLicense", array("photo_id" => $photo_id, "license_id" => $license_id), true
        );
    ...
    function photos_people_delete($photo_id, $user_id, $email = null)
    {
        ...
        return $this->call(
            'flickr.photos.people.delete', array('photo_id' => $photo_id, 'user_id' => $user_id, 'email' => $email)
        );
    }
    ...
    function photos_people_deleteCoords($photo_id, $user_id)
    {
        ...
        return $this->call(
            'flickr.photos.people.deleteCoords', array('photo_id' => $photo_id, 'user_id' => $user_id)
        );
    }
    ...
    function photos_search($args = [])
    {
        ...
        $result = $this->request("flickr.photos.search");
    ...
    function photos_setContentType($photo_id, $content_type)
    {
        ...
        return $this->call(
            'flickr.photos.setContentType', array('photo_id' => $photo_id, 'content_type' => $content_type)
        );
    }
    ...
    function photos_setMeta($photo_id, $title, $description)
    {
        ...
        $this->request(
            "flickr.photos.setMeta", array("photo_id" => $photo_id, "title" => $title, "description" => $description),
            true
        );
    ...
    function photos_transform_rotate($photo_id, $degrees)
    {
        ...
        $this->request(
            "flickr.photos.transform.rotate", array("photo_id" => $photo_id, "degrees" => $degrees), true
        );
    ...
    function photosets_addPhoto($photoset_id, $photo_id)
    {
        ...
        $this->request(
            "flickr.photosets.addPhoto", array("photoset_id" => $photoset_id, "photo_id" => $photo_id), true
        );
    ...
    function photosets_removePhoto($photoset_id, $photo_id)
    {
        ...
        $this->request(
            "flickr.photosets.removePhoto", array("photoset_id" => $photoset_id, "photo_id" => $photo_id), true
        );
    ...
    function photosets_removePhotos($photoset_id, $photo_ids)
    {
        ...
        return $this->call(
            'flickr.photosets.removePhotos', array('photoset_id' => $photoset_id, 'photo_ids' => $photo_ids)
        );
    }
    ...
    function photosets_reorderPhotos($photoset_id, $photo_ids)
    {
        ...
        return $this->call(
            'flickr.photosets.reorderPhotos', array('photoset_id' => $photoset_id, 'photo_ids' => $photo_ids)
        );
    }
    ...
    function photosets_setPrimaryPhoto($photoset_id, $photo_id)
    {
        ...
        return $this->call(
            'flickr.photosets.setPrimaryPhoto', array('photoset_id' => $photoset_id, 'photo_id' => $photo_id)
        );
    }
    ...
    function places_findByLatLon($lat, $lon, $accuracy = null)
    {
        ...
        return $this->call(
            'flickr.places.findByLatLon', array('lat' => $lat, 'lon' => $lon, 'accuracy' => $accuracy)
        );
    }
    ...
    function places_getChildrenWithPhotosPublic($place_id = null, $woe_id = null)
    {
        ...
        return $this->call(
            'flickr.places.getChildrenWithPhotosPublic', array('place_id' => $place_id, 'woe_id' => $woe_id)
        );
    }
    ...
    function request($command, $args = [], $nocache = false)
    {
        ...
        // Process arguments, including method and login data.
        $args = array_merge(
            array("method" => $command, "format" => "json", "nojsoncallback" => "1", "api_key" => $this->api_key), $args
        );
        if (!empty($this->token))
        {
            $args = array_merge($args, array("auth_token" => $this->token));
        ...
        /*
         * Uncomment this line (and comment out the next one) if you're doing large queries and you're concerned
         * about time. This will, however, change the structure of the result, so be sure that you look at the
         * results.
         */
        ...
        if ($this->parsed_response['stat'] == 'fail')
        {
            ...
            {
                die(
                "The Flickr API returned the following error: #{$this->parsed_response['code']} - {$this->parsed_response['message']}"
                );
            }
            else
            {
    ...
    function stats_getCollectionStats($date, $collection_id)
    {
        ...
        return $this->call(
            'flickr.stats.getCollectionStats', array('date' => $date, 'collection_id' => $collection_id)
        );
    }
    ...
    function stats_getPhotostreamDomains($date, $per_page = null, $page = null)
    {
        ...
        return $this->call(
            'flickr.stats.getPhotostreamDomains', array('date' => $date, 'per_page' => $per_page, 'page' => $page)
        );
    }
                ...
                if ($this->results)
                {
                    ...
                    if ($this->results)
                    {
                        ...
                        $this->photos = array_merge(
                            $this->photos, array_slice($this->results['photo'], 0, $max % 500 + 1)
                        );
                    }
