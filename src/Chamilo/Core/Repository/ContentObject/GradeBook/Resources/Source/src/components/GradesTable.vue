            ...
            <b-thead>
                <b-tr class="table-row table-head-row">
                    ...
                    <draggable :list="gradeBook.categories" tag="div" style="display: contents" @end="onDragEnd" :disabled="catEditItemId !== null">
                        <template v-for="{id, name, color, itemIds} in gradeBook.categories">
                    ...
                    <b-th v-if="showNullCategory" :colspan="Math.max(gradeBook.nullCategory.itemIds.length, 1)" class="mod-no-category-assigned" :class="{'is-droppable': categoryDropArea === 0}" title="Zonder categorie"
...
export default class GradesTable extends Vue {
    ...
    get showNullCategory() {
        return this.isDragging || this.gradeBook.nullCategory.itemIds.length > 0;
    }
    ...
    setCategoryName(id: number, name: string) {
    ...
    setName(itemId: ItemId, name: string) {
        this.gradeBook.setName(itemId, name);
