...
class TrackingServiceTest extends Test
{
    ...
    public function setUp()
    {
        ...
        $this->attemptSummaryCalculatorMock = $this->getMockBuilder(AttemptSummaryCalculator::class)
            ->disableOriginalConstructor()->getMock();

        $this->assessmentTrackingServiceMock = $this->getMockBuilder(AssessmentTrackingService::class)
            ->disableOriginalConstructor()->getMock();

        $this->progressCalculatorMock = $this->getMockBuilder(ProgressCalculator::class)
            ->disableOriginalConstructor()->getMock();

        $this->trackingService = new TrackingService(
            $this->attemptTrackingServiceMock, $this->attemptSummaryCalculatorMock,
            $this->assessmentTrackingServiceMock, $this->progressCalculatorMock
        );
    ...
    {
        $this->attemptTrackingServiceMock->expects($this->once())
            ->method('setActiveAttemptCompleted')
            ->with($this->learningPath, $this->treeNode, $this->user);
    ...
    {
        $this->attemptTrackingServiceMock->expects($this->once())
            ->method('setActiveAttemptTotalTime')
            ->with($this->learningPath, $this->treeNode, $this->user);
    ...
    {
        $this->attemptTrackingServiceMock->expects($this->once())
            ->method('setAttemptTotalTimeByTreeNodeAttemptId')
            ->with(5);
    ...
    {
        $this->progressCalculatorMock->expects($this->once())
            ->method('getLearningPathProgress')
    ...
    {
        $this->assessmentTrackingServiceMock->expects($this->once())
            ->method('isMaximumAttemptsReachedForAssessment')
            ->with($this->learningPath, $this->user, $this->treeNode);

        $this->trackingService->isMaximumAttemptsReachedForAssessment(
            $this->learningPath, $this->user, $this->treeNode
        );
    }
    ...
    {
        $this->assessmentTrackingServiceMock->expects($this->once())
            ->method('saveAnswerForQuestion')
            ->with($this->learningPath, $this->user, $this->treeNode, 5, 'test', 1, 'test');

        $this->trackingService->saveAnswerForQuestion(
            $this->learningPath, $this->user, $this->treeNode, 5, 'test', 1, 'test'
        );
    }
    ...
    {
        $this->assessmentTrackingServiceMock->expects($this->once())
            ->method('saveAssessmentScore')
            ->with($this->learningPath, $this->user, $this->treeNode, 1);

        $this->trackingService->saveAssessmentScore(
            $this->learningPath, $this->user, $this->treeNode, 1
        );
    }
    ...
    {
        $this->assessmentTrackingServiceMock->expects($this->once())
            ->method('changeAssessmentScore')
            ->with($this->learningPath, $this->user, $this->treeNode, 5, 1);

        $this->trackingService->changeAssessmentScore(
            $this->learningPath, $this->user, $this->treeNode, 5, 1
        );
    }
    ...
    {
        $this->assessmentTrackingServiceMock->expects($this->once())
            ->method('changeQuestionScoreAndFeedback')
            ->with($this->learningPath, $this->user, $this->treeNode, 5, 1, 'test feedback');

        $this->trackingService->changeQuestionScoreAndFeedback(
            $this->learningPath, $this->user, $this->treeNode, 5, 1, 'test feedback'
        );
    }
    ...
    {
        $this->assessmentTrackingServiceMock->expects($this->once())
            ->method('getQuestionAttempts')
            ->with($this->learningPath, $this->user, $this->treeNode, 5);

        $this->trackingService->getQuestionAttempts(
            $this->learningPath, $this->user, $this->treeNode, 5
        );
    }
    ...
    {
        $this->assessmentTrackingServiceMock->expects($this->once())
            ->method('registerQuestionAttempts')
            ->with($this->learningPath, $this->user, $this->treeNode, [5, 2, 8]);

        $this->trackingService->registerQuestionAttempts(
            $this->learningPath, $this->user, $this->treeNode, [5, 2, 8]
        );
    }
    ...
    public function testDeleteTreeNodeAttemptById()
    {
        ...
        $this->attemptTrackingServiceMock->expects($this->once())
            ->method('deleteTreeNodeAttemptById')
            ->with($this->learningPath, $this->user, $reportingUser, $this->treeNode, 5);

        $this->trackingService->deleteTreeNodeAttemptById(
            $this->learningPath, $this->user, $reportingUser, $this->treeNode, 5
        );
    }
    ...
    public function testDeleteTreeNodeAttemptsForTreeNode()
    {
        ...
        $this->attemptTrackingServiceMock->expects($this->once())
            ->method('deleteTreeNodeAttemptsForTreeNode')
            ->with($this->learningPath, $this->user, $reportingUser, $this->treeNode);

        $this->trackingService->deleteTreeNodeAttemptsForTreeNode(
            $this->learningPath, $this->user, $reportingUser, $this->treeNode
        );
    }
    ...
    public function testCanDeleteLearningPathAttemptData()
    {
        ...
        $this->attemptTrackingServiceMock->expects($this->once())
            ->method('canDeleteLearningPathAttemptData')
            ->with($this->user, $reportingUser);
    ...
    {
        $this->progressCalculatorMock->expects($this->once())
            ->method('isCurrentTreeNodeBlocked')
            ->with($this->learningPath, $this->user, $this->treeNode);
    ...
    {
        $this->progressCalculatorMock->expects($this->once())
            ->method('getResponsibleNodesForBlockedTreeNode')
            ->with($this->learningPath, $this->user, $this->treeNode);

        $this->trackingService->getResponsibleNodesForBlockedTreeNode(
            $this->learningPath, $this->user, $this->treeNode
        );
    }
    ...
    public function testGetTotalTimeSpentInTreeNode()
    {
        $this->attemptSummaryCalculatorMock->expects($this->once())
            ->method('getTotalTimeSpentInTreeNode')
            ->with($this->learningPath, $this->user, $this->treeNode);
    ...
    {
        $this->attemptSummaryCalculatorMock->expects($this->once())
            ->method('getAverageScoreInTreeNode')
            ->with($this->learningPath, $this->user, $this->treeNode);
    ...
    {
        $this->attemptSummaryCalculatorMock->expects($this->once())
            ->method('getMaximumScoreInTreeNode')
            ->with($this->learningPath, $this->user, $this->treeNode);
    ...
    {
        $this->attemptSummaryCalculatorMock->expects($this->once())
            ->method('getMinimumScoreInTreeNode')
            ->with($this->learningPath, $this->user, $this->treeNode);
    ...
    {
        $this->attemptSummaryCalculatorMock->expects($this->once())
            ->method('getLastAttemptScoreForTreeNode')
            ->with($this->learningPath, $this->user, $this->treeNode);
    ...
    public function testCountLearningPathAttemptsWithUsers()
    {
        ...
        $this->attemptTrackingServiceMock->expects($this->once())
            ->method('countLearningPathAttemptsWithUsers')
            ->with($this->learningPath, $this->treeNode, $condition);
    ...
    public function testGetLearningPathAttemptsWithUser()
    {
        ...
        $this->attemptTrackingServiceMock->expects($this->once())
            ->method('getLearningPathAttemptsWithUser')
            ->with($this->learningPath, $this->treeNode, $condition, 0, 0, []);
    ...
    public function testCountTargetUsersWithLearningPathAttempts()
    {
        ...
        $this->attemptTrackingServiceMock->expects($this->once())
            ->method('countTargetUsersWithLearningPathAttempts')
    ...
    public function testGetTargetUsersWithLearningPathAttempts()
    {
        ...
        $this->attemptTrackingServiceMock->expects($this->once())
            ->method('getTargetUsersWithLearningPathAttempts')
            ->with($this->learningPath, $this->treeNode, $condition, 0, 0, []);
