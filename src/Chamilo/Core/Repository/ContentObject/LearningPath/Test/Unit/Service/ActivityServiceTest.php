...
class ActivityServiceTest extends ChamiloTestCase
{
    ...
    protected function setUp(): void    {
        ...
        $this->activityService = new ActivityService($this->activityRepositoryMock, $this->automaticNumberingServiceMock);
    ...
    {
        $this->activityRepositoryMock->expects($this->exactly(2))->method('countActivitiesForContentObject')->withConsecutive(
            [$this->contentObjects[2]],
            [$this->contentObjects[6]])->will($this->onConsecutiveCalls(2, 5));
    ...
    public function testRetrieveActivitiesForTreeNode()
    {
        ...
        $this->activityRepositoryMock->expects($this->exactly(2))->method('retrieveActivitiesForContentObject')->withConsecutive(
            [$this->contentObjects[2]],
            [$this->contentObjects[6]])->will($this->onConsecutiveCalls($activities2, $activities5));

        $this->activityRepositoryMock->expects($this->once())->method('filterActivities')->will(
