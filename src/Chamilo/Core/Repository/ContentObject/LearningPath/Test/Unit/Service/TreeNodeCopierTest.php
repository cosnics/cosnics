...
class TreeNodeCopierTest extends ChamiloTestCase
{
    ...
    protected function setUp()
    {
        ...
        $this->treeBuilderMock = $this->getMockBuilder(TreeBuilder::class)
            ->disableOriginalConstructor()->getMock();

        $this->treeNodeDataServiceMock = $this->getMockBuilder(TreeNodeDataService::class)
            ->disableOriginalConstructor()->getMock();

        $this->contentObjectCopierWrapper = $this->getMockBuilder(ContentObjectCopierWrapper::class)
            ->disableOriginalConstructor()->getMock();
        ...
        $this->treeBuilderMock->expects($this->once())
            ->method('buildTree')
            ->with($this->contentObjects[1])
            ->will($this->returnValue($this->tree));

        $this->treeNodeDataServiceMock->expects($this->exactly(7))
    ...
    {
        $this->contentObjectRepositoryMock->expects($this->exactly(6))
            ->method('findById')
            ->will(
                $this->onConsecutiveCalls(
                    $this->contentObjects[2], $this->contentObjects[6], $this->contentObjects[3],
                    $this->contentObjects[4], $this->contentObjects[5], $this->contentObjects[7]
                )
            );

        $this->contentObjectCopierWrapper->expects($this->exactly(6))
            ->method('copyContentObject')
            ->will(
                $this->returnCallback(
                    function (ContentObject $contentObject)
                    {
