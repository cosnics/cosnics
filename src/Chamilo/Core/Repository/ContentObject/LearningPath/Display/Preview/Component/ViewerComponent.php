 ...
 */
class ViewerComponent extends \Chamilo\Core\Repository\ContentObject\LearningPath\Display\Preview\Manager
    implements GlossaryDisplaySupport, LearningPathDisplaySupport, AssessmentDisplaySupport, ForumDisplaySupport,
    BlogDisplaySupport, WikiDisplaySupport
{
    ...
    public function run()
    {
        ...
        return $this->getApplicationFactory()->getApplication(
            $className, new ApplicationConfiguration($this->getRequest(), $this->get_user(), $this)
        )->run();
    }
    ...
    {
        return $this->get_url([Embedder::PARAM_EMBEDDED_CONTENT_OBJECT_ID => null]);
    }
    ...
    public function get_assessment_question_attempts()
    {
        return $this->trackingService->getQuestionAttempts(
            Preview::get_root_content_object(), $this->getUser(), $this->getCurrentTreeNode()
        );
    }
    ...
    public function register_question_ids($question_ids)
    {
        $this->question_attempts = $this->trackingService->registerQuestionAttempts(
            Preview::get_root_content_object(), $this->getUser(), $this->getCurrentTreeNode(), $question_ids
        );
    }
    ...
    public function save_assessment_answer($complex_question_id, $answer, $score, $hint = null)
    {
        $this->trackingService->saveAnswerForQuestion(
            Preview::get_root_content_object(), $this->getUser(), $this->getCurrentTreeNode(), $complex_question_id,
            $answer, $score, $hint
        );
    }
    ...
    public function save_assessment_result($total_score)
    {
        $this->trackingService->saveAssessmentScore(
            Preview::get_root_content_object(), $this->getUser(), $this->getCurrentTreeNode(), $total_score
        );
    }
}
