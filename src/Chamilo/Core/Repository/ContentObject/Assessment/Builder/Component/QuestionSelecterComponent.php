...
class QuestionSelecterComponent extends Manager
{
    ...
    public function run()
    {
        ...
        if ($assessment_id)
        {
            $clois = DataManager::retrieve_complex_content_object_items(
                ComplexContentObjectItem::class, new EqualityCondition(
                    ...
                    ), new StaticConditionVariable($assessment_id), ComplexContentObjectItem::getStorageUnitName()
                )
            );
        ...
        {
            $question_ids = $this->getRequest()->getFromRequestOrQuery(self::PARAM_QUESTION_ID);
