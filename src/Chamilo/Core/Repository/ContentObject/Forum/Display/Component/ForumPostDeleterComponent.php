...
class ForumPostDeleterComponent extends Manager
{
    ...
    public function run()
    {
        $selected_post_id = $this->getRequest()->query->get(self::PARAM_SELECTED_FORUM_POST);
    ...
    private function my_redirect($success)
    {
        $message = htmlentities(
            Translation::get(
                ($success ? 'ObjectDeleted' : 'ObjectNotdeleted'), ['OBJECT' => Translation::get('ForumPost')],
        ...
        $filter = [self::PARAM_SELECTED_FORUM_POST];
