...
class OrderForm extends ContentObjectForm
{
    ...
    function build_option_form()
    {
        ...
        $this->addElement(
            ...
            null, 
            array('id' => Order :: PROPERTY_DISPLAY_TYPE));
        ...
        $order_limit[] = $this->createElement(
            'select', 
        ...
        try
        {
            ...
            if (! $allowed_to_edit_order_limit)
            {
                $this->get_group_element(
                    self :: PROPERTY_ORDER_LIMIT_GROUP, 
                    Order :: PROPERTY_ORDER_LIMIT)->freeze();
            }
            ...
            if (! $allowed_to_edit_options)
            {
                ...
                for ($option_number = 0; $option_number < $number_of_options; $option_number ++)
                {
                    $this->get_group_element(
                        self :: PROPERTY_OPTION_GROUP . '[' . $option_number . ']', 
                        self :: PROPERTY_OPTION . '[' . $option_number . ']' . '[' .
                             OrderOption :: PROPERTY_VALUE . ']')->freeze();
                    $this->get_group_element(
                        self :: PROPERTY_OPTION_GROUP . '[' . $option_number . ']', 
        ...
        if (! $this->isSubmitted())
        {
            ...
            if ($object->get_number_of_options())
            {
                ...
                foreach ($options as $key => $option)
                {
                    ...
                    $defaults[self :: PROPERTY_OPTION . '[' . $option_number . ']' . '[' .
                         OrderOption :: PROPERTY_ID . ']'] = $option->get_id();
                    $defaults[self :: PROPERTY_OPTION . '[' . $option_number . ']' . '[' .
                         OrderOption :: PROPERTY_VALUE . ']'] = $option->get_value();
                    $option_number ++;
                }
            }
        }
