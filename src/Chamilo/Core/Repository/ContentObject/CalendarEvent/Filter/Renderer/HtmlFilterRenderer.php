...
class HtmlFilterRenderer extends \Chamilo\Core\Repository\Filter\Renderer\HtmlFilterRenderer
{
    ...
    public function add_properties()
    {
        ...
        {
            $html[] = '<div class="parameter" id="' . $this->get_parameter_name(FilterData :: FILTER_END_DATE) . '">' . Translation :: get(
                'EndsBetween', 
                array(
                    'FROM' => $filter_data->get_end_date(FilterData :: FILTER_FROM_DATE), 
                    'TO' => $filter_data->get_end_date(FilterData :: FILTER_TO_DATE))) . '</div>';
        }
        else
        {
            if ($filter_data->get_end_date(FilterData :: FILTER_FROM_DATE))
            {
                $html[] = '<div class="parameter" id="' . $this->get_parameter_name(FilterData :: FILTER_END_DATE) . '">' .
                     Translation :: get(
                        'EndsAfter', 
            ...
            elseif ($filter_data->get_end_date(FilterData :: FILTER_TO_DATE))
            {
                $html[] = '<div class="parameter" id="' . $this->get_parameter_name(FilterData :: FILTER_END_DATE) . '">' . Translation :: get(
                    'EndsBefore', 
                    array('TO' => $filter_data->get_end_date(FilterData :: FILTER_TO_DATE))) . '</div>';
            }
        }
