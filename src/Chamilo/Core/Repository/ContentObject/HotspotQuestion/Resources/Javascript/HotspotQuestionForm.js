...
$(function() {
    ...
    function redrawPolygon() {
        ...
        $('#selected_image').fillPolygon(positions[currentPolygon].X, positions[currentPolygon].Y, {
            clss : 'polygon_fill_' + currentPolygon,
            color : colours[currentPolygon],
            alpha : 0.5
        });
        $('#selected_image').drawPolygon(positions[currentPolygon].X, positions[currentPolygon].Y, {
    ...
    function setCoordinates() {
        var coordinatesField = $('input[name="coordinates[' + currentPolygon + ']"]'), coordinatesData = [], currentCoordinates = positions[currentPolygon];
    ...
    function initializePolygons() {
        $('input[name*="coordinates"]').each(function(i) {
            var fieldName = $(this).attr('name'), id = fieldName.substr(12, fieldName.length - 13), fieldValue = $(this).val();
    ...
    function removeOption(ev, ui) {
        ...
        var tableBody = $(this).parent().parent().parent(), id = $(this).attr('id'), row = 0, rows;
    ...
    function addOption(ev, ui) {
        ...
        var numberOfOptions = $('#mc_number_of_options').val(), newNumber = (parseInt(numberOfOptions, 10) + 1), rowClass = (numberOfOptions - skippedOptions) % 2 === 0 ? 'row_even' : 'row_odd', name = 'correct[' + numberOfOptions + ']', id = name, fieldColour, fieldCoordinates, fieldAnswer, fieldComment, fieldScore, fieldEdit, fieldReset, fieldDelete, string, parameters, editorNameAnswer, editorNameComment;
        ...
        string = '<tr id="option_' + numberOfOptions + '" class="' + rowClass + '"><td>' + fieldColour + fieldCoordinates + '</td><td>' + fieldAnswer + '</td><td>' + fieldComment + '</td><td>' + fieldScore + '</td><td>' + fieldEdit + fieldReset + fieldDelete + '</td></tr>';
    ...
    function setHotspotImage(ev, ui) {
        ...
        $('div.formw', $('#selected_image').closest('div.row')).css('float', 'left').css('width', '100%');
    ...
    $(document).ready(function() {
        ...
        $(document).on('click', '.element_finder_inactive a:not(.disabled, .category)', setHotspotImage);
