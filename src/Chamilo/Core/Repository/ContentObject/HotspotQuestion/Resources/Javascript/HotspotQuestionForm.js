...
$(function() {
	...
	function getEditIcon() {
		return $('.data_table > tbody > tr:first > td:last .edit_option').attr(
				'src');
	}

	function getResetIcon() {
		return $('.data_table > tbody > tr:first > td:last .reset_option')
				.attr('src');
	}
	...
	$(document)
			.ready(
					function() {
						...
						$(document).on('click', 'input[name*="edit"]',
								editPolygon);
						$(document).on('click', 'input[name*="reset"]',
								resetPolygon);

						// Bind clicks on the image
						$(document).on('click', '#selected_image',
								getCoordinates);

						// Bind actions to option management buttons
						$(document).on('click', '.remove_option',
								removeOption);
						...
						if (value != '') {
							$('div.label',
									$('#selected_image').closest('div.row'))
									.remove();
							$('div.formw',
									$('#selected_image').closest('div.row'))
						...
						$(document).on('click',
								'input[name="recalculate_weight"]', lockWeight);
					});
