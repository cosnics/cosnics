...
class SorterComponent extends Manager implements BreadcrumbLessComponentInterface
{
    ...
    public function run()
    {
        ...
        {
            $direction = $this->getRequest()->query->get(self::PARAM_SORT, self::SORT_UP);
            ...
            else
            {
                ...
                if ($succes)
                {
                    $new_content_object_ids_path =
                        $this->get_current_node()->get_parents_content_object_ids(true, true);
            ...
            $message = htmlentities(
                Translation::get(
                    ($succes ? 'ObjectUpdated' : 'ObjectNotUpdated'), ['OBJECT' => Translation::get('ContentObject')],
