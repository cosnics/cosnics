...
class HotpotatoesForm extends ContentObjectForm
{
    ...
    public function upload()
    {
        ...
        $filename = Filesystem :: create_unique_name(
            Path :: getInstance()->getStoragePath('hotpotatoes') . $owner, 
            $_FILES['file']['name']);
        $filename_split = explode('.', $filename);
        unset($filename_split[count($filename_split) - 1]);
        $file = implode('.', $filename_split);
        
        $hotpot_path = Path :: getInstance()->getStoragePath('hotpotatoes') . $owner . '/';
    ...
    // $hotpot_path = Path :: getInstance()->getStoragePath('hotpotatoes') . $this->get_owner_id() . '/';
