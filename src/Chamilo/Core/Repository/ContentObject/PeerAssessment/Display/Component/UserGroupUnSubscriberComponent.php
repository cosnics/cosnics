...
class UserGroupUnSubscriberComponent extends Manager
{
    ...
    public function run()
    {
        if (!is_null(Request::get(self::PARAM_GROUP)))
        {
            ...
            $this->redirect(
                $message, !$success, array(self::PARAM_ACTION => self::ACTION_VIEW_USER_ATTEMPT_STATUS)
            );
        }
    ...
    private function render_groups(array $groups)
    {
        ...
        $image = new FontAwesomeGlyph('users');
        $level = 0;
    ...
    private function render_toolbar($group)
    {
        ...
        $toolbar->add_item(
            new ToolbarItem(
                Translation::get('Edit', null, Utilities::COMMON_LIBRARIES), new FontAwesomeGlyph('plus-circle'),
                $this->get_url(
