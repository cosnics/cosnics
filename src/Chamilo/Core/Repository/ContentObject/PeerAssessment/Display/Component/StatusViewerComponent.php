...
class StatusViewerComponent extends Manager
{
    ...
    private function render_closed_status($status, $attempt)
    {
        ...
        $item->set_href(
            $this->get_url(
                array(
                    ...
                    self::PARAM_ATTEMPT => $status->get_attempt_id(), self::PARAM_USER => $status->get_user_id()
                )
            )
        );
        ...
        if ($status->get_closed() === null)
        {
            ...
            $item->set_image(new FontAwesomeGlyph('lock', array('text-muted')));
        }
        else
        {
            if ($status->get_closed() < $attempt->get_end_date())
            {
                ...
                $item->set_image(
                    new FontAwesomeGlyph('lock', array('text-success'))
                );
            }
            else
            {
                ...
                $item->set_image(new FontAwesomeGlyph('lock', array('text-danger')));
                $item->set_href(null);
            }
        }
    ...
    {
        $item = new ToolbarItem(
            Translation::get('Details'), ($status->get_progress() > 0 ? new FontAwesomeGlyph('info-circle') :
            new FontAwesomeGlyph('info-circle', array('text-muted'))), $this->get_url(
            array(
                self::PARAM_ACTION => self::ACTION_VIEW_USER_STATUS, self::PARAM_ATTEMPT => $status->get_attempt_id(),
    ...
    private function render_tabs()
    {
        ...
        {
            $this->redirect(
                Translation::get('NoGroupsDefined'), 1,
