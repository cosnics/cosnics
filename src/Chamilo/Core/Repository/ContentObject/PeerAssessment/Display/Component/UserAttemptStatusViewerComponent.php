...
class UserAttemptStatusViewerComponent extends Manager implements TableSupport
{
    ...
    public function run()
    {
        ...
        if (!$enrolled && $settings->get_direct_subscribe_available() &&
            $settings->get_subscription_deadline() > time())
        {
    ...
    public function get_condition()
    {
        return new EqualityCondition(
            WeblcmsPeerAssessmentAttemptTracker::PROPERTY_PUBLICATION_ID, $this->publication_id
        );
    }
    ...
    public function render_action_bar()
    {
        ...
        $toolActions->addButton(
            new Button(
                ...
                Translation::get('List'), new FontAwesomeGlyph('list'),
                $this->get_url(array(self::PARAM_BROWSER => self::BROWSER_MODE_LIST))
            )
        );
        $toolActions->addButton(
            new Button(
                Translation::get('Table'), new FontAwesomeGlyph('table'),
    ...
    private function render_attempts_list(array $attempts)
    {
        ...
        foreach ($attempts as $a)
        {
            ...
            $info = sprintf(
                Translation::get('AttemptInfoDate'), date('d/m/Y', $a->get_start_date()),
