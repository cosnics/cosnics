...
class PeerAssessmentGroupForm extends FormValidator
{
    ...
    public function __construct(
        GroupSubscriptionService $groupSubscriptionService, $viewer, $group_id, $url, $type = self::FORM_TYPE_CREATE
    )
    {
            ...
            foreach ($values as $type => $elements)
            {
                foreach ($elements as $id)
                {
                    ...
                    elseif ($type == self::PARAM_PLATFORM_GROUP)
                    {
                        ...
                        {
                            $context_group_users =
                                $this->groupSubscriptionService->findUserIdsInGroupAndSubgroups($context_group);
