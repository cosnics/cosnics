...
class Display extends QuestionDisplay
{
    ...
    public function add_question_form()
    {
        ...
        $table_header[] = '<th class="list" style="text-align:center;">' . Translation :: get('MakeASelection') . '</th>';

        if ($question->get_display() == AssessmentMatchingQuestion :: DISPLAY_LIST)
        {
            $table_header[] = '<th class="nobg" style="width: 50px;"></th>';
            $table_header[] = '<th class="list"></th>';
    ...
    public function add_options()
    {
        ...
        for ($i = 0; $i < $maximum; $i ++)
        {
            ...
            else
            {
                if ($i == count($answers))
                {
                    $formvalidator->addElement('html', '<td colspan="3" rowspan="' . ($maximum - $i) . '"></td>');
                }
            }
