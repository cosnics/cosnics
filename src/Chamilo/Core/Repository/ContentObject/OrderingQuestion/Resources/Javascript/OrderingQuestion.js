...
$(function()
{
    ...
    {
        return $('.data_table > tbody > tr:first select[name*="order"]').html();
    }
    ...
    function addOption(ev, ui)
    {
        ...
        fieldAnswer = renderHtmlEditor('value[' + numberOfOptions + ']', parameters);
        fieldOrder = '<select name="order[' + numberOfOptions + ']">' + getSelectOptions() + '</select>';
        fieldFeedback = renderHtmlEditor('feedback[' + numberOfOptions + ']', parameters);
        ...
        string = '<tr id="option_' + numberOfOptions + '" class="' + rowClass + '"><td>' + fieldAnswer + '</td><td>' + fieldOrder + '</td><td>' + fieldFeedback + '</td><td>' + fieldScore + '</td><td>' + fieldDelete + '</td></tr>';

        $('.data_table > tbody').append(string);

        processItems();

        highestOptionValue = $('.data_table tbody tr:first select[name*="order"] option:last').val();
        $('.data_table > tbody > tr:last select[name*="order"]').val(highestOptionValue);
    }
