      ...
      <div class="criterium treenode-hover mod-responsive mod-entry-view" role="row" :class="feedbackVisibleClass">
            ...
            <div class="criterium-levels-wrapper">
                ...
                <ul class="criterium-levels">
                    <li v-for="{choice, isSelected} in getChoicesColumnData(ext, evaluation)" class="criterium-level mod-entry-view" :class="feedbackVisibleClass" role="gridcell" :aria-describedby="`criterium-${criterium.id}-title`">
                        <div v-if="preview" :aria-checked="choice.level.isDefault" class="criterium-level-header mod-entry-view" :class="{ 'is-selected': isSelected }">
                            ...
                            <span v-if="useScores" class="score-number" :class="{ 'is-selected': isSelected }" :aria-label="`${ choice.score } ${ $t('points') }`"><!--<i class="check fa"/>-->{{ choice.score }}</span>
                        ...
                        <button v-else role="radio" :aria-checked="isSelected" class="criterium-level-header mod-entry-view btn-score-number" :class="{ 'is-selected': isSelected }" @click="selectLevel(choice.level)">
                            <div class="criterium-level-title mod-entry-view" :class="{ 'is-selected': isSelected }">
                                {{choice.title}}
                            </div>
                            <span v-if="useScores" class="score-number" :class="{ 'is-selected': isSelected }" :aria-label="`${ choice.score } ${ $t('points') }`"><!--<i class="check fa"/>-->{{ choice.score }}</span>
...
<script lang="ts">
    ...
    export default class CriteriumEntry extends Vue {
        ...
        @Prop({type: Number, default: 0}) readonly criteriumScore!: number;
