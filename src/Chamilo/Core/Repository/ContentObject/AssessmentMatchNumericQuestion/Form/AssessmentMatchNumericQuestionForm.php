...
class AssessmentMatchNumericQuestionForm extends ContentObjectForm
{
    ...
    protected function addOptionsButtons()
    {
        ...
        $buttonToolBar->addItem(
            new Button(
                Translation::get('AddMatchNumericOption'), new FontAwesomeGlyph('plus'), ' ',
    ...
    private function add_options()
    {
        ...
        for ($option_number = 0; $option_number < $number_of_options; $option_number ++)
        {
            if (!in_array($option_number, $skippedOptions))
            {
                ...
                // Score
                $this->addElement(
                    'text', AssessmentMatchNumericQuestionOption::PROPERTY_SCORE . '[' . $option_number . ']',
                ...
                // Tolerance
                $this->addElement(
                    'text', AssessmentMatchNumericQuestionOption::PROPERTY_TOLERANCE . '[' . $option_number . ']',
                    Translation::get('Tolerance'), 'size="2"  class="input_numeric form-control"'
                );
    ...
    protected function buildBasicQuestionForm()
    {
        ...
        $this->addElement(
            'select', AssessmentMatchNumericQuestion::PROPERTY_TOLERANCE_TYPE, Translation::get('ToleranceType'),
            $selectOptions
        );
        ...
        if (!$this->isSubmitted())
        {
            ...
            if ($object->get_number_of_options() != 0)
            {
                ...
                foreach ($options as $index => $option)
                {
                    ...
                    $defaults[AssessmentMatchNumericQuestionOption::PROPERTY_TOLERANCE][$index] =
                        $option->get_tolerance() ? $option->get_tolerance() : 0;
                    $defaults[AssessmentMatchNumericQuestionOption::PROPERTY_SCORE][$index] = !is_null(
                        $option->get_score()
                    ) ? $option->get_score() : 1;
                    $defaults[AssessmentMatchNumericQuestionOption::PROPERTY_FEEDBACK][$index] =
                        $option->get_feedback();
                }
