...
class BackupUserForm extends FormValidator
{
    ...
    public function buildForm(): void
    {
        ...
        // Platform admin
        if ($this->user->isPlatformAdmin() && $this->user->getId() == $this->actionUser->getId() &&
     ...
     * @throws \QuickformException
     */
    public function checkPasswordRequirements(): bool
    {
    ...
    public function create_user(): bool
    {
        ...
        if (DataManager::is_username_available(
            $values[User::PROPERTY_USERNAME], $values[DataClass::PROPERTY_ID]
        ))
        {
            $user->setId($values[DataClass::PROPERTY_ID]);
            ...
            if ($value)
            {
                Event::trigger(
                    ...
                    ['target_user_id' => $user->getId(), 'action_user_id' => $this->actionUser->getId()]
                );
            }
     ...
     */
    public function send_email($user): void
    {
        $configurationConsulter = $this->getConfigurationConsulter();
    ...
    public function setDefaults($defaultValues = [], $filter = null)
    {
        ...
        if ($this->formType == self::TYPE_EDIT)
        {
            ...
            {
                $defaultValues[self::PROPERTY_TIME_PERIOD_FOREVER] = 0;
        ...
        else
        {
            ...
            $defaultValues['pw']['pass'] = $user->get_password();

            $defaultValues[User::PROPERTY_DATABASE_QUOTA] = '300';
            $defaultValues[User::PROPERTY_DISK_QUOTA] = '209715200';
        }

        $defaultValues['admin'][User::PROPERTY_PLATFORMADMIN] = $user->getPlatformAdmin();
        $defaultValues['mail']['send_mail'] = 0;
        ...
        $defaultValues['active'][User::PROPERTY_ACTIVE] = !is_null($user->get_active()) ? $user->get_active() : 1;
        ...
        if ($value)
        {
            Event::trigger(
                'Update', Manager::CONTEXT, [
                    ...
                    ChangesTracker::PROPERTY_USER_ID => $this->actionUser->getId()
                ]
            );
        }
