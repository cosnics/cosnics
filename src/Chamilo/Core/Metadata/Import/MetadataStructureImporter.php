...
class MetadataStructureImporter
{
     ...
     * 
     * @param \Chamilo\Core\Metadata\MetadataStructureImportParser $metadata_structure_import_parser
     *
     ...
     * 
     * @return \Chamilo\Core\Metadata\MetadataStructureImportParser
     */
    ...
    protected function create_controlled_vocabularies()
    {
        ...
        {
            $existing_controlled_vocabulary = \Chamilo\Core\Metadata\ControlledVocabulary\Storage\DataManager :: retrieve_controlled_vocabulary_by_value(
    ...
    protected function create_schemas()
    {
        ...
        {
            $existing_schema = \Chamilo\Core\Metadata\Schema\Storage\DataManager :: retrieve_schema_by_namespace(
    ...
    protected function create_elements_for_schema(Schema $schema)
    {
        ...
        {
            $existing_element = \Chamilo\Core\Metadata\Storage\DataManager :: retrieve_element_by_schema_id_and_name(
                $schema->get_id(), 
                $element->get_name());
    ...
    protected function create_attributes_for_schema(Schema $schema)
    {
        ...
        {
            $existing_attribute = \Chamilo\Core\Metadata\Attribute\Storage\DataManager :: retrieve_attribute_by_schema_id_and_name(
                $schema->get_id(), 
                $attribute->get_name());
    ...
    protected function create_controlled_vocabulary_for_elements($elements)
    {
        foreach ($elements as $element)
        {
            ...
            {
                $element_controlled_vocabulary = \Chamilo\Core\Metadata\Element\Storage\DataManager :: retrieve_element_controlled_vocabulary_by_element_and_controlled_vocabulary(
                    $element->get_id(), 
    ...
    protected function create_controlled_vocabulary_for_attributes($attributes)
    {
        foreach ($attributes as $attribute)
        {
            ...
            foreach ($controlled_vocabulary as $controlled_vocabulary_object)
            {
                $attribute_controlled_vocabulary = \Chamilo\Core\Metadata\Attribute\Storage\DataManager :: retrieve_attribute_controlled_vocabulary_by_attribute_and_controlled_vocabulary(
                    $attribute->get_id(), 
                    $controlled_vocabulary_object->get_id());
    ...
    protected function create_nested_elements_for_elements($elements)
    {
        foreach ($elements as $element)
        {
            ...
            {
                $element_nesting = \Chamilo\Core\Metadata\Element\Storage\DataManager :: retrieve_element_nesting_by_parent_and_child_element(
                    $element->get_id(), 
    ...
    protected function create_default_values_for_elements($elements)
    {
        foreach ($elements as $element)
        {
            ...
            foreach ($default_values as $default_value)
            {
                ...
                $existing_default_value = \Chamilo\Core\Metadata\Value\Element\Storage\DataManager :: retrieve_default_element_value_by_value_or_controlled_vocabulary(
                    $element->get_id(), 
    ...
    protected function create_default_values_for_attributes($attributes)
    {
        foreach ($attributes as $attribute)
        {
            ...
            foreach ($default_values as $default_value)
            {
                ...
                $existing_default_value = \Chamilo\Core\Metadata\Value\Attribute\Storage\DataManager :: retrieve_default_attribute_value_by_value_or_controlled_vocabulary(
                    $attribute->get_id(), 
                    $default_value->get_value(), 
                    $controlled_vocabulary_id);
