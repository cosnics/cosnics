<?php
namespace Chamilo\Core\Metadata\Value\Form;
...
use Chamilo\Core\Metadata\FixedElementsProvider;
use Chamilo\Core\Metadata\Schema\Storage\DataClass\Schema;
use Chamilo\Core\Metadata\Value\Storage\DataManager;
...
class ValueEditorFormBuilder
{
     ...
     * 
     * @param \Chamilo\Core\Metadata\FixedElementsProvider $fixed_elements_provider
     *
    ...
    protected function add_schemas()
    {
        ...
        $schemas = \Chamilo\Core\Metadata\Schema\Storage\DataManager :: retrieves(Schema :: class_name());
        while ($schema = $schemas->next_result())
        {
            if (\Chamilo\Core\Metadata\Element\Storage\DataManager :: count_elements_for_schema($schema->get_id()) == 0)
            {
                continue;
            }
    ...
    {
        $elements = \Chamilo\Core\Metadata\Element\Storage\DataManager :: retrieve_parent_elements_from_schema(
            $schema->get_id());
    ...
    protected function add_dynamic_elements(array $elements, Schema $schema)
    {
        ...
        foreach ($elements as $element)
        {
            ...
            if (\Chamilo\Core\Metadata\Element\Storage\DataManager :: element_has_controlled_vocabulary(
                $element->get_id()))
            {
                $vocabulary = \Chamilo\Core\Metadata\Element\Storage\DataManager :: retrieve_controlled_vocabulary_terms_from_element(
                    $element->get_id());
     ...
     * @param \Chamilo\Core\Metadata\schema\storage\data_class\Schema $schema
     */
    ...
    protected function add_attributes_for_element(array &$group_elements, Element $element)
    {
        ...
        $attributes = \Chamilo\Core\Metadata\Attribute\Storage\DataManager :: retrieve_attributes_for_element(
            $element->get_id());
        ...
        while ($attribute = $attributes->next_result())
        {
            ...
            if (\Chamilo\Core\Metadata\Storage\DataManager :: attribute_has_controlled_vocabulary($attribute->get_id()))
            {
                $vocabulary = \Chamilo\Core\Metadata\Attribute\Storage\DataManager :: retrieve_controlled_vocabulary_terms_from_attribute(
                    $attribute->get_id());
