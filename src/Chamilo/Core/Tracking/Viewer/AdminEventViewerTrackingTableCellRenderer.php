...
class AdminEventViewerTrackingTableCellRenderer
{
    ...
    public function get_modification_links($tracker)
    {
        ...
        $toolbar->add_item(
            new ToolbarItem(
                ($tracker->get_active() == 1) ? Translation :: get('Hide', null, Utilities :: COMMON_LIBRARIES) : Translation :: get(
                    'Visible', 
                    null, 
                    Utilities :: COMMON_LIBRARIES), 
                ($tracker->get_active() == 1) ? (Theme :: getInstance()->getCommonImagePath() . 'action_visible.png') : (Theme :: getInstance()->getCommonImagePath() .
    ...
    public function render_cell($property, $tracker)
    {
        ...
        {
            /*
             * case Event :: PROPERTY_NAME: return '<a href="' . $this->browser->get_event_viewer_url($event) . '">' .
    ...
    public function get_properties()
    {
        return array(
            ...
            TrackerRegistration :: PROPERTY_TRACKER, 
            TrackerRegistration :: PROPERTY_CONTEXT);
    }
}
