...
class SettingsConnector implements SettingsConnectorInterface
{
    ...
    public function __construct(
        protected LanguageConsulter $languageConsulter, protected MailerFactory $mailerFactory,
        protected Translator $translator, protected array $userRights
    )
    {
    }
    ...
    public function getCalendarFirstDaysOfWeek(): array
    {
        ...
        foreach ($dayOptions as $value => $label) {
            $day = new stdClass();
            $day->value = $value;
            $day->label = $this->translator->trans($label, [], StringUtilities::LIBRARIES);
            $day->attributes = [];
            $days[] = $day;
        }
    ...
    public function getCalendarViews(): array
    {
        ...
        foreach ($viewOptions as $viewOption) {
            ...
            $view->label = $this->translator->trans($viewOption, [], StringUtilities::LIBRARIES);
            $view->attributes = [];
            $views[] = $view;
        }
    ...
    public function getLanguages(): array
    {
        ...
        foreach ($this->languageConsulter->getLanguages() as $isoCode => $name) {
    ...
    {
        return array_key_exists($rightName, $this->userRights) && $this->userRights[$rightName];
    }
