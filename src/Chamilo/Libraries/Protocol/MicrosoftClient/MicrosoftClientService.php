 ...
 * Redirect URI: when registering a redirect URI on https://apps.dev.microsoft.com/ just use the base URL of the Chamilo
 * site, i.e my.chamilo.com/index.php.
 *
...
class MicrosoftClientService
{
    ...
    /**
     * The Microsoft Graph client with base URL https://graph.microsoft.com/v1.0/me/ where 'me' resolves to the logged
     * in user.
     *
     * @var \GuzzleHttp\Client
     */
     ...
     * Why do we acctually need parameter 'state' and Kernel::handleOAuth2(). The Microsoft OAuth2 does not allow reply
     * URI's which
     ...
     * @param string $endpoint Endpoint of Microsoft Graph REST API, e.g. drive/root/children for listing content of
     *        root directory.
    ...
    private function getAzureActiveDirectoryClient()
    {
        ...
        {
            $this->azureActiveDirectoryClient = new \GuzzleHttp\Client(
                array(
                    'base_url' => 'https://login.microsoftonline.com/' .
                         $this->microsoftClientSettingsProvider->getTenant() . '/oauth2/v2.0/'));
        }
    ...
    /**
     * Extends the scopes provided by MicrosoftClientSettingsProvider by scope 'offline_access' which enables refreshing
     * of access tokens.
     */
