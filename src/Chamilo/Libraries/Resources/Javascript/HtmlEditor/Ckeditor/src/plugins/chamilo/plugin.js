(function()
{
    ...
    var openRepoViewerCmd = {
        exec : function(editor)
        {
            ...
            var height = editor.config['filebrowserChamiloWindowHeight'] || editor.config.filebrowserWindowHeight || '70%';
    ...
    var openDialog = {
        exec : function(editor)
        {
            ...
            {
                var realElement = CKEDITOR.dom.element.createFromHtml(decodeURIComponent(element.getAttribute('data-cke-realelement')), this.document);
                var objectType = realElement.getAttribute('type');
                if (!CKEDITOR.dialog.exists(objectType + 'Dialog'))
                {
                    CKEDITOR.dialog.add(objectType + 'Dialog', web_path + 'repository/content_object/' + objectType + '/resources/javascript/html_editor/ckeditor/dialog.js');
                }
    ...
    {
        return editor.createChamiloFakeParserElement(realElement, 'cke_chamilo_' + realElement.attributes.type, 'chamilo', isResizable);
    }
...
//                ,               icon : this.path + 'chamilo.png'
            ...
            if (editor.contextMenu)
            {
                ...
                {
                    if (element && element.is('img') && !element.isReadOnly() && element.data('cke-real-element-type') == 'chamilo')
                    {
                        return {
