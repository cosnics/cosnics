...
class XmlFormParser
{
     ...
     */
    protected function create_and_add_validation_rule(
    ...
    )
    {
        $validation_rule = new XmlFormParserValidationRule(
            $element, $message, $type, $format, $validation, $reset, $force
        );
    ...
    protected function create_radio_element($elementNode, $elementName, $elementTitle)
    {
        ...
        foreach ($element_options as $option_value => $option_name)
        {
            $group_elements[] = $this->form_builder->createElement(
                self::ELEMENT_TYPE_RADIO, $elementName, null, $option_name, $option_value
            );
        }

        $this->create_and_add_element(
            self::ELEMENT_TYPE_GROUP, $elementName, $elementTitle, $group_elements, '', false
        );
    }
    ...
    protected function translate($translation_variable, $parameters = [])
    {
        $translation_variable =
            (string) StringUtilities::getInstance()->createString($translation_variable)->upperCamelize();
