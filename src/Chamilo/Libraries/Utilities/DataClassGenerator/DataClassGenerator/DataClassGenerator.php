...
class DataClassGenerator
{
    ...
    public function generate_data_class($xml_definition, $author, $package, $application)
    {
        ...
        if (! is_dir($location . ClassnameUtilities :: getInstance()->namespaceToPath($xml_definition['namespace']) . '/php/lib/'))
            mkdir($location . ClassnameUtilities :: getInstance()->namespaceToPath($xml_definition['namespace']) . '/php/lib/', 0777, true);

        $file = fopen(
            $location . ClassnameUtilities :: getInstance()->namespaceToPath($xml_definition['namespace']) . '/php/lib/' . $xml_definition['name'] .
                 '.class.php',
                'w+');
