...
class RecordResultCache extends RecordCache
{
     ...
     * @return boolean
     */
    public static function add($record, RecordRetrieveParameters $parameters = null)
    {
        if (! is_array($record))
        {
            throw new Exception(
        ...
        {
            self :: set_cache($parameters->hash(), $record);
        }
     ...
     * empty results
     *
     * @param \Chamilo\Libraries\Storage\Exception\NoRecordException $exception
     * @return boolean
     */
    public static function no_result(NoRecordException $exception)
    {
        self :: set_cache($exception->get_parameters()->hash(), false);
        return true;
    }
}
