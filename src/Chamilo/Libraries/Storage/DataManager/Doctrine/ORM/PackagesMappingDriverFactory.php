...
/**
 * Mapping driver factory based on a given set of packages. Parses the configuration of the packages and creates
...
class PackagesMappingDriverFactory
{
     ...
     * Constructor
     *
     * @param MappingDriverFactory $mappingDriverFactory
    ...
    /**
     * Creates the mapping driver for the given packages. The packages are defined with the namespace and the location
     * of the config file.
     *
     * @example $packages = array(
     *      'application/weblcms' => Path::getInstance()->getBasePath() . 'application/weblcms/resources/configuration/config.yml'
     * )
     *
    ...
    public function createMappingDriverForPackages($packages = array())
    {
        ...
        if(
            !array_key_exists('doctrine', $packageConfiguration) ||
