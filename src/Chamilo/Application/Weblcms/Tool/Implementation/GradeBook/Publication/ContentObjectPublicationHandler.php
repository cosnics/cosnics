<?php
namespace Chamilo\Application\Weblcms\Tool\Implementation\GradeBook\Publication;
...
/**
 * Custom publication handler for the gradebook tool
 *
...
class ContentObjectPublicationHandler extends \Chamilo\Application\Weblcms\Publication\ContentObjectPublicationHandler
{
    ...
    protected function createPublicationForContentObject($contentObject)
    {
        ...
        {
            $gradebookPublication = new Publication();
            $gradebookPublication->setPublicationId($publication->getId());

            if (!$gradebookPublication->create())
            {
                throw new \RuntimeException(
                    'Could not create a gradebook publication for publication with id ' . $publication->getId());
            }
        }
