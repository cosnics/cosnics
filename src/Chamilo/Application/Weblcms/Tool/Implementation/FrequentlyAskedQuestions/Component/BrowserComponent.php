...
class BrowserComponent extends Manager implements DelegateComponent
{
    ...
    public function run()
    {
        ...
        $conditions[] = new EqualityCondition(
            ...
            new StaticConditionVariable('faq'));
    ...
    {
        $table = new HTML_Table(array('class' => 'faq', 'cellspacing' => 1));
    ...
    public function create_table_header($table)
    {
        ...
        $table->setHeaderContents(
            ...
            Translation :: get(
                "FAQ",
                null,
                ContentObject :: get_content_object_type_namespace('FrequentlyAskedQuestions')));
    ...
    public function create_table_forums($table, &$row, $parent)
    {
        ...
        $subselect_condition = new EqualityCondition(
            new PropertyConditionVariable(ContentObject :: class_name(), ContentObject :: PROPERTY_TYPE),
            new StaticConditionVariable(FrequentlyAskedQuestions :: class_name()));
        ...
        if ($publications)
        {
            ...
            while ($publication = $publications->next_result())
            {
                     ...
                     ! $this->is_allowed(WeblcmsRights :: EDIT_RIGHT)))
                {
                    ...
                    $faq = \Chamilo\Core\Repository\Storage\DataManager :: retrieve_by_id(
                        FrequentlyAskedQuestions :: class_name(),
                        $publication[ContentObjectPublication :: PROPERTY_CONTENT_OBJECT_ID]);

                    if ($faq->get_locked() && ! $this->is_allowed(WeblcmsRights :: EDIT_RIGHT))
                    {
                        $title = $faq->get_title() . '<br/>' . strip_tags($faq->get_description());
                    }
                    else
                    {
                        $title = '<a href="' .
                             $this->get_url(
                                array(
                                    ...
                                    \Chamilo\Core\Repository\Display\Manager :: PARAM_ACTION => \Chamilo\Core\Repository\ContentObject\FrequentlyAskedQuestions\Display\Manager :: ACTION_VIEW_COMPLEX_CONTENT_OBJECT,
                                    \Chamilo\Application\Weblcms\Tool\Manager :: PARAM_PUBLICATION_ID => $publication[ContentObjectPublication :: PROPERTY_ID])) .
                             '">' . $faq->get_title() . '</a><br />' . strip_tags($faq->get_description());
                    }
                    if ($publication[ContentObjectPublication :: PROPERTY_HIDDEN])
                    {
    ...
    public function get_forum_actions($publication, $first, $last)
    {
        ...
        $faq_namespace = ContentObject :: get_content_object_type_namespace('FrequentlyAskedQuestions');

        if (! $faq->get_locked())
        {
            $subscribed = FrequentlyAskedQuestionsDataManager :: retrieve_subscribe(
                $faq->get_id(),
                $this->get_user_id());
            ...
            if (! $subscribed)
            {
                ...
                $parameters[self :: PARAM_FAQ_ID] = $faq->get_id();
                ...
                $toolbar->add_item(
                    new ToolbarItem(
                        Translation :: get('Subscribe', null, $faq_namespace),
                        Theme :: getInstance()->getImagePath($faq_namespace, 'Action/Mail'),
                        $this->get_url($parameters),
                        ToolbarItem :: DISPLAY_ICON,
                        true));
            }
            else
            {
                ...
                $parameters[self :: PARAM_SUBSCRIBE_ID] = $faq->get_id();
                $parameters[\Chamilo\Application\Weblcms\Tool\Manager :: PARAM_ACTION] = self :: ACTION_FORUM_UNSUBSCRIBE;
                $parameters[\Chamilo\Application\Weblcms\Tool\Manager :: PARAM_PUBLICATION_ID] = $publication[ContentObjectPublication :: PROPERTY_ID];

                $toolbar->add_item(
                    new ToolbarItem(
                        Translation :: get('UnSubscribe', null, $faq_namespace),
                        Theme :: getInstance()->getImagePath($faq_namespace, 'Action/Unmail'),
                        $this->get_url($parameters),
                        ToolbarItem :: DISPLAY_ICON,
                        true));
            }
