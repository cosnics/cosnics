...
class DoctrineExtension
{
    ...
    public function retrieve_results_by_assignment(DataClassRetrievesParameters $params, $entryClassName)
    {
        ...
        $query = 'SELECT ' . $er_alias . '.' . Request::PROPERTY_ID . ' AS ' . Request::PROPERTY_REQUEST_ID . ', ' .
            ...
            $ast_alias . '.' . Entry::PROPERTY_ID . ', ' . $rco_alias . '.' .
            ContentObject::PROPERTY_TITLE . ', ' . $rco_alias . '.' . ContentObject::PROPERTY_DESCRIPTION . ', ' .
            $rco_alias . '.' . ContentObject::PROPERTY_TYPE . ', ' . $ast_alias . '.' .
            Entry::PROPERTY_ENTITY_TYPE . ', ' . $ast_alias . '.' .
            Entry::PROPERTY_SUBMITTED . ', ' . $usr_alias . '.' . User::PROPERTY_FIRSTNAME . ', ' .
            $usr_alias . '.' . User::PROPERTY_LASTNAME . ' FROM ';

        $query .= $this->create_by_assignment_base_query($params->get_condition(), $entryClassName);
        ...
        return new DataClassResultSet($this->query($query), ContentObject::class);
    }

    public function count_results_content_objects_by_assignment($condition, $entryClassName)
    {
        $query = 'SELECT COUNT(*) FROM ';
        $query .= $this->create_by_assignment_base_query($condition, $entryClassName);
    ...
    private function create_by_assignment_base_query($condition, $entryClassName)
    {
        ...
        // submission tracker
        $ast_table = $entryClassName::get_table_name();
        $ast_alias = $tdm->get_alias($entryClassName::get_table_name());
        $ast_cid = $tdm->escape_column_name(Entry::PROPERTY_CONTENT_OBJECT_ID, $ast_alias);
        $ast_sid = $tdm->escape_column_name(Entry::PROPERTY_USER_ID, $ast_alias);
    ...
    public function retrieve_results_content_objects($guids)
    {
        ...
        {
            $tables[${$table_alias . $ref_table}] =
                ${$table_alias . $ref_table} . ' AS ' . ${$table_alias . $ref_alias};
        }
