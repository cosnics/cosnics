...
class StatisticsViewerComponent extends Manager
{
    ...
    public function run()
    {
        ...
        switch ($stats_action)
        {
            ...
            case self :: ACTION_DELETE_LPI_ATTEMPTS :
                $this->delete_lpi_attempts_from_item(
                    Request:: get('item_id'), Request:: get(self::PARAM_ATTEMPT_ID)
                );
    ...
    function get_menu($root_object_id, $selected_object_id, $pid, $lpi_tracker_data)
    {
        $url = '?go=courseviewer&course=' . Request:: get('course') .
            '&application=weblcms&tool=learning_path&tool_action=view&publication=' . $pid . '&' .
            self :: PARAM_LP_STEP .
            '=%s';
