...
class ToolVisibilityChangerComponent extends Manager
{
    ...
    public function run()
    {
        ...
        $visibility = $this->getRequest()->query->get(self::PARAM_VISIBILITY);
        ...
        if (!$courseSettingsController->canChangeCourseSetting(
            $this->get_course(), CourseSetting::COURSE_SETTING_TOOL_VISIBLE, $toolRegistration->getId()
        ))
        {
            throw new NotAllowedException();
        }
        ...
        $this->redirectWithMessage(Translation::get($message), false, [self::PARAM_ACTION => self::ACTION_BROWSE]);
    }
}
