...
use Chamilo\Application\Weblcms\Tool\Implementation\CourseGroup\Extension\Office365\Storage\Repository\AccessTokenRepository;
...
class Test extends ChamiloTestCase
{
    ...
    public function testGetApplicationAccessToken()
    {
        ...
        $this->localSettingMock->expects($this->once())
            ->method('get')
            ->with('access_token', 'Chamilo\Application\Weblcms\Tool\Implementation\CourseGroup\Extension\Office365')
    ...
    public function testGetDelegatedAccessToken()
    {
        ...
        $this->sessionUtilitiesMock->expects($this->once())
            ->method('get')
            ->with('office365_delegated_access_token')
            ->will($this->returnValue($accessTokenData));
    ...
    public function testStoreDelegatedAccessToken()
    {
        ...
        $this->sessionUtilitiesMock->expects($this->once())
            ->method('register')
            ->with(
                'office365_delegated_access_token', json_encode($accessToken->jsonSerialize())
            );
