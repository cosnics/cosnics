                                                  ...
                                                  // DelegateComponent
{
    ...
    public function generate_attachment_placeholder($attachment, $type = null)
    {
        ...
        if (self :: is_downloadable($attachment))
        {
            ...
            $html[] = '<img src="' . Theme :: getInstance()->getCommonImagePath('action_download') . '" title="' . Translation :: get(
                'Download') . '"/>';
            $html[] = '</a>';
        }
        else
        {
            $html[] = '<a>';
            $html[] = '<img src="' . Theme :: getInstance()->getCommonImagePath('action_download_na') . '" title="' . Translation :: get(
                'DownloadNotPossible') . '"/>';
            $html[] = '</a>';
        }
            ...
            foreach (explode(',', $this->get_assignment()->get_automatic_feedback_co_ids()) as $content_object_id)
            {
                            ...
                            new StaticConditionVariable($content_object_id)))))
                {
                    $html[] = '<li>' . $this->generate_attachment_placeholder(
                        \Chamilo\Core\Repository\Storage\DataManager :: retrieve_content_object($content_object_id),
