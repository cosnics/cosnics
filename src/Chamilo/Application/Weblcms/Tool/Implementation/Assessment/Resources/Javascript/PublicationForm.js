(function($) {
	function checkFeedback(ev, ui) {
		var feedbackSummary = $('input[name=feedback_summary]:checkbox:checked')
				.val();
		var feedbackPerPage = $(
				'input[name=feedback_per_page]:checkbox:checked').val();
			...
			.ready(
					function() {
						$(document)
								.on(
										'click',
										'input[name=feedback_summary]:checkbox, input[name=feedback_per_page]:checkbox',
										checkFeedback);
						checkFeedback();
					});
