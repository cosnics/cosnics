...
class ZipAndDownloadComponent extends Manager
{
    ...
    public function run()
    {
        ...
        $archiveSafeName = StringUtilities::getInstance()->createString($archiveName)->toAscii()->replace('/', '-')->replace(
            '\\',
        ...
        foreach ($category_folder_mapping as $category_id => $dir)
        {
            ...
            else
            {
                if ($category_id == 0)
                {
                    ...
                    $location_id = WeblcmsRights::getInstance()->get_weblcms_location_by_identifier_from_courses_subtree(
