(function($) {
	var handle_drop = function(ev, ui) {
		...
		$
				.post(
						"./index.php?go=courseviewer&course="
								+ course_code
								+ "&tool=course_sections&application=weblcms&tool_action=change_section",
						{
							target : target,
							source : source
						}, function(data) {
	...
	var handle_visible_clicked = function(ev, ui) {
		var visible_img = 'layout/aqua/images/common/Action/Visible.png';
		var invisible_img = 'layout/aqua/images/common/Action/Invisible.png';
		...
		$
				.post(
						"./application/weblcms/php/ajax/change_course_module_visibility.php",
						{
							tool : tool,
							visible : new_visible,
							course : course_code
						...
						function(data) {
							if (data.length == 0) {
								...
								if (new_visible == 0) {
									...
									imgtag.attr('src', is_visible.replace(
											'Action/Visible.png',
											'Action/Invisible.png'));
								} else {
								...
								if (disabled_block.attr('class') == 'disabledblock') {
									var clear_div = new_parent
											.children(".clear")[0];
									if (clear_div) {
										new_parent.children(".clear")[0].remove;
									}

									var message = $('div.normal-message',
											new_parent);
									if (message) {
										message.remove();
									}
									...
									if (old_parent.children('.tool').size() == 0) {
										old_parent
												.prepend('<div class="normal-message">'
														+ getTranslation(
																'NoToolsAvailable',
																'weblcms')
														+ '</div>');
