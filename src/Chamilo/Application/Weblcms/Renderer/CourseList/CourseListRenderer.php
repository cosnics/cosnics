        ...
        if (count($courses) > 0)
        {
            ...
            if ($this->get_new_publication_icons())
            {
                ...
                DataManager::fill_new_publications_cache(
                    $this->get_user(), DataManager::create_courses_array($courses)
                );
            }
            ...
            foreach ($courses as $course)
            {
                ...
                $html[] =
                    '<li class="list-group-item"><a href="' . htmlspecialchars($this->get_course_url($course)) . '"' .
    ...
    protected function display_new_publication_icons(Course $course)
    {
        ...
        foreach ($this->getTools() as $tool)
        {
            ...
            $visible = $course_settings_controller->get_course_setting(
                $course, CourseSetting::COURSE_SETTING_TOOL_VISIBLE, $tool->get_id()
            );
            ...
            {
                $identGlyph = new NamespaceIdentGlyph($tool->getContext(), false, true, false);
    ...
    protected function retrieve_courses()
    {
        if (!isset($this->retrievedCourses))
        {
            $this->retrievedCourses = CourseDataManager::retrieve_all_courses_from_user(
                $this->get_user(), $this->get_retrieve_courses_condition()
            );
        }
