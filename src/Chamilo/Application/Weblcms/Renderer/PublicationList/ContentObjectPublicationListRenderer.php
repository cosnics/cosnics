...
abstract class ContentObjectPublicationListRenderer
{
    ...
    public function render_up_action($publication, $first = false)
    {
        if (!$first)
        {
            ...
            $glyph = new FontAwesomeGlyph('sort-up');

            return '<a href="' . $up_url . '">' . $glyph->render() . '</a>';
        }
        else
        {
            $glyph = new FontAwesomeGlyph('sort-up', array('text-muted'));
    ...
    public function render_down_action($publication, $last = false)
    {
        if (!$last)
        {
            ...
            $glyph = new FontAwesomeGlyph('sort-down');

            return '<a href="' . $down_url . '">' . $glyph->render() . '</a>';
        }
        else
        {
            $glyph = new FontAwesomeGlyph('sort-down', array('text-muted'));
    ...
    public function get_publication_actions($publication, $show_move = true, $ascending = true)
    {
        ...
        $toolbar->add_item(
            new ToolbarItem(
                Translation::get('Details', null, Utilities::COMMON_LIBRARIES), new FontAwesomeGlyph('info-circle'),
