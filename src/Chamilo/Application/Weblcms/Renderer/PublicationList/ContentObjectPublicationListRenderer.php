...
abstract class ContentObjectPublicationListRenderer
{
    ...
    public function render_down_action($publication, $last = false)
    {
        if (!$last)
        {
            ...
            $down_link =
                '<a href="' . $down_url . '"><img src="' . Theme::getInstance()->getCommonImagePath($down_img) .
                '"  alt=""/></a>';
        }
        else
        {
    ...
    public function render_edit_action($publication)
    {
        ...
        $edit_link =
            '<a href="' . $edit_url . '"><img src="' . Theme::getInstance()->getCommonImagePath('Action/Edit') .
            '"  alt=""/></a>';
            ...
            if (count($attachments) > 0)
            {
                ...
                foreach ($attachments as $attachment)
                {
                    $html[] = '<li><a href="' . $this->tool_browser->get_url(
                        ...
                        '"><img src="' . $attachment->get_icon_path(Theme::ICON_MINI) . '" alt="' .
                        htmlentities(
    ...
    public function is_visible_for_target_users($publication)
    {
        return (!$publication[ContentObjectPublication::PROPERTY_HIDDEN] &&
            ($publication[ContentObjectPublication::PROPERTY_FROM_DATE] == 0 &&
                $publication[ContentObjectPublication::PROPERTY_TO_DATE] == 0) ||
            ($publication[ContentObjectPublication::PROPERTY_FROM_DATE] <=
    ...
    public function get_publication_actions($publication, $show_move = true, $ascending = true, $rendererParameters = []
    )
    {
...
//        );
        ...
        if ($has_edit_right)
        {
            ...
            if ($show_move)
            {
                $buttonGroup->addButton(
                    new Button(
                        Translation::get($first_row ? 'MoveUpNA' : 'MoveUp', null, Utilities::COMMON_LIBRARIES),
                        new FontAwesomeGlyph('arrow-up'),
                        $first_row ? null : $this->get_url(
                            array_merge(
                                $rendererParameters,
                                array(
                                    \Chamilo\Application\Weblcms\Tool\Manager::PARAM_ACTION => \Chamilo\Application\Weblcms\Tool\Manager::ACTION_MOVE,
                ...
                $buttonGroup->addButton(
                    new Button(
                        Translation::get($last_row ? 'MoveDownNA' : 'MoveDown', null, Utilities::COMMON_LIBRARIES),
                        new FontAwesomeGlyph('arrow-down'),
                        $last_row ? null : $this->get_url(
                            array_merge(
                                $rendererParameters,
                                array(
                                    \Chamilo\Application\Weblcms\Tool\Manager::PARAM_ACTION => \Chamilo\Application\Weblcms\Tool\Manager::ACTION_MOVE,
        ...
        if (method_exists($this->get_tool_browser()->get_parent(), 'add_content_object_publication_actions_dropdown'))
        {
            $this->get_tool_browser()->get_parent()->add_content_object_publication_actions_dropdown($buttonGroup, $dropdownButton, $publication);
        }
