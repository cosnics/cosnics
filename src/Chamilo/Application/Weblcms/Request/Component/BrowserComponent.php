...
class BrowserComponent extends Manager implements BreadcrumbLessComponentInterface
{
    ...
    public function run()
    {
        ...
        $user_requests = DataManager::count(Request::class, new StorageParameters(condition: $condition));
        ...
        if ($user_requests > 0 || Rights::getInstance()->request_is_allowed())
        {
            ...
            if ($user_requests > 0)
            {
                $totalNumberOfItems = DataManager::count(
                    \Chamilo\Core\Repository\Quota\Storage\DataClass\Request::class, new StorageParameters(
            ...
            if (Rights::getInstance()->request_is_allowed())
            {
                ...
                if (DataManager::count(Request::class, new StorageParameters(condition: $condition)) > 0)
                {
                    $tabs->add(
                        new ContentTab(
    ...
    protected function renderManagementRequestTable(int $requestType): string
    {
        $totalNumberOfItems = DataManager::count(
            ...
            new StorageParameters(condition: $this->getRequestCondition($requestType))
        );
        $requestTableRenderer = $this->getUserRequestTableRenderer();
