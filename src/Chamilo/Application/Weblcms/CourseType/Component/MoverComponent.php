...
class MoverComponent extends Manager
{
    ...
    public function run()
    {
        ...
        $message = $this->get_result(
            $failures, count($course_types), 'CourseTypeNotMoved', 'CourseTypesNotMoved', 'CourseTypeMoved',
            'CourseTypesMoved'
        );

        $this->redirectWithMessage(
            $message, ($failures > 0), [self::PARAM_ACTION => self::ACTION_BROWSE], [self::PARAM_COURSE_TYPE_ID]
        );
    }
