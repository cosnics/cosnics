...
class CourseSettingsController
{
    ...
    public function convert_course_settings_to_values_array(Dataclass $base_object)
    {
        ...
        foreach ($available_settings as $available_setting)
        {
            ...
            if ($available_setting[CourseSetting::PROPERTY_GLOBAL_SETTING])
            {
                $course_settings[self::SETTING_PARAM_COURSE_SETTINGS][$available_setting[CourseSetting::PROPERTY_NAME]] =
                    $values;

                if ($locked)
                {
                    $course_settings[self::SETTING_PARAM_LOCKED_PREFIX .
                    self::SETTING_PARAM_COURSE_SETTINGS][$available_setting[CourseSetting::PROPERTY_NAME]] = 1;
                }
            }
            else
            {
                ...
                $course_settings[self::SETTING_PARAM_TOOL_SETTINGS][$tool][$available_setting[CourseSetting::PROPERTY_NAME]] =
                    $values;

                if ($locked)
                {
                    $course_settings[self::SETTING_PARAM_LOCKED_PREFIX .
                    self::SETTING_PARAM_TOOL_SETTINGS][$tool][$available_setting[CourseSetting::PROPERTY_NAME]] = 1;
                }
            }
        }
    ...
    {
        $value =
            $this->get_setting_for_object(self::SETTING_TYPE_COURSE_TYPE, $setting_name, $course_type_id, $tool_id);
        if (!is_null($value))
        {
            return $value;
        }
    ...
    protected function get_locked_value_for_setting_from_values_array($course_setting, $values)
    {
        return $this->get_values_for_setting_from_values_array(
            $course_setting, $values, self::SETTING_PARAM_LOCKED_PREFIX
        );
    }
    ...
    )
    {
        ...
        foreach ($available_settings as $available_setting)
        {
            ...
            if ($action == self::SETTING_ACTION_UPDATE)
            {
                try
                {
                    $course_setting_relation =
                        $base_object->update_course_setting_relation($available_setting, $locked);
                }
                catch (Exception $e)
                {
                    $succes = false;
            ...
            if (is_null($course_setting_relation) || !$course_setting_relation)
            {
                ...
                {
                    $course_setting_relation =
                        $base_object->create_course_setting_relation($available_setting, $locked);
                }
                catch (Exception $e)
                {
                    $succes = false;
    ...
    protected function load_settings($setting_type, $object_ids)
    {
        ...
        foreach ($course_setting_objects as $course_setting_object)
        {
            $setting_type_caching_hash = $this->get_setting_type_caching_hash(
                $setting_type, $course_setting_object[CourseSettingRelation::ALIAS_OBJECT_ID]
            );
    ...
    {
        /**
         * @var \Chamilo\Libraries\Storage\Cache\DataClassRepositoryCache $dataClassRepositoryCache
         */
        $dataClassRepositoryCache = DependencyInjectionContainerBuilder::getInstance()->createContainer()->get(
            'Chamilo\Libraries\Storage\Cache\DataClassRepositoryCache'
        );
        $dataClassRepositoryCache->reset();
