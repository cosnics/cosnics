...
class ContentObjectPublicationForm extends BasePublicationForm
{
    ...
    public function build_basic_create_form()
    {
        ...
        $this->addElement(
            'static', null, '', '<div class="email-not-possible alert alert-info hidden">' .
            $this->getTranslation('SendEmailNotPossibleForDelayedPublications') . '</div>'
        );

        $this->addElement(
            'checkbox', ContentObjectPublication::PROPERTY_SHOW_ON_HOMEPAGE, Translation::get('ShowOnHomepage')
        );
    }
    ...
    public function build_basic_form()
    {
        ...
        if ($this->is_course_admin || WeblcmsRights::getInstance()->is_allowed_in_courses_subtree(
                WeblcmsRights::ADD_RIGHT, $tool->get_id(), WeblcmsRights::TYPE_COURSE_MODULE, $this->get_course_id(),
                $this->user->getId()
            ))
        {
        ...
        {
            $this->addElement(
                'checkbox', ContentObjectPublication::PROPERTY_ALLOW_COLLABORATION,
        ...
        {
            $this->addElement(
                'hidden', ContentObjectPublication::PROPERTY_ALLOW_COLLABORATION,
                Translation::get('CourseAdminCollaborate')
            );
        }
    }
    ...
    public function get_categories($parent_id)
    {
        ...
        foreach ($cats as $cat)
        {
            if ($this->is_course_admin || WeblcmsRights::getInstance()->is_allowed_in_courses_subtree(
                    ...
                    $this->get_course_id(), $this->user->getId()
                ))
            {
    ...
    public function set_publication_rights(ContentObjectPublication $publication, $category_changed = false)
    {
        ...
        if ($category_changed)
        {
            $new_parent_id = WeblcmsRights::getInstance()->get_weblcms_location_id_by_identifier_from_courses_subtree(
                WeblcmsRights::TYPE_COURSE_CATEGORY, $publication->get_category_id(), $publication->get_course_id()
            );
