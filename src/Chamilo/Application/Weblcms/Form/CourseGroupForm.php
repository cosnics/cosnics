...
class CourseGroupForm extends FormValidator
{
    ...
    function create_course_group()
    {
         ...
         */
        /*
         * else if ($qty == 1) { foreach ($new_titles as $title) { $course_group = new CourseGroup();
         * $course_group->set_name($values[$title]); $course_group->set_description($values[CourseGroup ::
         * PROPERTY_DESCRIPTION]); $course_group->set_max_number_of_members($values[CourseGroup ::
         * PROPERTY_MAX_NUMBER_OF_MEMBERS]); $course_group->set_self_registration_allowed($values[CourseGroup ::
         * PROPERTY_SELF_REG]); $course_group->set_self_unregistration_allowed($values[CourseGroup ::
         * PROPERTY_SELF_UNREG]); $course_group->set_parent_id($values[CourseGroup :: PROPERTY_PARENT_ID]);
         * $course_group->set_course_code($course_code); array_push($groups, $course_group); } }
         */
