...
class WikiPageMostActiveUsersBlock extends ToolBlock
{
    ...
    public function count_data()
    {
        ...
        $reporting_data->set_rows(
            [Translation::get('MostActiveUser'), Translation::get('NumberOfContributions')]
        );

        $complex_content_object_item = \Chamilo\Core\Repository\Storage\DataManager::retrieve_by_id(
            ComplexContentObjectItem::class,
            $this->getRequest()->query->get(Manager::PARAM_SELECTED_COMPLEX_CONTENT_OBJECT_ITEM_ID)
        );
        ...
        {
            $users[$version->get_owner_id()] =
                isset($users[$version->get_owner_id()]) ? $users[$version->get_owner_id()] ++ : 1;
        }
        arsort($users);
        $keys = array_keys($users);
        $user = DataManager::retrieve_by_id(
            User::class, (int) $keys[0]
        );

        $reporting_data->add_category(0);
        $reporting_data->add_data_category_row(
            0, Translation::get('MostActiveUser'), $user->get_fullname() . ' (' . $user->get_username() . ')'
        );
